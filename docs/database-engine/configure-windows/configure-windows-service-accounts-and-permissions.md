---
title: "Configurar contas de servi&#231;o e permiss&#245;es do Windows | Microsoft Docs"
ms.custom: ""
ms.date: "03/13/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "estados do serviço de inicialização [SQL Server]"
  - "Instalação [SQL Server], contas de usuário"
  - "permissões do Windows [SQL Server]"
  - "modificando contas de usuário"
  - "contas padrão"
  - "domínios [SQL Server], contas de usuário"
  - "contas de inicialização [SQL Server]"
  - "contas do sistema [SQL Server]"
  - "serviços [SQL Server], permissões"
  - "ACL (lista de controle de acesso)"
  - "contas Sistema Local [SQL Server]"
  - "serviços que reconhecem a instância [SQL Server]"
  - "permissões [SQL Server], serviços"
  - "Serviço do SQL Server Agent, contas de usuário"
  - "permissões do Windows NT [SQL Server]"
  - "contas de usuário [SQL Server]"
  - "identificando serviços que não reconhecem a instância [SQL Server]"
  - "instalando o SQL Server, contas de usuário"
  - "estado da inicialização desabilitado [SQL Server]"
  - "contas de usuário [SQL Server], usuários"
  - "conta de Serviço Local [SQL Server]"
  - "Assistente para Instalação do SQL Server"
  - "serviços que não reconhecem a instância [SQL Server]"
  - "serviços [SQL Server], configurando na instalação"
  - "contas do Windows [SQL Server]"
  - "Serviços do SQL Server, contas de usuário"
  - "contas de usuário [SQL Server], serviços"
  - "MSSQLServer"
  - "identificando serviços que reconhecem a instância [SQL Server]"
  - "serviços [SQL Server], contas"
  - "listas de controle de acesso"
  - "contas opcionais [SQL Server]"
  - "contas de serviço [SQL Server]"
  - "contas [SQL Server], serviços"
  - "contas de sistema internas [SQL Server]"
  - "estado da inicialização automática"
  - "domínios [SQL Server]"
  - "estado da inicialização manual [SQL Server]"
  - "contas [SQL Server], usuário"
...
##  <a name="a-nameservicedetailsa-services-installed-by-includessnoversiontokenssnoversionmdmd"></a><a name="Service_Details"></a> Serviços instalados pelo [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
...
-   **[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]**: fornece suporte de gerenciamento para o armazenamento e a execução de pacotes do [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]. O caminho do arquivo executável é \<MSSQLPATH>\130\DTS\Binn\MsDtsSrvr.exe  
...
##  <a name="a-nameservpropa-service-properties-and-configuration"></a><a name="Serv_Prop"></a> Propriedades e configuração do serviço  
...
###  <a name="a-namedefaultacctsa-default-service-accounts"></a><a name="Default_Accts"></a> Contas de serviço padrão  
...
####  <a name="a-namechangingaccountsa-changing-account-properties"></a><a name="Changing_Accounts"></a> Alterando as propriedades da conta  
...
###  <a name="a-namenewaccountsa-managed-service-accounts-group-managed-service-accounts-and-virtual-accounts"></a><a name="New_Accounts"></a> Contas de serviço gerenciado, contas de serviço gerenciado de grupo e contas virtuais  
...
-   <a name="VA_Desc"></a>**Contas virtuais**  
    ...
    |Serviço [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent na instância padrão de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]|**NT SERVICE\SQLSERVERAGENT**|  
    |Serviço [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent em uma instância de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] chamada **PAYROLL**|**NT SERVICE\SQLAGENT$PAYROLL**|  
...
###  <a name="a-nameautostarta-automatic-startup"></a><a name="Auto_Start"></a> Inicialização automática  
...
###  <a name="a-nameconfigureservicesa-configuring-services-during-unattended-installation"></a><a name="Configure_services"></a> Configurando serviços durante a instalação autônoma  
...
###  <a name="a-namefirewalla-firewall-port"></a><a name="Firewall"></a> Porta do firewall  
...
##  <a name="a-nameservperma-service-permissions"></a><a name="Serv_Perm"></a> Permissões de serviço  
...
###  <a name="a-namewindowsa-windows-privileges-and-rights"></a><a name="Windows"></a> Privilégios e direitos do Windows  
...
###  <a name="a-namereviewingaclsa-file-system-permissions-granted-to-sql-server-per-service-sids-or-local-windows-groups"></a><a name="Reviewing_ACLs"></a> Permissões do sistema de arquivos concedidas a SIDs por serviço do SQL Server ou a grupos locais do Windows  
...
||\<ToolsDir>\DReplayController\\{all dlls}|Leitura, Execução, Listar Conteúdo de Pastas|  
...
||\<ToolsDir>\DReplayClient\ (all dlls)|Leitura, Execução, Listar Conteúdo de Pastas|  
...
###  <a name="a-namefilesystemothera-file-system-permissions-granted-to-other-windows-user-accounts-or-groups"></a><a name="File_System_Other"></a> Permissões do sistema de arquivos concedidas a outros grupos ou contas de usuário do Windows  
...
||\<Conta do serviço Web do Servidor de Relatório>|*\<install>*\Reporting Services\ReportServer|Leitura|  
||\<Conta do serviço Web do Servidor de Relatório>|*\<install>*\Reporting Services\ReportServer\global.asax|Full (cheio)|  
...
||Serviço de Rede|*\<install>*\Reporting Services\ReportServer\ReportService.asmx|Full (cheio)|  
...
###  <a name="a-nameunusuallocationsa-file-system-permissions-related-to-unusual-disk-locations"></a><a name="Unusual_Locations"></a> Permissões do sistema de arquivos relacionadas a locais de disco incomuns  
...
###  <a name="a-namereviewadditionalconsiderationsa-reviewing-additional-considerations"></a><a name="Review_additional_considerations"></a> Revisando considerações adicionais  
...
###  <a name="a-nameregistrya-registry-permissions"></a><a name="Registry"></a> Permissões de Registro  
 O hive de Registro é criado sob **HKLM\Software\Microsoft\Microsoft SQL Server\\***<Instance_ID>* para componentes com reconhecimento de instância. Por exemplo  
...
###  <a name="a-namepipesa-named-pipes"></a><a name="Pipes"></a> Pipes nomeados  
...
##  <a name="a-nameprovisioninga-provisioning"></a><a name="Provisioning"></a> Provisionamento  
...
###  <a name="a-namedeprova-database-engine-provisioning"></a><a name="DE_Prov"></a> Provisionamento do Mecanismo de Banco de Dados  
 As contas a seguir são adicionadas como logons ao [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)].  
  
####  <a name="a-namewinprincipalsa-windows-principals"></a><a name="Win_Principals"></a> Entidades de segurança do Windows  
...
####  <a name="a-namesaa-sa-account"></a><a name="sa"></a> Conta sa  
...
####  <a name="a-nameloginsa-sql-server-per-service-sid-login-and-privileges"></a><a name="Logins"></a> Logon e privilégios de SID por serviço do SQL Server  
...
####  <a name="a-nameagenta-sql-server-agent-login-and-privileges"></a><a name="Agent"></a> Logon e privilégios do SQL Server Agent  
...
####  <a name="a-namehadrona-includesshadrctokensshadrcmdmd-and-sql-failover-cluster-instance-and-privileges"></a><a name="Hadron"></a> [!INCLUDE[ssHADRc](../../includes/sshadrc-md.md)] e privilégios e instância de cluster de failover do SQL  
...
####  <a name="a-namewritera-sql-writer-and-privileges"></a><a name="Writer"></a> Gravador e privilégios do SQL  
...
####  <a name="a-namesqlwmia-sql-wmi-and-privileges"></a><a name="SQLWMI"></a> WMI e privilégios do SQL  
...
###  <a name="a-namessasa-ssas-provisioning"></a><a name="SSAS"></a> Provisionamento SSAS  
...
###  <a name="a-namessrsa-ssrs-provisioning"></a><a name="SSRS"></a> Provisionamento SSRS  
...
##  <a name="a-nameupgradea-upgrading-from-previous-versions"></a><a name="Upgrade"></a> Atualização de versões anteriores  
...
##  <a name="a-nameappendixa-appendix"></a><a name="Appendix"></a> Apêndice  
...
###  <a name="a-nameservacctsa-description-of-service-accounts"></a><a name="Serv_Accts"></a> Descrição de contas de serviço  
...
####  <a name="a-nameanyosa-accounts-available-with-any-operating-system"></a><a name="Any_OS"></a> Contas disponíveis com qualquer sistema operacional  
...
>  Se você configurar o aplicativo para usar uma conta de domínio, poderá isolar os privilégios para o aplicativo, mas deverá gerenciar senhas manualmente ou criar uma solução personalizada para gerenciar essas senhas. Muitos aplicativos para servidores usam essa estratégia para aprimorar a segurança, mas essa estratégia requer administração adicional e complexidade. Nessas implantações, os administradores de serviço gastam um tempo considerável em tarefas de manutenção, como o gerenciamento de senhas de serviço e SPNs, que são necessários para a autenticação Kerberos. Além disso, essas tarefas de manutenção podem interromper serviço.  
 ...
 <a name="Local_User"></a> **Contas de usuário locais**  
...
###  <a name="a-nameidentifyinstanceawareandunawarea-identifying-instance-aware-and-instance-unaware-services"></a><a name="Identify_instance_aware_and_unaware"></a> Identificação de serviços com e sem reconhecimento de instância  
...
###  <a name="a-namelocalizedservicenamesa-localized-service-names"></a><a name="Localized_service_names"></a> Nomes de serviços localizados  
