...
-   Requisitos gerais para executar a ferramenta de configuração [ferramentas de configuração do PowerPivot](power-pivot-sharepoint/power-pivot-configuration-tools.md).  
...
-   A exceção a essa regra na Ferramenta de Configuração do PowerPivot para SharePoint 2010 é a conta de serviço do Analysis Services. Essa conta é especificada durante a instalação e digite uma senha para essa conta no **registrar o SQL Server Analysis Services (PowerPivot no servidor Local)** ação. A página resumida não inclui um campo para esta senha; portanto, insira-a na página para aquela ação.  
 ...
 A Ferramenta de Configuração do PowerPivot usa o Windows PowerShell para configurar o servidor. Você pode clicar no **Script** para analisar o script do Windows PowerShell o.  
...
|**Ativar recurso PowerPivot em um conjunto de sites**|URL de site||URL da coleção de sites para a qual você está ativando recursos do PowerPivot.|  
