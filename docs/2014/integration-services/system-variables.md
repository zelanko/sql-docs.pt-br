...
# <a name="system-variables"></a>Variáveis do sistema
  [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)][!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] fornece um conjunto de variáveis do sistema que armazenam informações sobre o pacote em execução e seus objetos. Essas variáveis podem ser usadas em expressões e expressões de propriedade para personalizar pacotes, contêineres, tarefas e manipuladores de eventos.  
...
|**Interativomode**|Boolean|Indica se o pacote é executado em modo interativo. Se um pacote estiver sendo executado no [!INCLUDE[ssIS](../includes/ssis-md.md)] Designer, essa propriedade será definida como `True`. Se um pacote estiver em execução usando o utilitário de prompt de comando **dtexec** , a propriedade `False`será definida como.|  
...
|**Tarefa**|String|O nome da instância da tarefa.|  
