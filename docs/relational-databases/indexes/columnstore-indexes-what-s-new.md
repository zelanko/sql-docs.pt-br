---
title: "Resumo de recursos com vers&#227;o dos &#237;ndices columnstore | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "11/17/2016"
...
|Recurso do índice columnstore|[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]|[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]|[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]|[!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Edição Premium|[!INCLUDE[ssSDW](../../includes/sssdw-md.md)]|  
|-------------------------------|---------------------------|---------------------------|---------------------------|--------------------------------------------|-------------------------|  
|Execução de lote para consultas com multithread|sim|sim|sim|sim|sim|  
|Execução de lote para consultas com um único thread|||sim|sim|sim|  
|Opção de compactação de arquivamento.||sim|sim|sim|sim|  
|Isolamento de instantâneo e isolamento de instantâneo de leitura confirmada|||sim|sim|sim|  
|Especificação do índice columnstore durante a criação de uma tabela.|||sim|sim|sim|  
|Suporte a índices columnstore pelo AlwaysOn.|sim|sim|sim|sim|sim|  
|Suporte ao índice columnstore não clusterizado somente leitura pelo secundário legível AlwaysOn|sim|sim|sim|sim|sim|  
|Suporte ao índice columnstore atualizável pelo secundário legível AlwaysOn|||sim|||  
|Índice columnstore não clusterizado somente leitura no heap ou árvore b.|sim|sim|sim*|sim*|sim*|  
|Índice columnstore não clusterizado atualizável no heap ou árvore b|||sim|sim|sim|  
|Índices adicionais de árvore b permitidos em um heap ou árvore b com um índice columnstore não clusterizado.|sim|sim|sim|sim|sim|  
|Índice columnstore clusterizado e atualizável.||sim|sim|sim|sim|  
|Índice de árvore b em um índice columnstore clusterizado|||sim|sim|sim|  
|Índice columnstore em uma tabela com otimização de memória.|||sim|sim|sim|  
|Suporte ao uso de uma condição filtrada pela definição de índice columnstore não clusterizado.|||sim|sim|sim|  
|Opção de atraso de compactação para índices columnstore em CREATE TABLE e ALTER TABLE.|||sim|sim|sim|   
...
## [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]  
 O [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] adiciona aprimoramentos importantes para melhorar o desempenho e a flexibilidade dos índices columnstore. Isso aprimora os cenários de data warehouse e habilita a análise operacional em tempo real.  
...
-   A definição do índice columnstore não clusterizado oferece suporte ao uso de uma condição filtrada. Use esse recurso para criar um índice columnstore não clusterizado apenas nos dados sem vida de uma carga de trabalho operacional. Ao fazer isso, o impacto no desempenho de um índice columnstore em uma tabela OLTP será mínimo.  
...
-   Os índices columnstore têm uma opção de atraso de compactação que minimiza o impacto da carga de trabalho transacional na análise operacional em tempo real.  Essa opção permite que as linhas alteradas frequentemente estabilizem antes de compactá-las no columnstore. Para obter detalhes, consulte [CREATE COLUMNSTORE INDEX &#40;Transact-SQL&#41;](../../t-sql/statements/create-columnstore-index-transact-sql.md) e [Get started with Columnstore for real time operational analytics](../../relational-databases/indexes/get-started-with-columnstore-for-real-time-operational-analytics.md) (Introdução ao Columnstore para análise operacional em tempo real).  
...
-   O columnstore oferece suporte à desfragmentação de índice por meio da remoção de linhas excluídas, sem a necessidade de recriar explicitamente o índice. A instrução ALTER INDEX … REORGANIZE removerá do columnstore as linhas excluídas, com base em uma política definida internamente, como uma operação online  
...
-   O processamento de lote fornece um desempenho de consulta 2 vezes melhor ou mais, mas está disponível apenas para execução de consulta paralela.  
