...
|**reason_code**|**int**|O código de erro que pode ser sensível ao contexto. No caso de conflitos UPDATE-UPDATE e Update-Delete, o valor usado para essa coluna é o mesmo que o **conflict_type**. No entanto, em conflitos de alteração com falha, o código da razão é o erro que impediu o Merge Agent de aplicar a alteração. Por exemplo, se o Agente de Mesclagem não puder aplicar uma inserção no Assinante devido a uma violação de chave primária, ele registrará um **conflict_type** de 6 ("falha no download de inserção") e uma **reason_code** de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 2627, que é a mensagem de erro interna para uma violação de chave primária: "violação da restrição% ls '%. * ls '. Não é possível inserir a chave duplicada no objeto '%. \*ls "."|  
