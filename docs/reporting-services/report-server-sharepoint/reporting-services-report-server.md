---
title: "O servidor de relatório do Reporting Services | Microsoft Docs"
ms.custom: 
ms.date: 09/25/2017
...
Saiba mais sobre a parte central de um [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instalação do Reporting Services. Ela consiste em um mecanismo de processamento juntamente com extensões para adicionar funcionalidade.

> [!NOTE]
> Integração do Reporting Services com o SharePoint não está mais disponível após o SQL Server 2016.

Um servidor de relatório do Reporting Services é executado em um dos dois modos de implantação; Modo nativo ou modo do SharePoint. Consulte a seção [Comparação de recursos do SharePoint e do modo nativo](#bkmk_featuresupport) para obter uma comparação entre os recursos.  
...
-   **Modo nativo**: incluindo o modo nativo com web parts do SharePoint, onde um servidor de relatório é executado como um servidor de aplicativo que fornece todos os recursos de processamento e gerenciamento exclusivamente através de componentes do Reporting Services. Você pode configurar um servidor de relatório de modo nativo com o Reporting Services configuration manager e o SQL Server Management Studio.  
...
|**Extensões de segurança personalizadas**|Sim|Extensões do Reporting Services personalizados de segurança não podem ser implantada ou usada no servidor de relatório. O servidor de relatório inclui uma extensão de segurança para fins especiais que é usada sempre que você configura um servidor de relatório para ser executado no modo integrado do SharePoint. Essa extensão de segurança é um componente interno e é necessário para operações integradas.|  
...
|**Relatórios .RDL**|Sim|Sim<br /><br /> . Relatórios RDL podem executar no Reporting Services servidores de relatório no modo nativo ou no modo do SharePoint.|  
|**Relatórios .RDLX**|Não|Sim<br /><br /> O Power View. Relatórios RDLX só podem executar no Reporting Services servidores de relatório no modo do SharePoint.|  
...
-   Um servidor de aplicativos do SharePoint com o serviço compartilhado do Reporting Services instalado e pelo menos um aplicativo de serviço do Reporting Services.  
 ...
 A ilustração a seguir mostra um ambiente do Reporting Services de modo do SharePoint:  
...
||Description|  
|-|-----------------|  
|**(1)**|Servidores Web ou WFE (front-ends da Web). O suplemento Reporting Services deve ser instalado em cada servidor web do qual você deseja utilizar os recursos de aplicativo da web como exibir relatórios ou páginas de gerenciamento do Reporting Services para tarefas como gerenciar fontes de dados ou assinaturas.|  
|**(2)**|O suplemento instala pontos de extremidade de URL e SOAP para clientes para se comunicar com os servidores de aplicativos, por meio do proxy de serviço do Reporting Services.|  
|**(3)**|Servidores de aplicativos executando o serviço compartilhado do Reporting Services. Expansão do processamento do relatório é gerenciada como parte do farm do SharePoint e adicionando o serviço Reporting Services para outros servidores de aplicativos.|  
|**(4)**|Você pode criar mais de um aplicativo de serviço do Reporting Services, com configurações diferentes inclusive permissões, email, proxy e assinaturas.|  
|**(5)**|Relatórios, fontes de dados e outros itens são armazenados nos bancos de dados de conteúdo do SharePoint.|  
|**(6)**|O Reporting Services criam três bancos de dados do servidor de relatório, temporário e recursos de alertas de dados. Os parâmetros de configuração que se aplicam a todos os aplicativos de serviço do SSRS são armazenadas no arquivo **RSReportserver.config** .|  
...
|Instale o Reporting Services no modo do SharePoint.|[Instalar o Reporting Services SharePoint Mode para SharePoint 2010](http://msdn.microsoft.com/47efa72e-1735-4387-8485-f8994fb08c8c)|  
