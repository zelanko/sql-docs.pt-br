...
helpviewer_keywords: 
  ...
  - "mineração de dados [Analysis Services], sequências"
  - "discriminação [Analysis Services]"
  - "conteúdo do modelo de mineração, exibindo"
  - "modelos de mineração [Analysis Services], sequências"
  - "Microsoft Sequence Cluster Viewer"
  - "sequence [Analysis Services]"
  - "transições [Analysis Services]"
...
###  <a name="BKMK_Profile"></a> Perfis de Cluster  
 A guia **Perfil de Cluster** oferece uma exibição geral dos clusters criados pelo algoritmo em seu modelo. Cada coluna após a coluna **População** na grade representa um cluster descoberto pelo modelo. A linha \<attribute>.samples representa sequências diferentes de dados existentes no cluster e a linha \<attribute> descreve todos os itens contidos no cluster, bem como sua distribuição geral.  
