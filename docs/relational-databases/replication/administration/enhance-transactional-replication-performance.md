...
helpviewer_keywords: 
  ...
  - "desempenho [replicação do SQL Server], replicação transacional"
  - "criando bancos de dados [SQL Server], desempenho de replicação"
  - "desempenho [replicação do SQL Server], replicação de instantâneo"
  ...
  - "Agente de Distribuição, desempenho"
  - "replicação transacional, desempenho"
  - "Log Reader Agent, desempenho"
...
-   Dimensione adequadamente o banco de dados de distribuição.  
     ...
     Teste a replicação com uma carga típica para seu sistema para determinar a quantidade de espaço necessária para armazenar comandos. Certifique-se de que o banco de dados seja amplo o suficiente para armazenar comandos, sem ter que aumentá-lo frequentemente. Para obter mais informações sobre como alterar o tamanho de um banco de dados, consulte [ALTER DATABASE & #40. O Transact-SQL e 41;](../../../t-sql/statements/alter-database-transact-sql.md).  
...
-   Espalhe os artigos por várias publicações.  
     ...
     Se você não pode usar o **- SubscriptionStreams** parâmetro (descrito posteriormente neste tópico), considere a criação de várias publicações. Espalhar os artigos por essas publicações permite à replicação aplicar alterações em paralelo aos Assinantes.  
...
-   Para resolver gargalos esporádicos e acidentais, use o **– MaxCmdsInTran** parâmetro para o Log Reader Agent.  
...
-   Use o parâmetro **–SubscriptionStreams** para o Agente de Distribuição.  
     ...
     Um valor para esse parâmetro do agente pode ser especificado usando o **@subscriptionstreams** de [sp_addsubscription & #40. O Transact-SQL e 41;](../../../relational-databases/system-stored-procedures/sp-addsubscription-transact-sql.md).  
  
-   Aumente o valor da **- ReadBatchSize** parâmetro para o Log Reader Agent.  
  
     O Log Reader Agent e o Distribution Agent dão suporte ao tamanho dos lotes para as operações de leitura e confirmação de transações. O tamanho padrão dos lotes é de 500 transações. O Log Reader Agent lê o número específico de transações do log, mesmo essas estando marcadas ou não, para a replicação. Quando um grande número de transações é gravado em um banco de dados de publicação, mas apenas um pequeno subconjunto desses são marcadas para replicação, você deve usar o **- ReadBatchSize** para aumentar o tamanho de lote de leitura do Log Reader Agent. Esse parâmetro não se aplica aos Editores Oracle.  
  
-   Aumente o valor da **- CommitBatchSize** parâmetro para o Distribution Agent.  
