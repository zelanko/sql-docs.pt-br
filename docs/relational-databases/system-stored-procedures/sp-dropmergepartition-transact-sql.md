...
`[ @suser_sname = ] 'suser_sname'` É o valor de [SUSER_SNAME](../../t-sql/functions/suser-sname-transact-sql.md) função no assinante usado para definir a partição. *suser_sname* está **sysname**, sem padrão.  
  
`[ @host_name = ] 'host_name'` É o valor de [HOST_NAME](../../t-sql/functions/host-name-transact-sql.md) função no assinante usado para definir a partição. *HOST_NAME* está **sysname**, sem padrão.  
