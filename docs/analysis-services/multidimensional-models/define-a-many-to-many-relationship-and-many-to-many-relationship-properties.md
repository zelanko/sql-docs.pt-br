...
ms.topic: "article"
helpviewer_keywords: 
  - "relações muitos-para-muitos [Analysis Services]"
...
---
# Definir uma rela&#231;&#227;o muitos-para-muitos e propriedades da rela&#231;&#227;o muitos-para-muitos
...
-   No Adventure Works, muitos clientes têm muitos motivos para pedir um produto, e um motivo de vendas pode ser associado a vários pedidos.  
 ...
 ![Ícone de muitos para muitos no uso da dimensão](../../analysis-services/multidimensional-models/media/ssas-m2m-icondimusage.png "Ícone de muitos para muitos no uso da dimensão")  
 ...
 ![Botão Definir relações no uso da dimensão](../../analysis-services/multidimensional-models/media/ssas-m2m-btndimusage.png "Botão Definir relações no uso da dimensão")  
...
3.  Verifique se todas as tabelas que você planeja usar nas relações muitos-para-muitos estão relacionadas na DSV por meio de relações de chave primária. Esse é um requisito para estabelecer um link para o grupo de medidas intermediário em uma etapa subsequente.  
    ...
    >  Se a fonte de dados subjacente não fornecer relações de chave primária e estrangeira, você poderá criar as relações manualmente na DSV. Para obter mais informações, consulte [Definir relações lógicas em uma exibição da fonte de dados &#40;Analysis Services&#41;](../../analysis-services/multidimensional-models/define-logical-relationships-in-a-data-source-view-analysis-services.md).  
     ...
     ![DSV que mostra as tabelas relacionadas](../../analysis-services/multidimensional-models/media/ssas-m2m-dsvpkeys.png "DSV que mostra as tabelas relacionadas")  
...
2.  Crie uma nova dimensão com base em uma tabela existente, **DimSalesReason**. Aceite todos os valores padrão ao especificar a origem.  
     ...
     ![Lista de atributos em uma nova dimensão](../../analysis-services/multidimensional-models/media/ssas-m2m-dimsalesreason.PNG "Lista de atributos em uma nova dimensão")  
...
4.  Em Especificar Informações sobre a Origem, você verá um aviso que indica que uma coluna Nome deve ser especificada. Escolha **SalesOrderNumber** como o Nome.  
     ...
     ![Dimensão de pedidos de vendas que exibe a coluna de nomes](../../analysis-services/multidimensional-models/media/ssas-m2m-dimsalesordersource.PNG "Dimensão de pedidos de vendas que exibe a coluna de nomes")  
  
5.  Na próxima página do assistente, escolha os atributos. Nesse exemplo, você pode selecionar apenas **SalesOrderNumber**.  
  
     ![Dimensão de pedidos de vendas que exibe a lista de atributos](../../analysis-services/multidimensional-models/media/ssas-m2m-dimsalesorderattrib.PNG "Dimensão de pedidos de vendas que exibe a lista de atributos")  
...
6.  Renomeie a dimensão como **Dim Sales Orders**para que você tenha uma convenção de nomenclatura consistente para as dimensões.  
     ...
     ![Página do assistente que mostra a renomeação da dimensão](../../analysis-services/multidimensional-models/media/ssas-m2m-dimsalesorders.PNG "Página do assistente que mostra a renomeação da dimensão")  
...
4.  Analise os resultados. Como estamos usando dados de exemplo, a impressão inicial é que todos os pedidos de vendas têm valores idênticos. No entanto, se você rolar para baixo, começará a ver variação de dados.  
     ...
     ![Planilha de Excel que mostra agregações muitos para muitos](../../analysis-services/multidimensional-models/media/ssas-m2m-excel.png "Planilha de Excel que mostra agregações muitos para muitos")  
...
5.  Role até a parte inferior da planilha. Agora é fácil ver que o Preço é o motivo mais importante para as compras dos clientes, em comparação com outros motivos assim como o total geral.  
     ...
     ![Pasta de trabalho do Excel que mostra totais em muitos para muitos](../../analysis-services/multidimensional-models/media/ssas-m2m-excelgrandtotal.png "Pasta de trabalho do Excel que mostra totais em muitos para muitos")  
...
## Consulte também  
 ...
 [Instalar dados de exemplo e projetos para o tutorial de modelagem multidimensional do Analysis Services](../../analysis-services/install sample data and projects.md)   
