...
description: Aborda o motivo pelo qual o Assistente de Migração do SQL Server (SSMA) para Oracle não pode converter algumas das instruções que têm o cursor como um tipo de retorno em qualquer função-erro de mensagem O2SS0245.
author: nahk-ivanov
ms.prod: sql
ms.technology: ssma
ms.devlang: sql
...
Considere o exemplo a seguir de ter um pacote `PackageName` que contém uma função `func_name` . Essa função retorna o cursor `out_cursor` definido dentro do pacote.
