...
`[ @job_id = ] job_id` número de identificação do trabalho ao qual o agendamento é adicionado. *job_id* é **uniqueidentifier**, sem padrão.  
...
|**64** (quando o serviço do agente de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] é iniciado)|*frequency_interval* não é usado.|  
...
`[ @freq_relative_interval = ] frequency_relative_interval` define ainda mais a *frequency_interval* quando *frequency_type* é definido como **32** (relativo mensal).  
...
`[ @active_start_time = ] active_start_time` horário em qualquer dia entre *active_start_date* e *active_end_date* para iniciar a execução do trabalho. *active_start_time* é **int**, sem padrão. A hora é formatada como HHMMSS em um relógio de 24 horas.  
  
`[ @active_end_time = active_end_time_` tempo em qualquer dia entre *active_start_date* e *active_end_date* para encerrar a execução do trabalho. *active_end_time* é **int**, sem padrão. A hora é formatada como HHMMSS em um relógio de 24 horas.  
...
-   **SQLAgentOperatorRole**  
 ...
 O exemplo a seguir atribui um plano de trabalho para `SaturdayReports` que será executado a cada sábado às 2:00.
```sql  
...
## <a name="see-also"></a>Consulte também  
 ...
 [Criar um agendamento](../../ssms/agent/create-a-schedule.md)   
