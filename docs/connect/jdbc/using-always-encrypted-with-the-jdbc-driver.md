...
| Classe                                                 | Descrição                                        | Nome (de pesquisa) do provedor  | Está previamente registrado? | Plataforma |
| :---------------------------------------------------- | :------------------------------------------------- | :---------------------- | :----------------- | :------- |
| **SQLServerColumnEncryptionAzureKeyVaultProvider**    | Um provedor para um repositório de chaves para o Azure Key Vault. | AZURE_KEY_VAULT         | _Não_ antes do JDBC driver versão 7.4.1, mas _sim_ do JDBC driver versão 7.4.1 em diante. | Windows, Linux, macOS |
| **SQLServerColumnEncryptionCertificateStoreProvider** | Um provedor para o Repositório de Certificados do Windows.      | MSSQL_CERTIFICATE_STORE | _Sim_                | Windows |
| **SQLServerColumnEncryptionJavaKeyStoreProvider**     | Um provedor para o repositório de chaves Java.                  | MSSQL_JAVA_KEYSTORE     | _Sim_                | Windows, Linux, macOS |
|||||
...
> [!IMPORTANT]
> As propriedades de conexão **keyVaultProviderClientId** e **keyVaultProviderClientKey** foram preteridas da v8.4.1 em diante. Os usuários são incentivados a usar **keyStoreAuthentication**, **KeyStorePrincipalId** e **KeyStoreSecret** no lugar.
...
### <a name="using-azure-key-vault-authentication-with-managed-identities"></a>Como usar a autenticação do Azure Key Vault com Identidades Gerenciadas

Do JDBC Driver **8.4.1** em diante, o driver adicionou suporte para autenticar os Azure Key Vaults usando identidades gerenciadas.

Se o aplicativo estiver hospedado no Azure, o usuário poderá usar [Identidades Gerenciadas](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview) para autenticar-se para o Azure Key Vault, eliminando a necessidade de fornecer e expor todas as credenciais no código. 

#### <a name="connection-properties-for-key-vault-authentication-with-managed-identities"></a>Propriedades de conexão para autenticação do Key Vault com identidades gerenciadas
...
| keyStorePrincipalId   | \<Azure AD Application Client ID\>    | \<Azure AD Application object ID\> (opcional)| N/D |
...
#### <a name="use-managed-identity-to-authenticate-to-akv"></a>Usar Identidade Gerenciada para autenticação no AKV
```
"jdbc:sqlserver://<server>:<port>;columnEncryptionSetting=Enabled;keyStoreAuthentication=KeyVaultManagedIdentity;"
```
#### <a name="use-managed-identity-and-the-principal-id-to-authenticate-to-akv"></a>Usar Identidade Gerenciada e a ID da entidade de segurança para autenticação no AKV
```
...
```
#### <a name="use-clientid-and-clientsecret-to-authentication-to-akv"></a>Usar clientId e clientSecret para autenticação no AKV
```
...
```
Os usuários são incentivados a usar essas propriedades de conexão para especificar o tipo de autenticação usado para os Repositórios de Chaves em vez de usar o `SQLServerColumnEncryptionAzureKeyVaultProvider`.

Observe que as propriedades de conexão `keyVaultProviderClientId` e `keyVaultProviderClientKey` adicionadas anteriormente são preteridas e substituídas pelas propriedades de conexão descritas acima.

Para obter informações sobre como configurar Identidades Gerenciadas, confira [Configurar Identidades Gerenciadas para recursos do Azure em uma VM usando o portal do Azure](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/qs-configure-portal-windows-vm).
...
> [!IMPORTANT]
> Embora os outros provedores de repositório de chaves neste artigo estejam disponíveis em todas as plataformas compatíveis com o driver, a implementação SQLServerColumnEncryptionCertificateStoreProvider do JDBC driver está disponível somente em sistemas operacionais Windows. Ela tem uma dependência do mssql-jdbc_auth-\<version>-\<arch>.dll que está disponível no pacote do driver. Para usar esse provedor, copie o arquivo mssql-jdbc_auth-\<version>-\<arch>.dll para um diretório no caminho do sistema Windows no computador em que o driver JDBC está instalado. Como alternativa, você pode definir a propriedade do sistema java.libary.path para especificar o diretório de mssql-jdbc_auth-\<version>-\<arch>.dll. Se você estiver executando uma JVM (Máquina Virtual Java) de 32 bits, use o arquivo mssql-jdbc_auth-\<version>-x86.dll na pasta x86, mesmo se o sistema operacional for a versão x64. Se estiver executando uma JVM de 64 bits em um processador x64, use o arquivo mssql-jdbc_auth-\<version>-x64.dll na pasta x64. Por exemplo, se você estiver usando a JVM de 32 bits e o driver JDBC estiver instalado no diretório padrão, você poderá especificar o local da DLL usando o seguinte argumento de VM (máquina virtual) quando o aplicativo Java for iniciado: `-Djava.library.path=C:\Microsoft JDBC Driver <version> for SQL Server\sqljdbc_<version>\enu\auth\x86`
...
As configurações de SQLServerStatementColumnEncryptionSetting não podem ser usadas para ignorar a criptografia e obter acesso a dados de texto sem formatação. Para obter mais informações sobre como configurar a criptografia de coluna em uma instrução, confira [Referência à API Always Encrypted para o JDBC Driver](always-encrypted-api-reference-for-the-jdbc-driver.md).  
