...
## <a name="intrinsic-properties"></a>Propriedades intrínsecas  
 ...
 Isso converte o objeto do gerenciador de conexões gerenciado em seu objeto de conexão subjacente. Se você estiver usando C++, o método `QueryInterface` do objeto <xref:Microsoft.SqlServer.Dts.Runtime.ConnectionManager> será chamado e a interface do objeto de conexão subjacente será requisitado.  
