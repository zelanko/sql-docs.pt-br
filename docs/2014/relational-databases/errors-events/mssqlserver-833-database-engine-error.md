...
|Texto da mensagem|SQL Server encontrou %d ocorrência (s) de solicitações de e/s demorando mais de %d segundos para ser concluído em [%ls] do arquivo no banco de dados`[%ls] (%d)`.  O identificador de arquivo do SO é 0x%p.  O deslocamento da E/S mais demorada é: %#016I64x.|  
