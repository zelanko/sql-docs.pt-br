...
# <a name="audit-server-operation-event-class"></a>Classe de evento Audit Server Operation
  A classe de evento **Audit Server Operation** ocorre quando são usadas operações de auditoria de segurança, como alteração de configurações, recursos, acesso externo ou autorização.  
