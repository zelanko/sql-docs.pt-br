---
title: Colunas SQL | Microsoft Docs
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
 ...
 **SQLColumns** podem ser executados em um cursor de servidor estático. Uma tentativa de executar **SQLColumns** em um cursor updatable (dinâmico ou keyset) retornará SQL_SUCCESS_WITH_INFO indicando que o tipo de cursor foi alterado.  
 ...
 Para ODBC 2. *x* aplicativos que não usam curingas em *TableName*, **SQLColumns** retorna informações sobre quaisquer tabelas cujos nomes correspondam a *TableName* e são de propriedade do usuário atual. Se o usuário atual não possuir nenhuma tabela cujo nome corresponda ao parâmetro *TableName,* **sQLColumns** retorna informações sobre quaisquer tabelas de outros usuários onde o nome da tabela corresponda ao parâmetro *TableName.* Para ODBC 2. *x* aplicativos usando curingas, **SQLColumns** retorna todas as tabelas cujos nomes correspondem *a TableName*. Para ODBC 3. *x* aplicativos **SQLColumns** retorna todas as tabelas cujos nomes correspondem *a TableName,* independentemente do proprietário ou se os curingas são usados.  
...
|COLUMN_SIZE|Retornos SQL_SS_LENGTH_UNLIMITED para tipos de dados **varchar(max)** indicando que o tamanho da coluna é ilimitado.|  
...
|SS_UDT_ASSEMBLY_TYPE_NAME|O nome qualificado do assembly do UDT.|  
 ...
 Para UDTs, a coluna TYPE_NAME existente é usada para indicar o nome do UDT; portanto, nenhuma coluna adicional para ela deve ser adicionada ao conjunto de resultados de **Colunas SQL** ou [SQLProcedureColumns](../../relational-databases/native-client-odbc-api/sqlprocedurecolumns.md). O DATA_TYPE de uma coluna ou parâmetro UDT é SQL_SS_UDT.  
 ...
 Quando um cliente [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] se conecta e chama SQLColumns, o uso de valores DE NULL ou wild card para o parâmetro de entrada do catálogo não retornará informações de outros catálogos. Em vez disso, serão retornadas apenas informações sobre o catálogo atual. O cliente pode primeiro ligar para o SQLTables para determinar em qual catálogo a tabela desejada está localizada. O cliente pode então usar esse valor de catálogo para o parâmetro de entrada do catálogo em sua chamada para SQLColumns para recuperar informações sobre as colunas nessa tabela.  
...
## <a name="sqlcolumns-support-for-enhanced-date-and-time-features"></a>Suporte de SQLColumns a recursos aprimorados de data e hora  
 Para obter informações sobre os valores retornados para os tipos de data/hora, consulte [Metadados do catálogo](../../relational-databases/native-client-odbc-date-time/metadata-catalog.md).  
 ...
 **O SQLColumns** suporta grandes tipos definidos pelo usuário CLR (UDTs). Para obter mais informações, consulte [Grandes tipos definidos pelo usuário da CLR &#40;&#41;ODBC ](../../relational-databases/native-client/odbc/large-clr-user-defined-types-odbc.md).  
  
## <a name="sqlcolumns-support-for-sparse-columns"></a>Suporte de SQLColumns para colunas esparsas  
 Duas [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] colunas específicas foram adicionadas ao conjunto de resultados para Colunas SQL:  
  
|Nome da coluna|Tipo de dados|Descrição|  
|-----------------|---------------|-----------------|  
|SS_IS_SPARSE|**Smallint**|Se a coluna for uma coluna esparsa, será SQL_TRUE; caso contrário, SQL_FALSE.|  
|SS_IS_COLUMN_SET|**Smallint**|Se a coluna for a coluna **column_set,** esta é SQL_TRUE; caso contrário, SQL_FALSE.|  
  
 Em conformidade com a especificação ODBC, SS_IS_SPARSE e SS_IS_COLUMN_SET aparecem diante [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] de todas [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]as colunas específicas do driver que foram adicionadas às versões anteriores , e depois de todas as colunas exigidas pela própria ODBC.  
 ...
 Para obter mais informações sobre colunas esparsas no ODBC, consulte [Colunas Esparsas Suporte &#40;ODBC&#41;](../../relational-databases/native-client/odbc/sparse-columns-support-odbc.md).  
