...
##  <a name="AvDbs"></a>Bancos de dados de disponibilidade  
 ...
 A junção coloca o banco de dados secundário no estado ONLINE e inicia sincronização de dados com o banco de dados primário correspondente. A *sincronização de dados* é o processo pelo qual as alterações em um banco de dado primário são reproduzidas em um banco de dados secundário. A sincronização de dados envolve o envio pelo banco de dados primário dos registros do log de transações ao banco de dados secundário.  
...
##  <a name="ClientConnections"></a>Conexões de cliente  
...
##  <a name="SessionTimeoutPerios"></a>Período de tempo limite da sessão  
 ...
 O período de tempo limite da sessão impede qualquer réplica de esperar indefinidamente para receber um ping de outra réplica. Se nenhum ping for recebido da outra réplica dentro do período de tempo limite da sessão, a réplica atingirá o tempo limite. Sua conexão está fechada e a réplica com tempo limite entra no estado desconectado. Até mesmo se uma réplica desconectada estiver configurada para o modo de confirmação assíncrona, as transações não esperarão que a réplica se reconecte e seja sincronizada novamente.  
