...
## <a name="remarks"></a>Remarks  
 ...
 Chamar **bcp_bind** ou [bcp_collen](bcp-collen.md) define o comprimento total de todas as partes de dados a serem copiados para a coluna do SQL Server. Uma tentativa de enviar o SQL Server mais bytes do que o especificado na chamada para **bcp_bind** ou `bcp_collen` gera um erro. Esse erro surgiria, por exemplo, em um aplicativo que usou `bcp_collen` para definir o comprimento dos dados disponíveis para um SQL Server `text` coluna como 4500 chamado **bcp_moretext** cinco vezes enquanto indicava em cada chamada Se os dados do buffer tinha 1.000 bytes.  
 ...
 Um aplicativo normalmente chama `bcp_sendrow` e **bcp_moretext** em loops enviar um número de linhas de dados. Eis um esboço de como fazer isso para uma tabela que contém duas `text` colunas:  
