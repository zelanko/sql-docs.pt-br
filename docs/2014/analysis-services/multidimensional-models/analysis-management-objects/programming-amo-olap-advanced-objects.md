...
3.  Adicione a ação à coleção de cubos e atualize o cubo. A ação não é um objeto atualizável.  
 ...
 O teste da ação exige um aplicativo de programa diferente. Você pode testar a sua ação no [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)]. Primeiro, você deve instalar [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] exemplos, consulte [processamento de objeto de modelo Multidimensional](../processing-a-multidimensional-model-analysis-services.md).  
