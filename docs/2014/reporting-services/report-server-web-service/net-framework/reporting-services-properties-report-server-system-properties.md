...
|SystemSnapshotLimit|O número máximo de instantâneos que são armazenados para um relatório. Os valores válidos são `-1` por meio de `2`,`147`,`483`,`647`. Se o valor for `-1`, não há nenhum limite de instantâneo.|  
...
|ExecutionLogDaysKept|O número de dias para manter informações de execução de relatório no log de execução. Os valores válidos para essa propriedade incluem `0` por meio de `2`,`147`,`483`,`647`. Se o valor for `0` as entradas não serão excluídas da tabela de log de execução. O valor padrão é `60`.|  
