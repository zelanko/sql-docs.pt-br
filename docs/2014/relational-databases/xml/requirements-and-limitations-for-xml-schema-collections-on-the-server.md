...
|**\<xsd:choice>**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]rejeita esquemas que têm uma **\<xsd:choice>** partícula sem filhos, a menos que a partícula seja definida com um valor de atributo **minOccurs** igual a zero.|  
|**\<xsd:include>**|Atualmente, o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] não oferece suporte a esse elemento. Os esquemas XML que incluem esse elemento são rejeitados pelo servidor.<br /><br /> Como uma solução, os esquemas XML que incluem a **\<xsd:include>** diretiva podem ser pré-processados para copiar e mesclar o conteúdo de quaisquer esquemas incluídos em um único esquema para carregar no servidor. Para obter mais informações, veja [Pré-processar um esquema para mesclar esquemas incluídos](preprocess-a-schema-to-merge-included-schemas.md).|  
...
|Atributo ID|Cada componente de esquema XML pode ter um atributo de ID. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]impõe exclusividade para **\<xsd:attribute>** declarações de tipo de **ID** , mas não armazena esses valores. O escopo para imposição de exclusividade é a instrução {CREATE &#124; ALTER} XML SCHEMA COLLECTION.|  
|Tipo ID|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]não oferece suporte a elementos do tipo **xs: ID**, **xs: IDREF**ou **xs: IDREFS**. Um esquema pode não declarar elementos desse tipo ou elementos derivados pela restrição ou extensão desse tipo.|  
|Namespace local|O namespace local deve ser especificado explicitamente para o **\<xsd:any>** elemento. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] rejeita esquemas que usam uma cadeia de caracteres vazia ("") como um valor para o atributo de namespace. Em vez disso, o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] exige o uso explícito de "##local" para indicar um elemento ou atributo não qualificado, como a instância do caractere curinga.|  
...
|[O elemento &#60;xsd:redefine&#62; Element](the-xsd-redefine-element.md)|Explica a limitação de usar o \<xsd:redefine> elemento e descreve uma solução alternativa.|  
|[The xs:QName Type](the-xs-qname-type.md)|Descreve a limitação relativa ao tipo xs:QName.|  
|[Valores para declarações &#60;xsd:simpleType&#62;](values-for-xsd-simpletype-declarations.md)|Descreve as restrições que são aplicadas às \<xsd:simpleType> declarações.|  
...
## <a name="see-also"></a>Consulte Também  
 [&#40;de dados XML SQL Server&#41;](xml-data-sql-server.md)   
