...
-   Uma restrição FOREIGN KEY  
 ...
 Ao trabalhar com **tempdb**, a cláusula [COLLATE](/sql/t-sql/statements/collations) inclui uma opção *database_default* para especificar que a coluna em uma tabela temporária usa o agrupamento padrão do banco de dados de usuário atual para a conexão em vez do agrupamento de **tempdb**.  
  
## <a name="collations-and-text-columns"></a>Agrupamentos e colunas de texto  
 Você pode inserir ou atualizar valores em um `text` cujo agrupamento seja diferente da página de código do agrupamento padrão do banco de dados de coluna. [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] converte implicitamente os valores para o agrupamento da coluna.  
