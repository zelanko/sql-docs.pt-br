---
title: Esquemas de mapeamento anotados para um updategram (SQLXML)
description: Saiba como um esquema de mapeamento XSD ou XDR anotado que é especificado em um updategram 4,0 do SQLXML é usado para processar atualizações em um banco de dados.
ms.date: 03/17/2017
ms.prod: sql
...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]
 ...
 Para especificar um esquema de mapeamento, use o atributo **Mapping-Schema** do **\<sync>** elemento. Os exemplos a seguir mostram dois diagramas de atualização: um que usa um esquema de mapeamento simples e outro que usa um esquema mais complexo.  
...
```  
 ...
 O updategram a seguir usa esse esquema XSD para adicionar um novo registro de detalhes do pedido (um **\<OD>** elemento no **\<after>** bloco) para a ordem 43860. O atributo **Mapping-Schema** é usado para especificar o esquema de mapeamento no updategram.  
...
### <a name="c-inserting-a-record-by-using-the-parent-child-relationship-and-inverse-annotation-specified-in-the-xsd-schema"></a>C. Inserindo um registro usando a relação pai-filho e a anotação de inverso especificada no esquema XSD  
 Este exemplo ilustra como a lógica updategram usa a relação pai-filho especificada no XSD para processar atualizações e como a anotação **inversa** é usada. Para obter mais informações sobre a anotação **inversa** , consulte [especificando o atributo SQL: inverso em sql: relationship &#40;SQLXML 4,0&#41;](../../../relational-databases/sqlxml-annotated-xsd-schemas-using/specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md).  
...
```  
 ...
 Para instruir a lógica do updategram a reverter a relação pai-filho durante a operação de atualização, a anotação **inversa** é especificada no **\<relationship>** elemento. Como resultado, os registros são adicionados primeiro na tabela Cust e, depois, na tabela Ord, e a operação é bem-sucedida.  
