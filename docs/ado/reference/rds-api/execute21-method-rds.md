...
ms.openlocfilehash: 13b3da0c799b0bc3ce14adb42df8a3d5d79398fe
 ...
 *QueryString*  
 Um comando na linguagem de comando suportado pelo provedor OLE DB identificado na cadeia de conexão. Para provedores baseados em SQL, ele pode conter um [!INCLUDE[tsql](../../../includes/tsql_md.md)] instrução de comando, mas para provedores não-SQL (por exemplo, MSDataShape) isso pode não ser um [!INCLUDE[tsql](../../../includes/tsql_md.md)] instrução de consulta.  
  
 Além disso, se um manipulador está sendo usado (e é altamente recomendável que um manipulador de ser usado), o manipulador pode alterar ou substituir o valor especificado aqui. Por exemplo, o manipulador geralmente substitui *QueryString* com uma cadeia de caracteres de consulta de seu arquivo. ini. Por padrão, o arquivo Msdfmap.ini é usado.  
  
 *lMarshalOptions*  
 Usado para definir as opções de marshaling no conjunto de linhas/registros que está sendo retornado.  
  
 *TableID*  
 Uma variante do tipo VT_EMPTY ou VT_BSTR. Se esse valor é do tipo VT_EMPTY, ele será ignorado. Se ela é do tipo VT_BSTR, o conjunto de registros é criado usando **adCmdTableDirect** usando o valor especificado aqui e *QueryString* parâmetro é ignorado.  
 ...
 *lExecuteOptions*  
 ...
 Embora *ReadOnly*, *NoBatch* e *GetHiddenColumns* são opções mutuamente exclusivas, não é um erro ao definir mais de um deles. Se várias opções forem definidas, *GetHiddenColumns* tem precedência sobre todas as outras opções, seguidas por *ReadOnly*. Se nenhuma opção for especificada, por padrão, o conjunto de registros é aberto usando **adLockBatchOptimistic** mas colunas ocultas não são incluídas no conjunto de registros.  
...
> [!NOTE]
>  O **Execute21** método é uma versão do [(RDS) do método Execute](../../../ado/reference/rds-api/execute-method-rds.md). Em que você precisa usar o **Execute** método para se comunicar com o ADO 2.1, o **Execute21** método pode ser chamado em vez disso. Os recursos do **Execute** método no ADO 2.5 e superior são um subconjunto dos recursos fornecidos para o mesmo método no ADO 2.1.  
