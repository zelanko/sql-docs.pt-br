...
ms.topic: "article"
helpviewer_keywords: 
  - "desabilitando a publicação"
  - "publicação [replicação do SQL Server], desabilitando"
  - "desabilitando a distribuição [replicação do SQL Server]"
  - "removendo a replicação "
  - "replicação [replicação do SQL Server], removendo"
  - "desabilitando a replicação"
  - "desabilitando a distribuição "
...
2.  Clique com botão direito do **replicação** pasta e clique **Desabilitar publicação e distribuição**.  
...
2.  Em cada assinante no banco de dados de assinatura, execute [sp_removedbreplication](../../relational-databases/system-stored-procedures/sp-removedbreplication-transact-sql.md) para remover objetos de replicação de banco de dados. Esse procedimento armazenado não removerá trabalhos de replicação no Distribuidor.  
  
3.  No publicador do banco de dados de publicação, execute [sp_removedbreplication](../../relational-databases/system-stored-procedures/sp-removedbreplication-transact-sql.md) para remover objetos de replicação de banco de dados.  
  
4.  Se o publicador usa um distribuidor remoto, execute [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md).  
...
7.  No distribuidor, execute [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md) para remover a designação distribuidor do servidor.  
    ...
    > [!NOTE]  
    >  Se todos os objetos de publicação e distribuição de replicação não são descartados antes de executar [sp_dropdistpublisher](../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md) e [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md), estes procedimentos retornarão um erro. Para descartar objetos todas relacionadas à replicação quando um editor ou distribuidor for descartado, o **@no_checks** parâmetro deve ser definido como **1**. Se um editor ou distribuidor estiver offline ou inacessível, o **@ignore_distributor** parâmetro pode ser definido como **1** para que possam ser descartados; no entanto, qualquer publicação e distribuição de objetos deixados para trás devem ser removidos manualmente.  
...
3.  Criar uma conexão com o distribuidor usando a <xref:Microsoft.SqlServer.Management.Common.ServerConnection> classe.  
  
4.  Criar uma instância do <xref:Microsoft.SqlServer.Replication.DistributionPublisher> classe. Especifique o <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Name%2A> propriedade e passe o <xref:Microsoft.SqlServer.Management.Common.ServerConnection> objeto da etapa 3.  
  
5.  (Opcional) Chamar o <xref:Microsoft.SqlServer.Replication.ReplicationObject.LoadProperties%2A> method para obter as propriedades do objeto e verificar a existência de publicador. Se esse método retornar **false**, o nome do Publicador definido na etapa 4 estava incorreto ou o Publicador não está sendo usado pelo Distribuidor.  
  
6.  Chamar o <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Remove%2A> método. Passe um valor de **true** para *force* caso o Publicador e o Distribuidor estejam em servidores diferentes, e quando o Publicador tiver de ser desinstalado no Distribuidor, sem verificar primeiro se não há publicações no Publicador.  
  
7.  Criar uma instância do <xref:Microsoft.SqlServer.Replication.ReplicationServer> classe. Passar o <xref:Microsoft.SqlServer.Management.Common.ServerConnection> objeto da etapa 3.  
  
8.  Chamar o <xref:Microsoft.SqlServer.Replication.ReplicationServer.UninstallDistributor%2A> método. Passe um valor de **true** para a *force* para remover todos os objetos de replicação no Distribuidor sem verificar primeiro se todos os bancos de dados de publicação locais foram desabilitados e os bancos de dados de distribuição desinstalados.  
