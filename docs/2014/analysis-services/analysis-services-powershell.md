...
## <a name="in-this-article"></a>Neste artigo  
 ...
 [Analysis Services tarefas do PowerShell](#bkmk_tasks)  
...
|Conexões nativas a cubos locais<br /><br /> "Data Source = c:\backup\test.Cub"|Não compatível.|  
|Conexões HTTP a arquivos de conexão do modelo semântico BI (.bism) no SharePoint<br /><br /> "Fonte de dados http://server/shared_docs/name.bism ="|Não compatível.|  
|Conexões inseridas em bancos de dados PowerPivot<br /><br /> "Data Source=$Embedded$"|Não compatível.|  
|Contexto de servidor local em procedimentos armazenados do Analysis Services<br /><br /> "Fonte de dados = *"|Não compatível.|  
...
-   [Carregar o provedor de Analysis Services e os cmdlets](#bkmk_load)  
  
-   [Habilitar a administração remota](#bkmk_remote)  
  
-   [Conectar-se a um objeto Analysis Services](#bkmk_connect)  
  
-   [Administrar o serviço](#bkmk_admin)  
  
-   [Obter ajuda para Analysis Services PowerShell](#bkmk_help)  
...
3.  No servidor remoto, verifique se os seguintes serviços foram iniciados:  Serviço de chamada de procedimento remoto (RPC), serviço auxiliar NetBIOS TCP/IP, serviço Instrumentação de Gerenciamento do Windows (WMI), Serviço Gerenciamento Remoto do Windows (WS-Management).  
...
7.  Clique duas vezes **em Firewall do Windows: Permitir exceção**de compartilhamento de arquivos e impressoras de entrada, selecione **habilitado**e clique em **OK**.  
 ...
 **Conexões nativas para Analysis Services**  
  
 ![Conexão nativa com Analysis Services](media/ssas-powershell-nativeconnection.gif "Conexão nativa com Analysis Services")  
 ...
 **Conexões HTTP para Analysis Services**  
  
 ![Conexão HTTP para Analysis Services](media/ssas-powershell-httpconnection.gif "Conexão HTTP para Analysis Services")  
  
 As conexões HTTP são úteis se você configurou o servidor para acesso HTTP usando as instruções neste tópico: [Configurar o acesso HTTP ao Analysis Services no IIS &#40;(Serviços de Informações da Internet)&#41; 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
  
 Supondo que uma URL http://localhost/olap/msmdpump.dll de servidor do, uma conexão pode ser parecida com a seguinte:  
...
###  <a name="bkmk_help"></a>Obter ajuda para Analysis Services PowerShell  
