 ...
 **SPAdmin** é uma conta de domínio usada para instalar e configurar o farm. É a conta usada para executar o Assistente de configuração do SharePoint e a ferramenta de configuração do PowerPivot para SharePoint 2013 **SPAdmin** é a única conta que exige direitos de administrador local. Antes de executar a ferramenta de configuração do PowerPivot, conceda o **SPAdmin** privilégios para a instância de banco de dados do SQL Server em que o SharePoint cria bancos de dados de conteúdo e configuração de conta. Para configurar a conta SPAdmin em um cenário de privilégios mínimos, ela deve ser um membro das funções **securityadmin** e **dbcreator**.  
