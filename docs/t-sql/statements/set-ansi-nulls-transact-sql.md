...
## <a name="remarks"></a>Remarks  
 Quando SET ANSI_NULLS é definido como ON, uma instrução SELECT que usa WHERE *column_name* = **NULL** retornará zero linha, mesmo que haja valores nulos em *column_name*. Uma instrução SELECT usando WHERE *column_name* <> **NULL** retorna zero linha mesmo que haja valores não nulos em *column_name*.  
