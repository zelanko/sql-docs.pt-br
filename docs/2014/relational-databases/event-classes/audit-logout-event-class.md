...
# <a name="audit-logout-event-class"></a>Classe de evento Audit Logout
  A classe de evento **Audit logout** indica que um usuário fez logoff (desconectado) [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Os eventos nesta classe são disparados por novas conexões ou por conexões reutilizadas de um pool de conexões.  
...
|**Leituras**|**bigint**|Número de E/Ss de leitura lógicas emitidas pelo usuário durante a conexão.|16|Sim|  
...
|**Gravações**|**bigint**|Número de E/Ss de gravação lógicas emitidas pelo usuário durante a conexão.|17|Sim|  
