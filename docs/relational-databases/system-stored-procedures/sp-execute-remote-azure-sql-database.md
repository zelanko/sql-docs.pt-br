...
Crie uma credencial no escopo do banco de dados em um banco de dados de usuário, especificando credenciais de administrador para o banco de dados mestre. Crie uma fonte de dados externa apontando para o banco de dados mestre e especificando a credencial no escopo do banco de dado. Em seguida, o exemplo a seguir, no banco de dados `sp_set_firewall_rule` do usuário, executa o procedimento no banco de dados mestre. O `sp_set_firewall_rule` procedimento requer 3 parâmetros e requer que o `@name` parâmetro seja Unicode.
