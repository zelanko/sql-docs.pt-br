...
-   [Permissões de serviço](#Serv_Perm)  
    ...
    -   [Direitos e privilégios do Windows](#Windows)  
...
-   [Provisionamento](#Provisioning)  
    ...
    -   [Provisionamento de Mecanismo de Banco de Dados](#DE_Prov)  
  
        -   [Entidades de segurança do Windows](#Win_Principals)  
  
        -   [Conta SA](#sa)  
  
        -   [SQL Serverndo logon e privilégios de SID por serviço](#Logins)  
  
        -   [SQL Server Agent logon e privilégios](#Agent)  
  
        -   [Instância e privilégios de cluster de failover HADRON e SQL](#Hadron)  
  
        -   [Gravador e privilégios do SQL](#Writer)  
  
        -   [WMI e privilégios do SQL](#SQLWMI)  
  
    -   [Provisionamento do SSAS](#SSAS)  
  
    -   [Provisionamento do SSRS](#SSRS)  
...
-   [Apêndice](#Appendix)  
    ...
    -   [Identificar serviços com reconhecimento de instância e sem reconhecimento de instância](#Identify_instance_aware_and_unaware)  
  
    -   [Nomes de serviço localizados](#Localized_service_names)  
...
-   **[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]**-Fornece suporte de gerenciamento [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] para o armazenamento e a execução de pacotes. O caminho do executável \<é MSSQLPATH> \120\dts\binn\msdtssrvr.exe  
  
-   Navegador-o serviço de resolução de nomes [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que fornece informações de conexão para computadores cliente. ** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ** O caminho do executável é c:\Arquivos de Programas (x86)\Microsoft SQL Server\90\Shared\sqlbrowser.exe  
  
-   **Pesquisa de texto completo** – cria rapidamente índices de texto completo em conteúdo e propriedades de dados estruturados e semiestruturados para fornecer filtragem de documentos e separação de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]palavras para o.  
  
-   **Gravador do SQL** – permite que aplicativos de backup e restauração operem na estrutura do serviço de cópias de sombra de volume (VSS).  
  
-   Controlador de Distributed Replay-fornece orquestração de reprodução de rastreamento em vários computadores cliente Distributed Replay. ** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **  
  
-   Distributed Replay cliente – um ou mais computadores cliente Distributed Replay que trabalham junto com um controlador de Distributed Replay para simular cargas de trabalho simultâneas em uma [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]instância do. ** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **  
...
-   [Configurando a inicialização do serviço](#Configure_services)  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Controlador de Distributed Replay|[SERVIÇO DE REDE](#Network_Service)|[Conta virtual](#VA_Desc)<sup>*</sup>|  
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Distributed Replay cliente|[SERVIÇO DE REDE](#Network_Service)|[Conta virtual](#VA_Desc)<sup>*</sup>|  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Gravador VSS|[SISTEMA LOCAL](#Local_System)|[SISTEMA LOCAL](#Local_System)|  
 ...
 <sup>*</sup>Quando são necessários recursos externos [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ao computador, [!INCLUDE[msCoName](../../includes/msconame-md.md)] o recomenda usar uma MSA (conta de serviço gerenciado), configurada com os privilégios mínimos necessários.  
  
 **SQL Server instância de cluster de failover**  
...
####  <a name="Changing_Accounts"></a>Alterando propriedades da conta  
...
-   <a name="MSA"></a> **Contas de serviço gerenciado**  
  
     Uma MSA é um tipo de conta de domínio criada e gerenciada pelo controlador de domínio. Ela é atribuída a um único computador membro para a execução de um serviço. A senha é gerenciada automaticamente pelo controlador de domínio. Não é possível usar uma MSA para fazer logon em um computador, mas um computador pode usar uma MSA para iniciar um serviço do Windows. Uma MSA tem a capacidade de registrar SPN com o Active Directory. Um MSA é nomeado com um **$** sufixo, por exemplo, **DOMAIN\ACCOUNTNAME $**. Ao especificar uma MSA, deixe a senha em branco. Como um MSA é atribuído a um único computador, não pode ser usado em diferentes nós de um cluster do windows.  
...
-   <a name="VA_Desc"></a>**Contas virtuais**  
    ...
    |A instância nomeada de um serviço [!INCLUDE[ssDE](../../includes/ssde-md.md)] nomeado **PAYROLL**|**FOLHA DE PAGAMENTO DO NT SERVICE\MSSQL $**|  
    |
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent na instância padrão de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]|**NT SERVICE\SQLSERVERAGENT**|  
    |[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Serviço do Agent em uma instância [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] do nome da **folha de pagamento**|**FOLHA DE PAGAMENTO DO NT SERVICE\SQLAGENT $**|  
...
-   **Desabilitado** O serviço está instalado, mas não está em execução no momento.  
  
-   **Manual** O serviço está instalado, mas só será iniciado quando outro serviço ou aplicativo precisar de sua funcionalidade.  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Controlador de Distributed Replay|DRU_CTLR, CTLRSVCACCOUNT, CTLRSVCPASSWORD, CTLRSTARTUPTYPE, CTLRUSERS|  
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Distributed Replay cliente|DRU_CLT, CLTSVCACCOUNT, CLTSVCPASSWORD, CLTSTARTUPTYPE, CLTCTLRNAME, CLTWORKINGDIR, CLTRESULTDIR|  
...
-   [Permissões do sistema de arquivos concedidas a SQL Server SIDs por serviço ou SQL Server grupos locais do Windows](#Reviewing_ACLs)  
  
-   [Permissões do sistema de arquivos concedidas a outras contas de usuário ou grupos do Windows](#File_System_Other)  
...
###  <a name="Windows"></a>Direitos e privilégios do Windows  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Serviço|Permissões concedidas pela Instalação do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]|  
|---------------------------------------|------------------------------------------------------------|  
|**[!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]:**<br /><br /> (Todos os direitos são concedidos ao SID por serviço. Instância padrão: **NT SERVICE\MSSQLSERVER**. Instância nomeada: **NT SERVICE\MSSQL$** InstanceName.)|**Fazer logon como serviço** (SeServiceLogonRight)<br /><br /> **Substituir um token de nível de processo** (SeAssignPrimaryTokenPrivilege)<br /><br /> **Ignorar verificação de desvio** (SeChangeNotifyPrivilege)<br /><br /> **Ajustar cotas de memória para um processo** (SeIncreaseQuotaPrivilege)<br /><br /> Permissão para iniciar o Gravador do SQL<br /><br /> Permissão para ler o serviço Log de Eventos<br /><br /> Permissão para ler o serviço Chamada de Procedimento Remoto|  
|Agente: <sup>1</sup> ** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **<br /><br /> (Todos os direitos são concedidos ao SID por serviço. Instância padrão: **NT Service\SQLSERVERAGENT**. Instância nomeada: **NT Service\SQLAGENT$**_InstanceName_.)|**Fazer logon como serviço** (SeServiceLogonRight)<br /><br /> **Substituir um token de nível de processo** (SeAssignPrimaryTokenPrivilege)<br /><br /> **Ignorar verificação de desvio** (SeChangeNotifyPrivilege)<br /><br /> **Ajustar cotas de memória para um processo** (SeIncreaseQuotaPrivilege)|  
|**[!INCLUDE[ssAS](../../includes/ssas-md.md)]:**<br /><br /> (Todos os direitos são concedidos a um grupo local do Windows. Instância padrão: **SQLServerMSASUser$**_ComputerName_**$MSSQLSERVER**. Instância nomeada: **SQLServerMSASUser $**_ComputerName_**$**_InstanceName_. [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)]instância: **SQLServerMSASUser $**_ComputerName_**$**_PowerPivot_.)|**Fazer logon como serviço** (SeServiceLogonRight)<br /><br /> Somente tabular:<br /><br /> **Aumentar um conjunto de trabalho de processo** (SeIncreaseWorkingSetPrivilege)<br /><br /> **Ajustar cotas de memória para um processo** (SeIncreaseQuotaSizePrivilege)<br /><br /> **Bloquear páginas na memória** (SeLockMemoryPrivilege) – isso é necessário somente quando a paginação é desativada inteiramente.<br /><br /> Somente para instalações de cluster de failover:<br /><br /> **Aumentar prioridade de agendamento** (SeIncreaseBasePriorityPrivilege)|  
|**[!INCLUDE[ssRS](../../includes/ssrs.md)]:**<br /><br /> (Todos os direitos são concedidos ao SID por serviço. Instância padrão: **NT SERVICE\ReportServer**. Instância nomeada: **serviço\\NT**_InstanceName_.)|**Fazer logon como serviço** (SeServiceLogonRight)|  
|**[!INCLUDE[ssIS](../../includes/ssis-md.md)]:**<br /><br /> (Todos os direitos são concedidos ao SID por serviço. Instância padrão e instância nomeada: **NT SERVICE\MsDtsServer120**. O[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] não tem um processo separado para uma instância nomeada).|**Fazer logon como serviço** (SeServiceLogonRight)<br /><br /> Permissão para gravar no log de eventos do aplicativo.<br /><br /> **Ignorar verificação de desvio** (SeChangeNotifyPrivilege)<br /><br /> **Representar um cliente após a autenticação** (SeImpersonatePrivilege)|  
|**Pesquisa de texto completo:**<br /><br /> (Todos os direitos são concedidos ao SID por serviço. Instância padrão: **NT Service\MSSQLFDLauncher**. Instância nomeada: **NT Service\ MSSQLFDLauncher$**_InstanceName_.)|**Fazer logon como serviço** (SeServiceLogonRight)<br /><br /> **Ajustar cotas de memória para um processo** (SeIncreaseQuotaPrivilege)<br /><br /> **Ignorar verificação de desvio** (SeChangeNotifyPrivilege)|  
|**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Navegador:**<br /><br /> (Todos os direitos são concedidos a um grupo local do Windows. Instância padrão ou nomeada: **SQLServer2005SQLBrowserUser**_$ComputerName_. O Navegador do[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] não tem um processo separado para uma instância nomeada.)|**Fazer logon como serviço** (SeServiceLogonRight)|  
|**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Gravador VSS:**<br /><br /> (Todos os direitos são concedidos ao SID por serviço. Instância padrão ou nomeada: **NT Service\SQLWriter**. 
...
|**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Controlador de Distributed Replay:**|**Fazer logon como serviço** (SeServiceLogonRight)|  
|**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Distributed Replay cliente:**|**Fazer logon como serviço** (SeServiceLogonRight)|  
...
###  <a name="Reviewing_ACLs"></a>Permissões do sistema de arquivos concedidas a SQL Server SIDs por serviço ou grupos locais do Windows  
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Controlador de Distributed Replay|
  \<ToolsDir>\DReplayController\Log\ (diretório vazio)|Leitura, Execução, Listar Conteúdo de Pastas|  
||
...
|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Distributed Replay cliente|
  \<ToolsDir>\DReplayClient\Log\| Leitura, Execução, Listar Conteúdo de Pastas|  
||
...
||
 ...
 <sup>1</sup> O [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] serviço Agent é desabilitado em instâncias [!INCLUDE[ssExpress](../../includes/ssexpress-md.md)] do [!INCLUDE[ssExpress](../../includes/ssexpress-md.md)] e do com Advanced Services.  
...
###  <a name="File_System_Other"></a>Permissões do sistema de arquivos concedidas a outras contas de usuário ou grupos do Windows  
...
|[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]|Conta de serviço do Windows do servidor de relatório|instalar>\Reporting Services\LogFiles * \< *|Delete (excluir)<br /><br /> READ_CONTROL<br /><br /> SYNCHRONIZE<br /><br /> FILE_GENERIC_READ<br /><br /> FILE_GENERIC_WRITE<br /><br /> FILE_READ_DATA<br /><br /> FILE_WRITE_DATA<br /><br /> FILE_APPEND_DATA<br /><br /> FILE_READ_EA<br /><br /> FILE_WRITE_EA<br /><br /> FILE_READ_ATTRIBUTES<br /><br /> FILE_WRITE_ATTRIBUTES|  
...
> [!NOTE]  
>  Contas virtuais não podem ser autenticadas em um local remoto. Todas as contas virtuais usam a permissão da conta de máquina. Provisione a conta do computador no formato _<domain_name>_ **\\** _<computer_name _ **$** de re>.  
...
-   Permissão para **criar notificação de evento DDL** no servidor.  
  
-   Permissão para **criar notificação de evento** de [!INCLUDE[ssDE](../../includes/ssde-md.md)]rastreamento no.  
  
-   **EXIBA qualquer** permissão no nível de servidor do banco de dados.  
...
###  <a name="DE_Prov"></a>Provisionamento de Mecanismo de Banco de Dados  
...
####  <a name="Logins"></a>SQL Serverndo logon e privilégios de SID por serviço  
...
####  <a name="Agent"></a>SQL Server Agent logon e privilégios  
...
####  <a name="SQLWMI"></a>WMI e privilégios do SQL  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]A instalação provisiona a conta **NT SERVICE\Winmgmt** como [!INCLUDE[ssDE](../../includes/ssde-md.md)] um logon e a adiciona à função de servidor fixa **sysadmin** .  
...
###  <a name="SSAS"></a>Provisionamento do SSAS  
...
###  <a name="SSRS"></a>Provisionamento do SSRS  
...
##  <a name="Upgrade"></a>Atualizando de versões anteriores  
...
>  Se você configurar o aplicativo para usar uma conta de domínio, poderá isolar os privilégios para o aplicativo, mas deverá gerenciar senhas manualmente ou criar uma solução personalizada para gerenciar essas senhas. Muitos aplicativos para servidores usam essa estratégia para aprimorar a segurança, mas essa estratégia requer administração adicional e complexidade. Nessas implantações, os administradores de serviço gastam um tempo considerável em tarefas de manutenção, como o gerenciamento de senhas de serviço e SPNs, que são necessários para a autenticação Kerberos. Além disso, essas tarefas de manutenção podem interromper serviço.  
 ...
 A conta de Serviço de Rede é uma conta interna que tem mais acesso a recursos e objetos do que os membros do grupo Usuários. Os serviços que são executados como a conta de serviço de rede acessam recursos de rede usando as credenciais da conta de computador no formato _<domain_name>_ **\\** _<computer_name _ **$**. O nome real da conta é **NT AUTHORITY\NETWORK SERVICE**.  
  
 <a name="Local_System"></a>**Conta do sistema local**  
...
###  <a name="Identify_instance_aware_and_unaware"></a>Identificar serviços com reconhecimento de instância e sem reconhecimento de instância  
...
###  <a name="Localized_service_names"></a>Nomes de serviço localizados  
