...
|Exemplo|Descrição|  
|-------------|-----------------|  
|`=Parameters!` *\<ParameterName>* `.IsMultiValue`|Retorna `False`.<br /><br /> Verifica se um parâmetro é de diversos valores. Se `True`, o parâmetro é de diversos valores e é uma coleção de objetos. Se `False`, o parâmetro é de valor único e é um único objeto.|  
