...
|sync_type|**tinyint**|O tipo de sincronização:<br /><br /> **1** = automatic.<br /><br /> **2** não = nenhuma sincronização.|  
...
|attempted_validate|**datetime**|A última **datetime** tentativa de validação da assinatura.|  
...
|metadatacleanuptime|**datetime**|A última **datetime** que metadados expirados foram removidos de tabelas do sistema de replicação de mesclagem.|  
...
|last_makegeneration_datetime|**datetime**|A última **datetime** que o processo de makegeneration executou para o publicador. Para obter mais informações, consulte o parâmetro - MakeGenerationInterval [Replication Merge Agent](../../relational-databases/replication/agents/replication-merge-agent.md).|  
