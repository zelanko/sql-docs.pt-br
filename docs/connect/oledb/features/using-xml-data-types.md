...
description: Usando tipos de dados XML com OLE DB driver para SQL Server
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
 ...
 **ISequentialStream** deve ser usado para recuperação de documentos XML extensos. As mesmas técnicas usadas para outros tipos de valor extenso também se aplicam ao XML. Para obter mais informações, consulte [usando tipos de valor grande](../../oledb/features/using-large-value-types.md).  
...
>  Nenhum dado é retornado para valores de XML NULL.  
 ...
 A validação ocorre quando os dados são enviados para o servidor. As alterações de validação e codificação do lado do cliente devem ser tratadas pelo seu aplicativo. É recomendável que você não processe os dados XML diretamente, mas, em vez disso, deve usar um leitor DOM ou SAX para processá-los.  
