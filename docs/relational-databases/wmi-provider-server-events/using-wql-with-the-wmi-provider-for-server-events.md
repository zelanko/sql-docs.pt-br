 ...
 *event_property*  
 É uma propriedade de um evento. Os exemplos incluem **PostTime**, **SPID**, e **LoginName**. Procure cada evento listado na [provedor WMI para Classes de eventos do servidor e as propriedades](../../relational-databases/wmi-provider-server-events/wmi-provider-for-server-events-classes-and-properties.md) para determinar as propriedades que ele contém. Por exemplo, o evento DDL_DATABASE_LEVEL_EVENTS contém as **DatabaseName** e **nome de usuário** propriedades. Ele também herda as **SQLInstance**, **LoginName**, **PostTime**, **SPID**, e **ComputerName** Propriedades de seus eventos pai.  
...
```  
 ...
 Notificações de eventos no escopo do servidor são criadas na **mestre** do banco de dados e pode ser consultada por metadados, usando o [server_event_notifications](../../relational-databases/system-catalog-views/sys-server-event-notifications-transact-sql.md) exibição do catálogo.  
  
 No escopo do banco de dados ou notificações de eventos no escopo do objeto são criadas no banco de dados especificado e podem ser consultadas por metadados, usando o [event_notifications](../../relational-databases/system-catalog-views/sys-event-notifications-transact-sql.md) exibição do catálogo. (A exibição do catálogo deve ser prefixada com o nome do banco de dados correspondente.)  
