---
title: "Trabalhar com dados de altera&#231;&#245;es (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/03/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "dados de alterações [SQL Server]"
  - "change data capture [SQL Server], cenários de função de consulta"
  - "change data capture [SQL Server], limites de LSN"
  - "change data capture [SQL Server], funções de consulta"
...
-   A função [cdc.fn_cdc_get_all_changes_<capture_instance>](../Topic/cdc.fn_cdc_get_all_changes_%3Ccapture_instance%3E%20%20\(Transact-SQL\).md) retorna todas as alterações ocorridas para o intervalo especificado. Essa função sempre é gerada. As entradas sempre são retornadas em ordem, a primeira pelo LSN de confirmação da transação da alteração e, em seguida, por um valor que ordena a alteração dentro de sua transação. Dependendo da opção de filtro de linhas escolhida, ou a linha final é retornada na atualização (opção de filtro de linhas "all") ou os valores novos e antigos são retornados na atualização (opção de filtro de linhas "all update old").  
  
-   A função [cdc.fn_cdc_get_net_changes_<capture_instance>](../Topic/cdc.fn_cdc_get_net_changes_%3Ccapture_instance%3E%20\(Transact-SQL\).md) é gerada quando o parâmetro @supports_net_changes é definido como 1 e a tabela de origem está habilitada.  
...
### Consultando todas as novas alterações feitas até agora  
...
### Adicionando uma hora de confirmação a um conjunto de resultados Todas as Alterações  
...
### Unindo dados de alteração com outros dados da mesma transação  
...
### Consultando com funções de wrapper de data e hora  
 ...
 As funções de wrapper geradas dão suporte à seguinte convenção para percorrer sistematicamente a linha do tempo do Change Data Capture: espera-se que o parâmetro @end_time do intervalo anterior seja usado como o parâmetro @start_time do intervalo subsequente. A função de wrapper mapeia os valores de data e hora para valores LSN e assegura que nenhum dado seja perdido ou repetido se esta convenção for seguida.  
...
### Usando as funções de wrapper de data e hora para transição entre instâncias de captura  
