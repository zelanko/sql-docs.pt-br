---
title: (Ferramentas de análise tabela para Excel) de análise de cesta de compras | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="shopping-basket-analysis-table-analysistools-for-excel"></a>Análise da Cesta de Compras (Ferramentas de Análise de Tabela para Excel)
 ...
 O **análise da cesta de compras** ferramenta ajuda você a encontrar `associations` em seus dados. Uma associação pode informar quais itens são frequentemente adquiridos ao mesmo tempo. Na mineração de dados, essa técnica é um método conhecido como *análise da cesta*, usado para analisar o comportamento de compra dos clientes em grandes conjuntos de dados. Os analistas de marketing podem usar as informações para recomendar produtos relacionados aos clientes e promover produtos relacionados substituindo-os em páginas da Web próximas, em catálogos ou no comércio.  
 ...
 O **regras da cesta de compras** planilha usa as estatísticas derivadas da análise para criar regras sobre como os itens estão relacionados. Por exemplo, uma regra pode ser que se os clientes compram o produto A, é bem prováveis que adquirir o produto B. As regras podem ser usadas para criar recomendações. Cada regra tem estatísticas de suporte que ajudam a avaliar sua força potencial, para que você só possa fazer uma recomendação se a regra exceder um determinado limite de probabilidade.  
...
3.  No **análise da cesta de compras** caixa de diálogo, escolha a coluna que contém a ID de transação e, em seguida, escolha a coluna que contém os itens ou produtos que você deseja analisar.  
  
4.  Se desejar, adicione uma coluna que contenha valores de produtos.  
  
5.  Clique em**avançado**para abrir o **configuração avançada de parâmetros** caixa de diálogo. Aumente o valor para **suporte mínimo** para reduzir o número de produtos que são agrupados como conjuntos de itens. Aumentar o **regra de probabilidade mínima** filtrar conjuntos de itens muito comuns.  
  
### <a name="requirements"></a>Requisitos  
 Para usar o **análise da cesta de compras** ferramenta, os dados devem ser armazenados em uma tabela do Excel e deve conter as seguintes colunas:  
...
## <a name="understanding-the-reports"></a>Entendendo os relatórios  
 O **análise da cesta de compras** ferramenta cria dois relatórios complementares. O primeiro relatório, denominado **grupos significativo de itens identificados durante a análise**, fornece uma lista de todos os conjuntos de itens que foram encontradas. Use as novas ferramentas de tabela no Microsoft Excel para classificar, filtrar e explorar os dados.  
  
 O segundo relatório, denominado **regras da cesta de compras**, informa que tipo de inferências pode ser feito com base em conjuntos de itens listados no primeiro relatório. Enquanto a lista de conjuntos de itens é mais útil para explorar e entender os dados, a lista de regras é mais útil para fazer previsões e recomendações.  
  
### <a name="shopping-basket-item-groups-report"></a>Relatório de grupos de itens da cesta de compras  
 Este relatório contém uma lista de todas as combinações de itens possíveis encontradas no conjunto de dados. Por exemplo, se os dados da transação contém pedidos para cada pedido de **análise da cesta de compras** ferramenta calcula quantas vezes o item individual foi pedido e, em seguida, calcula todas as combinações desse item com outros itens.  
...
|Coluna no relatório|O que informa|  
|----------------------|-----------------------|  
|Itens Existentes|Lista os itens necessários para fazer uma recomendação.<br /><br /> Na mineração de dados, esses itens são considerados no *lado esquerdo* da regra de associação.|  
|Item Previsto|Lista o item a ser recomendado.<br /><br /> Na mineração de dados, esses itens são considerados no *direita* da regra de associação.|  
