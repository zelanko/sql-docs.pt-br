...
|----------------|-----------------|  
|**o padrão é**|O atributo selecionado **segue o padrão** de um atributo específico, um valor de atributo específico ou está em branco.<br /><br /> Esta ação é válida para valores de texto, número, data e link.|  
|**usa como padrão um valor gerado**|O atributo selecionado **segue o padrão de um valor gerado** que é determinado pela inserção de um valor inicial e incremental.<br /><br /> Esta ação é válida para valores de texto e número.|  
|**usa como padrão um valor concatenado**|O atributo selecionado **segue o padrão de um valor concatenado** que é determinado pela especificação de vários atributos.<br /><br /> Esta ação é válida para valores de texto e link.|  
  
## <a name="change-value-actions"></a>Ações de alteração de valor  
 As ações de **alteração de valor** atualizam o valor de um atributo ou valor de atributo especificado. Os usuários somente poderão alterar esses valores se o novo valor fizer com que a ação seja verdadeira.  
...
|**é igual a um valor concatenado**|O atributo selecionado é alterado para um valor concatenado, que é determinado pela especificação de vários atributos.<br /><br /> Esta ação é válida para valores de texto e link.|  
  
## <a name="validation-actions"></a>Ações de validação  
 Ações de **validação** , quando não são avaliadas como true, enviam email para um usuário ou grupo especificado. Para confirmar uma versão, todas as ações de validação devem ser avaliadas como verdadeiras.  
...
|**deve ser exclusivo**|O atributo selecionado **deve ser exclusivo** , independentemente ou em combinação com os atributos definidos.<br /><br /> **Prática recomendada:** Combine essa ação com uma condição obrigatória para garantir a validade de campos de índice em sistemas de assinatura.<br /><br /> Esta ação é válida para valores de texto, número, data e link.<br /><br /> **Observação**: se o primeiro atributo for do tipo DateTime, ele não poderá ser usado em combinação com um atributo de tipo numérico ou atributo de texto. Se o primeiro atributo for do tipo Numeric, ele não poderá ser usado em combinação com um atributo do tipo DateTime.|  
|**deve ter um dos valores a seguir**|O atributo selecionado **deve ter um dos valores** especificados em uma lista.<br /><br /> Esta ação é válida para valores de texto.|  
...
## <a name="external-action"></a>Ação externa  
 Ações **externas** interagem com aplicativos fora [!INCLUDE[ssMDSshort](../includes/ssmdsshort-md.md)]do.  
