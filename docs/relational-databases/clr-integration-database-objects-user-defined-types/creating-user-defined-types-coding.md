...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
 ...
 O exemplo nesta seção ilustra a implementação de um **ponto** UDT como um **struct** (ou **estrutura** no Visual Basic). O **ponto** UDT consiste em X e coordenadas Y implementadas como procedimentos de propriedade.  
...
```  
 ...
 O **SQLServer** namespace contém os objetos necessários vários atributos da UDT e o **SqlTypes** namespace contém classes que representam [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]tipos de dados nativos disponíveis para o assembly. É claro que talvez haja namespaces adicionais necessários ao funcionamento correto do assembly. O **ponto** UDT também usa o **System. Text** namespace para trabalhar com cadeias de caracteres.  
...
## <a name="specifying-attributes"></a>Especificando atributos  
 ...
 O **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute** é necessária. O **Serializable** atributo é opcional. Você também pode especificar o **Microsoft.SqlServer.Server.SqlFacetAttribute** para fornecer informações sobre o tipo de retorno de um UDT. Para obter mais informações, confira [Atributos personalizados para rotinas do CLR (Common Language Runtime)](../../relational-databases/clr-integration/database-objects/clr-integration-custom-attributes-for-clr-routines.md).  
  
### <a name="point-udt-attributes"></a>Atributos da UDT Point  
 O **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute** define o formato de armazenamento para o **ponto** UDT **nativo**. **IsByteOrdered** é definido como **verdadeiro**, que garante que os resultados das comparações sejam os mesmos no SQL Server, como se a mesma comparação foi feita no código gerenciado. A UDT implementa a **System.Data.SqlTypes.INullable** interface para que o nulo UDT reconheça.  
...
```  
 ...
 Ambas as consultas retornam as IDs de pontos com não -**nulo** locais. Na Consulta 1, é usada a manipulação de nulos normal, não havendo nenhuma desserialização das UDTs obrigatória. Consulta 2, por outro lado, tem desserializar cada não -**nulo** do objeto e chamar o CLR para obter o valor da **IsNull** propriedade. Claramente, usar **é nulo** exibirão um melhor desempenho e nunca deverá haver um motivo para ler o **IsNull** propriedade de um UDT no [!INCLUDE[tsql](../../includes/tsql-md.md)] código.  
  
 Então, qual é o uso do **IsNull** propriedade? Primeiro, é necessário para determinar se um valor é **nulo** dentro do código CLR. Segundo, o servidor precisa de uma maneira de testar se uma instância é **nulo**, portanto, essa propriedade é usada pelo servidor. Depois de determinar está **nulo**, em seguida, ele pode usar a manipulação de nulos nativa para tratá-la.  
  
## <a name="implementing-the-parse-method"></a>Implementando o método Parse  
 O **analisar** e **ToString** métodos permitem conversões de e para representações de cadeia de caracteres do UDT. O **analisar** método permite que uma cadeia de caracteres a ser convertido em um UDT. Ele deve ser declarado como **estáticos** (ou **compartilhado** no Visual Basic) e usar um parâmetro de tipo **System.Data.SqlTypes.SqlString**.  
  
 O código a seguir implementa o **analisar** método para o **ponto** UDT, que separa as coordenadas X e Y. O **analisar** método tem um único argumento do tipo **System.Data.SqlTypes.SqlString**e pressupõe que os valores de X e Y são fornecidos como uma cadeia de caracteres delimitada por vírgulas. Definindo o **Microsoft.SqlServer.Server.SqlMethodAttribute.OnNullCall** atributo **falso** impede que o **analisar** método seja chamado a partir de uma instância nula de Ponto.  
...
## <a name="implementing-the-tostring-method"></a>Implementando o método ToString  
 O **ToString** método converte os **ponto** UDT como um valor de cadeia de caracteres. Nesse caso, a cadeia de caracteres "NULL" é retornada para uma instância nula do **ponto** tipo. O **ToString** método inverte a **analisar** método usando um **StringBuilder** para retornar uma delimitada por vírgula **System. String**consiste em valores de coordenada X e Y. Porque **InvokeIfReceiverIsNull** padrão é false, a verificação de uma instância nula de **ponto** é desnecessária.  
...
## <a name="validating-udt-values"></a>Validando valores de UDT  
 ...
 O **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute.ValidationMethodName** propriedade da **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute** permite que você forneça o nome de um método de validação que o servidor é executada quando os dados são atribuídos a uma UDT ou convertidos em um UDT. **ValidationMethodName** também é chamado durante a execução do utilitário bcp, BULK INSERT, DBCC CHECKDB, DBCC CHECKFILEGROUP, DBCC CHECKTABLE, consulta distribuída e operações de RPC (chamada) de procedimento remoto de protocolo TDS de dados tabulares. O valor padrão para **ValidationMethodName** é nulo, indicando que não há nenhum método de validação.  
  
### <a name="example"></a>Exemplo  
 O fragmento de código a seguir mostra a declaração para o **ponto** classe, que especifica um **ValidationMethodName** dos **ValidatePoint**.  
...
```  
 ...
 O método de validação pode ter qualquer escopo e deve retornar **verdadeira** se o valor for válido, e **falso** caso contrário. Se o método retornar **falsos** ou gera uma exceção, o valor é tratado como não é válido e um erro é gerado.  
...
### <a name="validation-method-limitations"></a>Limitações do método de validação  
 ...
 Você deve chamar explicitamente o método de validação nos setters de propriedade e o **analisar** método se desejar que o método de validação para ser executado em todas as situações. Não se trata de um requisito e, em alguns casos, talvez não seja sequer desejável.  
  
### <a name="parse-validation-example"></a>Exemplo de validação de Parse  
 Para garantir que o **ValidatePoint** método é invocado na **ponto** classe, você deve chamá-lo no **analisar** método e para os procedimentos de propriedade que defina X e Y valores de coordenadas. O fragmento de código a seguir mostra como chamar o **ValidatePoint** método de validação das **analisar** função.  
...
>  O **SqlMethodAttribute** herda o **SqlFunctionAttribute** classe, isso **SqlMethodAttribute** herda o **FillRowMethodName** e **TableDefinition** campos da **SqlFunctionAttribute**. Isso indica que é possível escrever um método com valor de tabela, o que não é o caso. O método compila e o assembly implanta, mas um erro sobre o **IEnumerable** retornar o tipo é gerado em tempo de execução com a seguinte mensagem: "método, propriedade ou campo '\<nome >' em classe\<classe >' no assembly '\<assembly >' tem o tipo de retorno inválido. "  
 ...
 IsDeterministic  
 Indica se a função produz os mesmos valores de saída, dados os mesmos valores de entrada e o mesmo estado de banco de dados. O padrão é **falsos**.  
  
 IsMutator  
 Indica se o método causa uma alteração de estado na instância da UDT. O padrão é **falsos**.  
  
 IsPrecise  
 Indica se a função envolve computações imprecisas como, por exemplo, operações de ponto flutuante. O padrão é **falsos**.  
  
 OnNullCall  
 Indica se o método é chamado quando são especificados argumentos de entrada de referência nulos. O padrão é **verdadeira**.  
...
>  Os métodos Mutator não são permitidos em consultas. Eles só podem ser chamados em instruções de atribuição ou de modificação de dados. Se um método marcado como modificador não retorne **void** (ou não é um **Sub** no Visual Basic), CREATE TYPE falhará com um erro.  
 ...
 A instrução a seguir pressupõe a existência de um **triângulos** UDT que tem um **girar** método. O seguinte [!INCLUDE[tsql](../../includes/tsql-md.md)] instrução update invoca o **girar** método:  
  
```  
UPDATE Triangles SET t.RotateY(0.6) WHERE id=5  
```  
  
 O **girar** método é decorado com o **SqlMethod** configuração de atributo **IsMutator** para **true** , de modo que [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] pode marcar o método como um método modificador. O código também define **OnNullCall** à **falso**, que indica para o servidor que o método retorna uma referência nula (**nada** no Visual Basic) se qualquer uma de entrada parâmetros são referências nulas.  
...
## <a name="implementing-a-udt-with-a-user-defined-format"></a>Implementando uma UDT com um formato definido pelo usuário  
 Ao implementar uma UDT com um formato definido pelo usuário, você deve implementar **leitura** e **gravar** métodos que implementam a interface IBinarySerialize para tratar a serialização e durante a desserialização de dados de UDT. Você também deve especificar o **MaxByteSize** propriedade da **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute**.  
...
### <a name="the-currency-udt"></a>A UDT Currency  
 ...
 O **moeda** UDT dá suporte à manipulação de quantias em dinheiro no sistema monetário de uma determinada cultura. Você deve definir dois campos: uma **cadeia de caracteres** para **CultureInfo**, que especifica quem emitiu a moeda (en-us, por exemplo) e uma **decimal** para  **CurrencyValue**, a quantidade de dinheiro.  
  
 Embora não seja usada pelo servidor para realizar comparações, o **moeda** UDT implementa a **System. IComparable** interface, que expõe um único método,  **System.IComparable.CompareTo**. Isso é usado do lado de cliente, em situações nas quais é desejável comparar com precisão ou ordenar valores de moeda dentro de culturas.  
...
1.  Defina a **IsByteOrdered** do atributo como true, que informa ao [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] para usar a representação binária mantida em disco para comparações.  
...
## <a name="creating-read-and-write-methods-with-ibinaryserialize"></a>Criando métodos Read e Write com IBinarySerialize  
 Quando você escolhe **UserDefined** formato de serialização, você também deve implementar a **IBinarySerialize** interface e criar seus próprios **leitura** e **gravar**  métodos. Os procedimentos a seguir da **moeda** uso UDT o **System.IO.BinaryReader** e **System.IO.BinaryWriter** para ler e gravar na UDT.  
