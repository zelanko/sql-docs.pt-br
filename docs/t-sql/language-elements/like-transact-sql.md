...
### <a name="f-using-not-like-with-the--wildcard-character"></a>F. Usando NOT LIKE com o caractere curinga %  
 O exemplo a seguir localiza todos os números de telefone no `DimEmployee` tabela que não começam com `612`.  para obter informações sobre a ferramenta de configuração e recursos adicionais.  
