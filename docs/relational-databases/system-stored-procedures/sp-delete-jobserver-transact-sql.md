...
## <a name="arguments"></a>Argumentos  
 [ **@job_id=** ] *job_id*  
 O número de identificação do trabalho no qual o servidor de destino especificado será removido. *job_id* está **uniqueidentifier**, com um padrão NULL.  
  
 [ **@job_name=** ] **'***job_name***'**  
 O nome do trabalho no qual o servidor de destino especificado será removido. *job_name* está **sysname**, com um padrão NULL.  
  
> [!NOTE]  
>  Qualquer um dos *job_id* ou *job_name* deve ser especificado; não podem ser especificados.  
  
 [  **@server_name=** ] **'***server***'**  
 O nome do servidor de destino a ser removido do trabalho especificado. *servidor* está **nvarchar (30)**, sem padrão. *servidor* pode ser **(LOCAL)** ou o nome de um servidor de destino remoto.  
