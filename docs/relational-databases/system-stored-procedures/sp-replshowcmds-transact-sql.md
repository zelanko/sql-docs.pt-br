...
`[ @maxtrans = ] maxtrans` É o número de transações sobre o qual retornar informações. *maxtrans* está **int**, com um padrão de **1**, que especifica o número máximo de transações pendentes de replicação para o qual **sp_replshowcmds** Retorna informações.  
