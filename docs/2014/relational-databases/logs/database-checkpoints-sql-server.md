...
##  <a name="Overview"></a> Visão geral de pontos de verificação  
...
> [!NOTE]  
>  O `-k` [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] opção de configuração avançada permite que um administrador de banco de dados com base na taxa de transferência do subsistema de e/s de alguns tipos de pontos de verificação de comportamento de e/s do ponto de verificação de limitação. O `-k` opção de configuração se aplica a pontos de verificação automáticos e a qualquer ponto de verificação manual e interna.  
...
###  <a name="InteractionBwnSettings"></a> Interação de TARGET_RECOVERY_TIME e opções 'recovery interval'  
 A tabela a seguir resume a interação entre o servidor- **sp_configure'`recovery interval`'** configuração e específicas do banco de dados ALTER DATABASE... TARGET_RECOVERY_TIME.  
...
####  <a name="PerformanceImpact"></a> Impacto do intervalo de recuperação no desempenho de recuperação  
 Para um sistema (OLTP) usando transações curtas, de processamento de transações online `recovery interval` é o principal fator para determinar o tempo de recuperação. No entanto, o `recovery interval` opção não afeta o tempo necessário para desfazer uma transação de longa execução. Recuperação de um banco de dados com uma transação de longa duração pode levar mais tempo do que o especificado no `recovery interval` opção. Por exemplo, se uma transação demorada levou duas horas para executar atualizações antes que a instância do servidor fosse desabilitada, a recuperação real levará muito mais o `recovery interval` valor para recuperar a transação demorada. Para obter mais informações sobre o impacto de uma transação de longa duração em um tempo de recuperação, veja [O log de transações &#40;SQL Server&#41;](the-transaction-log-sql-server.md).  
...
-   Se você notar que os pontos de verificação frequentes estão prejudicando o desempenho em um banco de dados.  
 ...
 Se você decidir aumentar a configuração `recovery interval`, recomendamos fazer isso gradativamente em pequenos incrementos e avaliar o efeito de cada aumento incremental no desempenho de recuperação. Esse método é importante porque como o `recovery interval` configuração aumenta, recuperação de banco de dados demora muitas vezes para concluir. Por exemplo, se você alterar `recovery interval` 10, a recuperação levará aproximadamente 10 vezes mais para ser concluído quando `recovery interval` está definido como zero.  
