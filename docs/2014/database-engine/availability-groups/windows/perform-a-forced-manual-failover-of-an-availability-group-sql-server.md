...
> [!IMPORTANT]  
>  A sincronização de dados com o banco de dados primário não ocorrerá até o banco de dados secundário for retomado. Para obter informações sobre a retomada de um banco de dados secundário, consulte [acompanhamento: Tarefas essenciais após um Failover forçado](#FollowUp) mais adiante neste artigo.  
...
> [!NOTE]  
>  Para obter mais informações sobre os pré-requisitos e recomendações para forçar o failover em um cenário de exemplo que usa um failover forçado para recuperar de uma falha catastrófica, consulte [cenário de exemplo: Usando um Failover forçado para recuperar-se de uma falha catastrófica](perform-a-forced-manual-failover-of-an-availability-group-sql-server.md#ExampleRecoveryFromCatastrophy), mais adiante neste tópico.  
...
5.  Depois de forçar um failover do grupo de disponibilidade, conclua as etapas de acompanhamento necessárias. Para obter mais informações, consulte [acompanhamento: Tarefas essenciais após um Failover forçado](#FollowUp), mais adiante neste tópico.  
...
3.  Depois de forçar um failover do grupo de disponibilidade, conclua as etapas de acompanhamento necessárias. Para obter mais informações, consulte [acompanhamento: Tarefas essenciais após um Failover forçado](#FollowUp), mais adiante neste tópico.  
...
##  <a name="FollowUp"></a> Acompanhar: Tarefas essenciais depois de um failover forçado  
...
1.  Depois de um failover forçado, a réplica secundária na qual foi feito o failover se torna a nova réplica primária. No entanto, para tornar essa réplica de disponibilidade acessível aos clientes, você pode precisar reconfigurar o quorum WSFC ou ajustar a configuração do modo de disponibilidade do grupo de disponibilidade, da seguinte maneira:  
    ...
    -   **Se você fez failover fora do [!INCLUDE[ssFosAuto](../../../includes/ssfosauto-md.md)]:**  Ajuste os votos de quorum de nós WSFC para refletir a nova configuração de grupo de disponibilidade. Se o nó WSFC, que hospeda a réplica secundária de destino, não tiver um voto de quorum WSFC, você poderá precisar forçar o quorum WSFC.  
