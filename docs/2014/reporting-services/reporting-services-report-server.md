...
-   [Use o PowerShell para criar uma VM do Windows Azure com um servidor de relatório no modo nativo](http://msdn.microsoft.com/library/windowsazure/dn449661.aspx).  
...
|**Extensões de segurança personalizadas**|Sim|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] extensões de segurança personalizadas não podem ser implantadas ou usadas no servidor de relatório. O servidor de relatório inclui uma extensão de segurança para fins especiais que é usada sempre que você configura um servidor de relatório para ser executado no modo integrado do SharePoint. Essa extensão de segurança é um componente interno e é necessário para operações integradas.|  
...
|Tarefa|Link|  
|----------|----------|  
|Revisar os requisitos de hardware e software.|[Requisitos de hardware e Software para o Reporting Services no modo do SharePoint](../../2014/sql-server/install/hardware-and-software-requirements-for-reporting-services-in-sharepoint-mode.md).|  
