...
|**Regularmente**|Especifica uma relação de dimensão normal. As seguintes opções são exibidas no painel **Detalhes** :|**Atributo de granularidade**: <br />                      Selecione o atributo que define a granularidade do grupo de medidas com relação à dimensão. Esse atributo normalmente é o atributo de chave da dimensão.|  
|||**Tabela de dimensões** : exibe a tabela principal para a dimensão.|  
|||**Tabela de grupos de medidas** : exibe a tabela de fatos para o grupo de medidas.|  
|||**Relação**: exibe uma grade de colunas de dimensão e colunas de grupos de medidas nas quais a relação se baseia. A grade contém as seguintes colunas:<br /><br /> **Colunas de dimensão**: exibe as colunas associadas ao atributo de granularidade selecionado. Observação: se a dimensão ainda não foi gerada, essa opção será definida como **Gerar**.<br />**Colunas do grupo de medidas** :<br />                              Selecione as colunas no grupo de medidas que estão relacionadas às colunas de dimensão.|  
...
|**Fato**|Especifica uma relação de dimensão de fato. As seguintes opções são exibidas no painel **Detalhes** :|**Atributo de granularidade**: selecione o atributo que define a granularidade do grupo de medidas em relação à dimensão. Esse atributo normalmente é o atributo de chave da dimensão.|  
|||**Tabela de dimensões**: exibe a tabela de dimensões primária.|  
...
|**Muitos para muitos**|Especifica uma relação de muitos para muitos da dimensão. As seguintes opções são exibidas no painel **Detalhes** :|**Dimensão** : exibe a dimensão selecionada.|  
...
|**Mineração de dados**|Especifica uma relação de mineração de dados da dimensão. As seguintes opções são exibidas no painel **Detalhes** :|**Dimensão de destino**: exibe a dimensão de Data Mining selecionada.<br /><br /> Observação: você deve selecionar uma dimensão de mineração de dados para criar uma relação de dimensão de mineração de dados.|  
|||**Dimensão de origem**: selecione a dimensão na qual a dimensão de Data Mining fornece análise preditiva.|  
