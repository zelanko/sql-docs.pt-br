...
7.  Na lista **Executar como** , selecione **Conta de Serviço do SQL Server Agent** ou selecione uma conta proxy com as credenciais que a etapa de trabalho usará. Para obter informações sobre como criar uma conta proxy, consulte [Create a SQL Server Agent Proxy](../../ssms/agent/create-a-sql-server-agent-proxy.md).  
  ...
  7.1 Ao executar o trabalho com um Proxy, é preciso ter os seguintes itens de segurança em vigor para que o trabalho seja executado com êxito.

      As credenciais de logon usadas pelo Proxy, a conta que executa o SQL Server Agent e a conta que executa o serviço do SQL Server exigem as seguintes permissões: Atributo da Política de Segurança Local: substituir um controle completo de Token de Nível de Processo em %SYSTEMROOT%\Temp 
