...
---
# Exportar um dom&#237;nio para um arquivo .dqs
  Este tópico descreve como exportar um domínio para um arquivo .dqs no [!INCLUDE[ssDQSnoversion](../includes/ssdqsnoversion-md.md)] (DQS). Você pode exportar um domínio ou uma base de dados de conhecimento inteira para um arquivo de dados. Para obter informações sobre como exportar uma base de Conhecimento, consulte [Exportar uma Base de dados de conhecimento para um arquivo. DQS](../data-quality-services/export-a-knowledge-base-to-a-dqs-file.md).  
...
3.  No **página Gerenciamento de domínio** (com qualquer guia selecionada), selecione um domínio único ou um domínio composto de **domínio** lista.  
  
4.  Clique no ícone **Exportar dados da Base de Dados de Conhecimento** acima da lista de domínios e clique em **Exportar Domínio**. Como alternativa, clique no domínio a **domínio** lista, aponte para **exportar**, e, em seguida, clique em **Exportar domínio**.  
  
5.  No **Exportar para arquivo de dados** caixa de diálogo, vá para a pasta que você deseja salvar o arquivo no nome do arquivo ou mantenha o nome padrão, manter **os arquivos de dados do DQS (\*. DQS)** como o **Salvar como tipo**, e, em seguida, clique em **Salvar**.  
