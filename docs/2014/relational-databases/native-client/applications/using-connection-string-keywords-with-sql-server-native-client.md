...
|`APP`|Nome do aplicativo que chama [SQLDriverConnect](../../native-client-odbc-api/sqldriverconnect.md) (opcional). Se for especificado, esse valor é armazenado na **Master** coluna **program_name** e é retornado pela [sp_who](/sql/relational-databases/system-stored-procedures/sp-who-transact-sql) e o [APP_NAME](/sql/t-sql/functions/app-name-transact-sql)funções.|  
...
|`WSID`|A ID da estação de trabalho. Normalmente, trata-se do nome de rede do computador em que está o aplicativo (opcional). Se for especificado, esse valor é armazenado na **Master** coluna **nome do host** e é retornado pela [sp_who](/sql/relational-databases/system-stored-procedures/sp-who-transact-sql) e o [HOST_NAME](/sql/t-sql/functions/host-name-transact-sql)função.|  
  
 **Configurações regionais de conversão se aplicam a tipos de dados de hora, numérico, data e moeda. A configuração da conversão é aplicável somente a conversão de saída e só é visível quando numérico, moeda, data ou valores de hora são convertidos em cadeias de caracteres**.  
