---
title: Como implantar um aplicativo
...
R e Python de aplicativos são implantados e gerenciados com o **mssqlctl-pre** utilitário de linha de comando que está incluído no CTP 2.2. Este artigo fornece exemplos de como implantar esses scripts de R e Python como aplicativos da linha de comando.
...
No CTP 2.2, que você pode criar, excluir, listar e executar um aplicativo de R ou Python. A tabela a seguir descreve os comandos de implantação do aplicativo que você pode usar com **mssqlctl-pre**.
...
Antes de configurar aplicativos R e Python, primeiro faça logon em seu SQL Server cluster de big data com o `mssqlctl-pre login` comando. Especifique o endereço IP externo do `service-proxy-lb` ou `service-proxy-nodeport` serviços (por exemplo: `https://ip-address:30777`) junto com o nome de usuário e senha para o cluster.

Você pode obter o endereço IP do **serviço de proxy de lb** ou **serviço de proxy de nodeport** serviço executando este comando em uma janela de bash ou cmd:
