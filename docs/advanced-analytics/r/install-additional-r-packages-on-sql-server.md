...
3.  Métodos de instalação do pacote diferem dependendo se o servidor tem acesso à internet e em sua versão do SQL Server. Os processos recomendados são da seguinte maneira:
...
Esta seção fornece procedimentos detalhados para os seguintes cenários de instalação do pacote de chaves. O melhor método para você usar depende desses factores:

- A versão do SQL Server que você está usando

- Se você é o único proprietário da instância ou está tentando mamaneg pacotes para várias pessoas usando funções de banco de dados.

- Se você estiver instalando os pacotes de um ou vários pacotes com dependências
...
4.  O banco de dados em que você deseja usar o pacote, execute o [criar biblioteca externa](https://docs.microsoft.com/sql/t-sql/statements/create-external-library-transact-sql) instrução.
    ...
    Neste exemplo, vamos supor que a sua conta tem permissão para carregar novos pacotes para o servidor e instalá-los ao escopo compartilhado no banco de dados.
...
4.  Para instalar o pacote para a biblioteca padrão R usada pela instância, execute o R `library()` comando dentro do procedimento armazenado sp_execute_external_script.
    ...
    Se for bem-sucedido, o **mensagens** janela deve relatar uma mensagem, como "pacote 'zoo' descompactados com êxito e verificada somas MD5". Se um pacote exigido já estiver instalado, o processo de instalação anexar e carregar o pacote desejado.
...
1. Antes de começar, certifique-se de que essas condições forem atendidas:
    ...
    + O cliente de R tem a versão mais recente do RevoScale. As versões de pré-lançamento não incluíam algumas funções de gerenciamento de pacote.
...
5.  Execute o comando de R `install.packages` para instalar o pacote.
    ...
    **Instalar o pacote usando uma conexão de internet**
    ...
    Se o pacote de destino depender de outros pacotes, o instalador de R baixará automaticamente as dependências e as instalará para você.
...
Se você estiver instalando os pacotes de um repositório miniCRAN, o processo geral é muito semelhante à instalação de um pacote de um único arquivo compactado. No entanto, em vez de carregar um pacote individual no formato compactado, o repositório de miniCRAN contém o pacote de destino, bem como os pacotes necessários relacionados.
...
3. Para instalar os pacotes para uso com o SQL Server, execute o comando a seguir como parte do código R em um procedimento armazenado.
    ...
    Se for bem-sucedido, o **mensagens** janela deve relatar uma mensagem, como "o pacote 'randomForest' descompactado com êxito e somas de MD5 verificada" e também "concluído encadeados execução".
...
Há várias fontes de pacotes R, os mais conhecidos entre eles são CRAN e Bioconductor. O site oficial da linguagem R (<https://www.r-project.org/>) lista vários desses recursos. Muitos pacotes também são publicados no GitHub, onde é possível obter o código-fonte. No entanto, talvez você também tenha recebido pacotes R desenvolvidos por alguém de sua empresa.
...
Você também deve determinar se o pacote é compatível com a versão do R que está em execução no SQL Server.
...
Para obter mais informações sobre o conteúdo do formato de arquivo zip e como criar um pacote do R, recomendamos este tutorial, que pode ser baixado no formato PDF no site do projeto do R: [Freidrich Leisch: criação de pacotes do R](http://cran.r-project.org/doc/contrib/Leisch-CreatingPackages.pdf).
