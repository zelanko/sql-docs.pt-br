...
Este tópico discute como você pode usar o atributo PDO:: sqlsrv_attr_direct_query para especificar a execução de instrução direta em vez do padrão, que é a execução da instrução preparada.  Quando o driver prepara uma instrução, isso pode resultar em melhor desempenho se a instrução for executada mais de uma vez usando parâmetros associados.  
...
Se uma consulta exigir o contexto foi definido em uma consulta anterior, você deve executar as consultas com PDO:: sqlsrv_attr_direct_query definida como True. Por exemplo, se você usar tabelas temporárias em suas consultas, PDO:: sqlsrv_attr_direct_query deve ser definido como True.  
