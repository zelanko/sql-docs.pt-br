...
## <a name="resolving-the-default-member-when-no-default-member-is-specified"></a>Resolvendo o membro padrão quando não houver um membro padrão especificado.  
 Se nenhum membro padrão for especificado para uma hierarquia de atributo, e a hierarquia de atributo for agregável (a `IsAggregatable` do atributo estiver definida como `True`), o membro (All) é o membro padrão. Se nenhum membro padrão for especificado e a hierarquia de atributo não for agregável (a `IsAggregatable` do atributo estiver definida como `False`), um membro padrão é selecionado do nível superior da hierarquia de atributo.  
