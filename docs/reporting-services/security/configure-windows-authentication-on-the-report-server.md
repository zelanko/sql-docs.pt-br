...
-   [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] deve ser configurado para a Autenticação do Windows. Por padrão, os arquivos Web.config do serviço Web Servidor de Relatórios incluem a configuração \<authentication mode="Windows">. Se você alterar essa configuração para \<modo de autenticação="Formulários">, a Autenticação do Windows para o [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] falhará.  
...
-   Os valores no item \<Explicit> conterão os SPNs configurados no Active Directory para a conta de serviço do [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)].  
