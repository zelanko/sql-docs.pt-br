...
2.  Clique em qualquer lugar no **organizador de diagramas** painel, clique em **novo diagrama**e, em seguida, nomeie o diagrama `Sales Quotas`.  
  
3.  Arraste o **funcionário**, **território de vendas**, e `Date` tabelas do **tabelas** painel para o **diagrama** painel.  
...
6.  No Designer de exibição da fonte de dados, alterar o **FriendlyName** propriedade o **FactSalesQuota** tabela `SalesQuotas`.  
  
7.  Mude para o cubo Tutorial do [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] e clique na guia **Estrutura do Cubo** .  
  
8.  Clique em qualquer lugar no **medidas** painel, clique em **novo grupo de medidas**, clique em `SalesQuotas` no **novo grupo de medidas** caixa de diálogo caixa e, em seguida, clique em **Okey**.  
  
     O `Sales Quotas` consta do grupo de medidas de **medidas** painel. No **dimensões** painel, observe que um novo `Date` dimensão de cubo também esteja definida, com base no `Date` dimensão do banco de dados. Uma nova dimensão do cubo relacionada ao tempo é definida porque o [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] não sabe quais dimensões do cubo relacionadas ao tempo existentes devem ser relacionadas à coluna **DateKey** da tabela de fatos **FactSalesQuota** subjacente ao grupo de medidas Cotas de Vendas. Você alterará isso depois em outra tarefa neste tópico.  
...
12. Excluir o **trimestre do calendário** medir a partir de `Sales Quotas` grupo de medidas.  
...
14. No **nova medida** caixa de diálogo, selecione **contagem distinta** no **uso** lista, verifique `SalesQuotas` está selecionado no **detabeladeorigem** lista, selecione **EmployeeKey** no **coluna de origem** lista e, em seguida, clique em **Okey**.  
...
15. Altere o valor para o **nome** propriedade para o **contagem distinta do funcionário chave** medida a `Sales Person Count`e, em seguida, digite `#,#` como o valor para o **FormatString** propriedade.  
...
4.  Na lista de campos da tabela dinâmica, expanda o `Sales Quotas` grupo de medidas e, em seguida, arraste o **cota de vendas** medida à área valores.  
...
5.  Expanda a dimensão **Sales Territory** e arraste a hierarquia **Sales Territories** definida pelo usuário para Rótulos de Linha.  
     ...
     ![Dimensão de cubo região de vendas](../../2014/tutorials/media/l5-granularity-2.gif "dimensão de cubo região de vendas")  
...
7.  Na lista Campos da Tabela Dinâmica, selecione a hierarquia **Sales Territories** definida pelo usuário e clique na seta para baixo à direita.  
     ...
     ![Hierarquia de regiões de vendas na lista de campos](../../2014/tutorials/media/l5-granularity-1a.png "hierarquia região de vendas na lista de campos")  
...
8.  No filtro, clique na caixa de seleção Selecionar Tudo para desmarcar todas as seleções e escolha somente **América do Norte**.  
     ...
     ![Painel de filtro para selecionar América do Norte](../../2014/tutorials/media/l5-granularity-1b.png "painel de filtro para selecionar América do Norte")  
...
11. Na Tabela Dinâmica, clique na seta para baixo ao lado de Rótulos de Linha. Desmarque todos os anos, com exceção de **FY 2008**.  
     ...
     Observe que apenas o **de julho de 2007** membro do **mês** nível é exibido, em vez do **de julho de 2007**, **de agosto de 2007**e **De setembro de 2007** membros de **mês** nível e que somente o **1 de julho de 2007** membro do `Date` nível for exibida, em vez de todos os 31 dias. Esse comportamento ocorre porque a granulação dos dados na tabela de fatos é trimestral e a granulação do `Date` dimensão é diária. Você alterará esse comportamento na próxima tarefa deste tópico.  
     ...
     ![Cota do valor de valores de vendas](../../2014/tutorials/media/l5-granularity-3.png "cota do valor de valores de vendas")  
...
2.  No painel **Atributos** , selecione **SalesTerritoryKey**e defina a propriedade **AttributeHierarchyVisible** como **False** na janela Propriedades, a propriedade **AttributeHierarchyOptimizedState** como **NotOptimized**e a propriedade **AttributeHierarchyOrdered** como **False**.  
     ...
     Esse atributo é necessário para vincular o **território de vendas** de dimensão para o `Sales Quotas` e **cotas de vendas 1** grupos de medidas como uma dimensão de referência.  
  
3.  No Designer de cubo para o [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] cubo do Tutorial, clique o **uso da dimensão** guia e, em seguida, examine o uso da dimensão dentro de `Sales Quotas` e **cotas de vendas 1** grupos de medidas.  
  
     Observe que o **funcionário** e `Date` dimensões do cubo são vinculados para o **cotas de vendas vendas Quotasand 1** grupos de medidas através de relações regulares. Observe também que a dimensão do cubo **Região de Vendas** não está vinculada a nenhum desses grupos de medidas.  
...
19. No **dimensões** lista, renomeie o **data do pedido** dimensão do cubo para `Date`.  
  
     Renomeando o **data do pedido** dimensão do cubo para `Date` torna mais fácil para os usuários a entender sua função como a dimensão de data primária desse cubo.  
...
1.  Alternar para **Designer de dimensão** para o `Date` da dimensão e, em seguida, clique no **relações de atributo** guia.  
  
     Observe que embora **ano civil** está vinculada a **trimestre do calendário** por meio de **semestre do calendário** atributo, o calendário fiscal atributos estão vinculados somente a um outro; não estão vinculadas a **trimestre do calendário** de atributos e, portanto, não as agregará corretamente no `Sales Quotas` grupo de medidas.  
...
4.  Clique em **OK**.  
     ...
     Observe que uma mensagem de aviso será exibida informando que o `Date` dimensão contém uma ou mais relações de atributo redundantes que podem impedir que os dados sejam agregados quando um atributo de chave não é usado como um atributo de granularidade.  
...
8.  Expanda **FY 2008**, **H1 FY 2008**e **Q1 FY 2008**.  
     ...
     ![Grupo de medidas cota vendas dimensionado corretamente](../../2014/tutorials/media/l5-granularity-7.gif "grupo de medidas cota de vendas dimensionado corretamente")  
  
## <a name="next-lesson"></a>Próxima lição  
 [Lição 6: Definindo cálculos] ((lição-6-definindo-calculations.md)  
