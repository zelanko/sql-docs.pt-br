...
-   Executa a instrução MDX(Multidimensional Expressions) que usa o objeto `Cellset`.  
  
-   Recupera os metadados e os dados do objeto `Position`, recuperado do objeto `Cellset`.  
...
-   Cria uma conexão com um objeto `AdomdConnection`.  
  
-   Executa a instrução MDX usando um objeto `AdomdCommand`.  
  
-   Recupera os metadados e os dados do objeto `Set`, recuperado do objeto `Cellset`.  
