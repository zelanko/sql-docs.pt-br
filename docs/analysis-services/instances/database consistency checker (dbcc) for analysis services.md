---
title: "DBCC (Verificador de consist&#234;ncia de banco de dados) para bancos de dados tabulares e multidimensionais do Analysis Services | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/04/2017"
...
manager: "erikre"
---
# DBCC (Verificador de consist&#234;ncia de banco de dados) para bancos de dados tabulares e multidimensionais do Analysis Services
...
-   Metadados para novos bancos de dados de modelo Tabulares no nível de compatibilidade 1200 consistem em descritores como **TableName** e **PartitionName**.  
...
## Sintaxe de comando para bancos de dados Tabulares 1200  
 Bancos de dados tabulares no nível de compatibilidade 1200 usam metadados tabulares para definições de objeto. A sintaxe completa de DBCC para um banco de dados tabular criado em um nível funcional do SQL Server 2016 é ilustrada no exemplo a seguir.  
  
 As principais diferenças entre as duas sintaxes incluem um namespace mais recente do XMLA, nenhum elemento \<Object> e nenhum elemento \<Model> (há apenas um modelo por banco de dados).  
...
## A sintaxe de comando para bancos de dados Multidimensionais e Tabulares 110x  
...
### Execute os comandos do DBCC no Management Studio  
...
### Rastreamento de saída do DBCC no SQL Server Profiler 2016  
...
4.  No SQL Server Profiler, atividade do DBCC é indicada por meio de eventos de **Command** com uma subclasse de evento de DBCC:  
     ...
     As mensagens de status começam com “Verificando a consistência do \<object>”, “Iniciada a verificação do \<object>” ou “Concluída a verificação do \<object>”.  
...
### Rastreie a saída do DBCC em uma sessão xEvent no SSMS  
...
## Referência: verificações de consistência e erros de bancos de dados multidimensionais  
...
## Referência: verificações de consistência e erros de bancos de dados tabulares  
...
## Resoluções comuns para condições de erro  
...
|-----------|-----------|----------------|  
|**Erros no gerenciador de metadados**<br /><br /> A referência de objeto '\<objectID>' não é válida. Ela não corresponde à estrutura da hierarquia da classe de metadados.|comando malformado|Verifique a sintaxe do comando. Provavelmente, você incluiu um objeto de nível inferior sem especificar um ou mais dos seus objetos pai.|  
|**Erros no gerenciador de metadados**<br /><br /> O \<object> com a ID de '\<objectID>' não existe no \<parentobject> com a ID de '\<parentobjectID>' ou o usuário não tem permissões para acessar o objeto.|Corrupção de índice (multidimensional)|Reprocesse o objeto e todos os objetos dependentes.|  
|**Erro durante a verificação de consistência da partição**<br /><br /> Ocorreu um erro durante a verificação de consistência da partição \<partition-name> do grupo de medidas \<measure-group-name> do cubo \<cube-name> do banco de dados \<database-name>. Processe novamente a partição ou os índices para corrigir a corrupção.|Corrupção de índice (multidimensional)|Reprocesse o objeto e todos os objetos dependentes.|  
...
|**Metadados de tabela corrompidos**<br /><br /> Arquivo de metadados \<table-name> da tabela está corrompido. A tabela principal não foi encontrada no nó DataFileList.|Corrupção de metadados (somente tabular)|Excluir e reimplantar o projeto ou restaurar de um backup e reprocessar.<br /><br /> Consulte [How to Deal with Corruption in Analysis Services (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx) (Como lidar com danos nos bancos de dados do Analysis Services) para obter instruções.|  
|**Corrupção na camada de armazenamento**<br /><br /> Corrupção na camada de armazenamento: a coleção de \<type-name> no \<parent-name> \<parent-type> está corrompida.|Corrupção de metadados (somente tabular)|Excluir e reimplantar o projeto ou restaurar de um backup e reprocessar.<br /><br /> Consulte [How to Deal with Corruption in Analysis Services (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx) (Como lidar com danos nos bancos de dados do Analysis Services) para obter instruções.|  
|**A tabela do sistema está ausente**<br /><br /> A tabela do sistema \<table-name> está ausente.|Corrupção do objeto (somente tabular)|Reprocesse o objeto e todos os objetos dependentes|  
|**As estatísticas de tabela estão corrompidas**<br /><br /> As estatísticas de tabela da tabela do sistema \<table-name> estão ausentes.|Corrupção de metadados (somente tabular)|Excluir e reimplantar o projeto ou restaurar de um backup e reprocessar.<br /><br /> Consulte [How to Deal with Corruption in Analysis Services (blog)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx) (Como lidar com danos nos bancos de dados do Analysis Services) para obter instruções.|  
  
## Desabilitar verificações automáticas de consistência em operações de carregamento de banco de dados por meio do arquivo de configuração msmdsrv.ini  
