...
> [!NOTE]  
>  Para as funções com valor de tabela, as colunas do tipo de tabela de retorno não podem incluir colunas de carimbo de data e hora ou colunas de tipo de dados da cadeia de caracteres não Unicode (como `char`, `varchar` e `text`). Não tem suporte para a restrição NOT NULL.  
...
### <a name="defining-a-table-valued-function-in-transact-sql"></a>Definindo uma função com valor de tabela no Transact-SQL  
 A sintaxe para definir uma função com valor de tabela de CLR é semelhante a de uma função com valor de tabela do [!INCLUDE[tsql](../../includes/tsql-md.md)], com a adição da cláusula `EXTERNAL NAME`. Por exemplo:   
