 ...
 *expressão*  
 É uma expressão numérica válida que especifica o número de linhas a serem retornadas. *expressão* é convertido implicitamente em um **float** valor se PERCENT for especificada; caso contrário, ele será convertido em **bigint**.  
  
 PERCENT  
 Indica que a consulta retorna apenas os primeiros *expressão* porcentagem de linhas do conjunto de resultados. Os valores fracionários são arredondados até o próximo valor inteiro.  
  
 WITH TIES  
 Usado quando você quiser retornar duas ou mais linhas empatadas no último lugar do conjunto de resultados limitado. Deve ser usado com o **ORDER BY** cláusula. **WITH TIES** pode fazer com que mais linhas a serem retornadas ao valor especificado em *expressão*. Por exemplo, se *expressão* é definido como 5 mas 2 linhas adicionais corresponderem aos valores da **ORDER BY** colunas na linha 5, o conjunto de resultados conterá 7 linhas.  
...
-   Como parte de uma instrução SELECT, o otimizador de consulta pode considerar o valor de *expressão* nas cláusulas TOP ou FETCH durante a otimização da consulta. Como SET ROWCOUNT é usada fora de uma instrução que executa uma consulta, seu valor não pode ser considerado em um plano de consulta.  
...
## <a name="interoperability"></a>Interoperabilidade  
 A expressão TOP não afeta instruções que possam ser executadas devido a um gatilho acionado. O **inserido** e **excluído** tabelas nos gatilhos retornará somente as linhas que realmente foram afetadas pelas instruções INSERT, UPDATE, MERGE ou DELETE. Por exemplo, se um INSERT TRIGGER for acionado como resultado de uma instrução INSERT que usou uma cláusula TOP.  
...
|[Incluindo valores vinculados](#tie)|WITH TIES|  
...
###  <a name="tie"></a>Incluindo valores vinculados  
...
## <a name="examples-includesssdwfullincludessssdwfull-mdmd-and-includesspdwincludessspdw-mdmd"></a>Exemplos: [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] e[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]  
 O exemplo a seguir retorna as primeiras 31 linhas que correspondem aos critérios de consulta. O **ORDER BY** cláusula é usada para garantir que a 31 retornadas linhas são as primeiro 31 linhas com base em uma ordem alfabética do `LastName` coluna.  
  
 Usando **superior** sem especificar vínculos.  
...
```  
 ...
 Resultado: 33 linhas são retornadas, porque 3 funcionários denominados Brown ligar para a linha de 31.  
