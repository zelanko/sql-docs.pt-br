...
As seções a seguir mostram exemplos de como usar mssql conf para cada um desses cenários.
...
Para exibir todas as configurações que você definiu explicitamente com **mssql conf**, execute o seguinte comando:

```bash
sudo cat /var/opt/mssql/mssql.conf
```

Observe que as configurações não são mostradas neste arquivo estão usando seus valores padrão.
