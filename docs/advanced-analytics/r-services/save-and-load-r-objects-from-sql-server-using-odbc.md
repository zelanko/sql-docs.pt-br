---
title: "Salvar e carregar objetos de R no SQL Server usando o ODBC | Microsoft Docs"
ms.custom: ""
ms.date: "12/16/2016"
...
---
# Salvar e carregar objetos de R no SQL Server usando o ODBC
...
Para melhorar o desempenho desta etapa crítica, o pacote **RevoScaleR** agora inclui novas funções de serialização e desserialização, que melhoram significativamente o desempenho e armazenam o objeto de forma mais compacta. Além disso, você pode salvar objetos de R diretamente no SQL Server em um ambiente de R, chamando essas novas funções em uma conexão ODBC usando *RxOdbcData*.

## <a name="overview"></a>Visão geral

O pacote **RevoScaleR** agora inclui novas funções que facilitam a salvar objetos de R no SQL Server e, em seguida, ler os objetos da tabela do SQL Server. Essas funções exigem que uma conexão seja estabelecida com o SQL Server usando a fonte de dados *RxOdbcData*.

Em geral, as chamadas de função são modeladas com um repositório de valor de chave simples, no qual a chave é o nome do objeto, e o valor associado com a chave é o objeto varbinary de R a ser movido para dentro ou para fora de uma tabela. 

- Por padrão, qualquer objeto que é chamado do R para ser movido para o SQL Server é serializado e compactado. 
- Por outro lado, quando você carrega um objeto de uma tabela do SQL Server para usar em seu código de R, o objeto é desserializado e descompactado.
- Opcionalmente, você pode especificar que o objeto não seja serializado e escolher um novo algoritmo de compactação para usar em vez do algoritmo de compactação padrão.
...
Para obter ajuda detalhada sobre a sintaxe de cada função, use a Ajuda de R. Os detalhes estarão disponíveis na [Referência de ScaleR](https://msdn.microsoft.com/microsoft-r/scaler/scaler) no MSDN em uma data posterior.
...
[Recursos e tarefas dos Serviços de R](../../advanced-analytics/r-services/sql-server-r-services-features-and-tasks.md)
