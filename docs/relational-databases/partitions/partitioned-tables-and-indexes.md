...
-   Você deseja que a tabela base participe de junções colocadas com mais tabelas que usam colunas de junção diferentes.  
 ...
 Eliminação de partição O processo pelo qual o otimizador de consulta acessa apenas as partições relevantes para atender aos critérios de filtro da consulta.  
...
### <a name="processor-cores-and-number-of-partitions-guidelines"></a>Diretrizes de núcleos de processador e número de partições  
 Para maximizar o desempenho com operações paralelas, é recomendado usar o mesmo número de partições que de núcleos de processador, até no máximo 64 (que é o número máximo de processadores paralelos que o SQL Server pode utilizar).  
