---
title: Ambiente Hospedado da CLR | Microsoft Docs
description: Este artigo descreve como o CLR e o SQL Server são integrados para gerenciar os recursos do sistema de forma uniforme e como a segurança do CAS e do SQL Server são integradas.
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
ms.reviewer: ''
...
[!INCLUDE[appliesto-ss-asdbmi-xxxx-xxx-md](../../includes/appliesto-ss-asdbmi-xxxx-xxx-md.md)]
  ...
  O CLR possui memória coletada de lixo, threading preventivo, serviços de metadados (reflexão de tipo), verificabilidade de código e segurança de acesso ao código. Ele usa metadados para localizar e carregar classes, distribuir as instâncias na memória, resolver invocações de métodos, gerar código nativo, impor a segurança e definir limites de contexto em tempo de execução.  
  
 O CLR e o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] diferem como ambientes de tempo de execução pela forma como tratam memória, threads e sincronização. Este artigo descreve a forma como esses dois tempos de execução são integrados para que todos os recursos do sistema sejam gerenciados uniformemente. Este artigo também abrange a forma como o CLR code access security (CAS) e [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] a segurança são integrados para fornecer um ambiente de execução confiável e seguro para o código do usuário.  
...
###### <a name="scalability-common-threading-scheduling-and-synchronization"></a>Escalabilidade: Threading, agendamento e sincronização comuns  
 ...
 Existem algumas situações em que o código gerenciado de longa duração produzirá automaticamente, e algumas situações em que não o fará. Nas seguintes situações, o código gerenciado de longa duração produzirá automaticamente:
 
 - Se o código chamar o Sistema Operacional SQL (para consultar dados, por exemplo)
 - Se a memória suficiente for alocada para desencadear a coleta de lixo
 - Se o código entrar no modo de prevenção chamando funções do SO

 O código que não faz nenhum dos anteriores, por exemplo, loops apertados que contêm apenas computação, não produzirá automaticamente o agendador, o que pode levar a longas esperas por outras cargas de trabalho no sistema. Nessas situações, cabe ao desenvolvedor ceder explicitamente ligando para a função System.Thread.Sleep() do .NET Framework, ou inserindo explicitamente o modo preemtive com System.Thread.BeginThreadAffinity(), em quaisquer seções de código que são previstas para ser de longa duração. Os exemplos de código a seguir mostram como produzir manualmente usando cada um desses métodos.
...
###### <a name="security-permission-sets"></a>Segurança: conjuntos de permissões  
 O [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] permite que os usuários especifiquem os requisitos de confiabilidade e segurança para o código implantado no banco de dados. Quando as assembléias são carregadas no banco de dados, o autor da montagem pode especificar um dos três conjuntos de permissões para essa montagem: SEGURO, EXTERNAL_ACCESS e INSEGURO.  
...
### <a name="programming-model-restrictions"></a>Restrições do Modelo de Programação  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]também proíbe chamadas para APIs de framework .NET que são anotadas com os atributos de proteção de host **SharedState,** **Synchronization**e **ExternalProcessMgmt.** Isso impede que os assemblies SAFE e EXTERNAL_ACCESS chamem APIs que permitam compartilhar o estado, fazer sincronização e afetar a integridade do processo do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Para obter mais informações, consulte [as restrições do modelo de programação de integração da CLR](../../relational-databases/clr-integration/database-objects/clr-integration-programming-model-restrictions.md).  
