 ...
 **SQLGetInfo** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] em Cliente Nativo difere do **SQLGetInfo** no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] driver ODBC (SQLSRV32. DLL) quando **o SQLGetInfo** é chamado com SQL_KEYWORDS e um comprimento de buffer 0.  O driver do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client retorna SQL_SUCCESS, mas o driver ODBC [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] retorna SQL_SUCCESS_WITH_INFO.  No entanto, quando chamado com um comprimento de buffer não zero que [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] é menor do que a seqüência de palavras-chave de saída, **o SQLGetInfo** no Native Client retorna SQL_SUCCESS_WITH_INFO e um SQLState de 01004.  
  
|fInfoType|rgbInfoValue|  
|---------------|------------------|  
|SQL_ACCESSIBLE_PROCEDURES|"Y"|  
|SQL_ACCESSIBLE_TABLES|"Y"|  
|SQL_ACTIVE_CONNECTIONS|O número de conexões é limitado pelo [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. O driver retorna 0 para esta solicitação **SQLGetInfo.**|  
|SQL_ACTIVE_ENVIRONMENTS|O número de ambientes não é limitado pelo driver. O driver retorna 0 para esta solicitação **SQLGetInfo.**|  
|SQL_ACTIVE_STATEMENTS|O driver retorna 1 para esta solicitação **SQLGetInfo.** O número de identificadores de instrução disponíveis para um aplicativo não é limitado pelo driver, mas a execução padrão de um identificador de instrução bloqueará a execução em qualquer outro identificador.|  
...
|SQL_CONVERT_DATE|Nenhum suporte para conversão do tipo de dados ODBC SQL_TYPE_DATE. O [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] driver Cliente Nativo ODBC suporta o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] tipo de dados de **data-hora** como tipo ODBC SQL_TYPE_TIMESTAMP. Consulte SQL_CONVERT_TIMESTAMP abaixo.|  
...
|SQL_CONVERT_DOUBLE|Nenhum suporte para conversão de tipo de dados ODBC SQL_DOUBLE. O [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] driver Cliente Nativo ODBC suporta o tipo de dados SQL_DOUBLE ODBC como SQL_FLOAT. Consulte SQL_CONVERT_FLOAT abaixo.|  
...
|SQL_DATA_SOURCE_NAME|Nome da fonte de dados atual. Define o valor apontado por *StringLengthPtr* para 0 se a conexão não especificar um nome de origem de dados.|  
...
|SQL_DRIVER_VER|O número da versão do motorista.|  
