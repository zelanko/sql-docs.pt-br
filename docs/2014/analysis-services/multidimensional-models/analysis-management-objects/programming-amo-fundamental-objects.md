...
###  <a name="AMO"></a> Objetos de exceção AmoException  
 O AMO lançará exceções quando problemas diferentes forem encontrados. Para obter uma explicação detalhada sobre exceções, consulte [métodos e as outras Classes AMO](amo-other-classes-and-methods.md). O código de exemplo a seguir mostra a forma correta de capturar exceções no AMO:  
...
1.  Crie um objeto <xref:Microsoft.AnalysisServices.DataSourceView>.  
     ...
     Primeiro verifique a existência do objeto <xref:Microsoft.AnalysisServices.DataSource>; se for `true`, descarte <xref:Microsoft.AnalysisServices.DataSource> e crie-o. Se <xref:Microsoft.AnalysisServices.DataSource> não existir, crie-o.  
...
> [!NOTE]  
>  A cláusula `'WHERE 1=0'` serve para impedir que a consulta retorne linhas para o objeto `DataSet`.  
...
```  
 ...
 No código de exemplo, os métodos `AddTable` e `AddComputedColumn` usam o método `FillSchema` do objeto `DataAdapter` para adicionar um `DataTable` a um `DataSet` e para configurar o esquema de forma que ele corresponda ao da fonte de dados. As propriedades estendidas adicionam as informações necessárias à configuração do esquema para o [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)].  
