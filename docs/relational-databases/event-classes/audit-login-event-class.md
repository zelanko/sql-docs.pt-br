...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] A classe de evento **Audit Login** indica que um usuário teve êxito no logon no [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]. Os eventos nesta classe são disparados por novas conexões ou por conexões reutilizadas de um pool de conexões.  
...
|**BinaryData**|**image**|Configurações de nível de sessão, incluindo nulos ANSI, preenchimento de ANSI, fechamento de cursor na confirmação, concatenação de nulos e identificadores entre aspas.|2|Sim|  
