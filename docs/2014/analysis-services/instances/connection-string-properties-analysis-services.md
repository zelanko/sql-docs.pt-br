...
|`Safety Options`|Define o nível de segurança das ações e funções definidas pelo usuário. Os valores válidos são 0, 1 e 2. Em uma conexão do Excel, essa propriedade é Safety Options=2. Você encontrará informações detalhadas sobre essa opção em <xref:Microsoft.AnalysisServices.AdomdClient.AdomdConnection.ConnectionString%2A>.|  
