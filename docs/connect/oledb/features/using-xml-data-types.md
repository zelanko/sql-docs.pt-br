...
description: Usando tipos de dados XML com o Driver do OLE DB para SQL Server
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
## <a name="ole-db-driver-for-sql-server"></a>Driver do OLE DB para SQL Server 
 DBTYPE_XML é um novo tipo de dados específico ao XML no Driver OLE DB para SQL Server. Além disso, é possível acessar dados XML pelos tipos de OLE DB existentes DBTYPE_BYTES, DBTYPE_WSTR, DBTYPE_BSTR, DBTYPE_XML, DBTYPE_STR, DBTYPE_VARIANT e DBTYPE_IUNKNOWN. Dados armazenados em colunas do tipo XML podem ser recuperados de uma coluna em um Driver OLE DB para SQL Server de conjunto de linhas nos seguintes formatos:  
...
> [!NOTE]  
>  O Driver OLE DB para SQL Server não inclui um leitor SAX, mas o **ISequentialStream** pode ser passado facilmente para objetos SAX e DOM em MSXML.  
  
 **ISequentialStream** deve ser usado para recuperação de documentos XML grandes. As mesmas técnicas usadas para outros tipos de valor extenso também se aplicam ao XML. Para obter mais informações, consulte [usando tipos de valor grande](../../oledb/features/using-large-value-types.md).  
  
 Dados armazenados em colunas do tipo XML em um conjunto de linhas também podem ser recuperados, inseridos ou atualizados por um aplicativo por meio das interfaces comuns como **IRow:: Getcolumns**, **irowchange::**, e **ICommand:: execute**. De maneira semelhante ao caso da recuperação, um programa aplicativo pode passar uma cadeia de caracteres do texto ou um **ISequentialStream** para o Driver OLE DB para SQL Server.  
...
>  Para enviar dados XML em formato de cadeia de caracteres por meio de **ISequentialStream** interface, você deve obter **ISequentialStream** especificando DBTYPE_IUNKNOWN e definir seu *pObject* argumento nulo na associação.  
 ...
 Quando os dados XML recuperados estão truncados porque o buffer de consumidor é muito pequeno, a extensão poderá ser retornada como 0xffffffff, o que significará que ela é desconhecida. Esse comportamento é consistente com sua implementação como um tipo de dados que é transmitido para o cliente sem enviar informações de extensão antes dos dados reais. Em alguns casos, o comprimento real pode ser retornado quando o provedor de buffer do valor inteiro, como **IRowset:: GetData** e onde ocorre a conversão de dados.  
  
 Os dados XML enviados ao SQL Server são tratados como dados binários pelo servidor. Esse comportamento impede que qualquer conversão e permite que o analisador XML autodetecte a codificação XML. Além disso, permite que uma variedade mais ampla de documentos XML (por exemplo, aqueles codificados em UTF-8) seja aceita como entrada no [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].  
...
|DBTYPE_VARIANT (VT_BSTR)|Passar<sup>6,10</sup>|N/D <sup>2</sup>|OKEY<sup>3</sup>|N/D <sup>2</sup>|  
 ...
 <sup>5</sup>formato é caracteres multibyte codificados em página de código de cliente com terminador nulo. Conversão do servidor Unicode fornecido pode causar corrupção de dados, por isso essa associação não é recomendada.  
 ...
 <sup>9</sup>dados são convertidos em Unicode usando a página de código do cliente antes de serem enviados ao servidor. Se a codificação do documento não coincide com a página de código do cliente, corrupção de dados pode ocorrer para que essa associação é altamente desaconselhável.  
  
 <sup>10</sup>um BOM sempre é adicionado aos dados enviados ao servidor. Se os dados já iniciado com um BOM, haverá dois BOMs no início do buffer. O servidor usa o primeiro BOM para reconhecer a codificação como UTF-16 e, em seguida, descarta-lo. O segundo BOM é interpretado como um caractere de espaço incondicional de largura zero.  
 ...
 <sup>12</sup>se o tamanho do buffer é menos de dois caracteres – ou seja, não há espaço suficiente para terminação nula – é reportado um erro de estouro.  
  
> [!NOTE]  
>  Nenhum dado é retornado para valores de XML NULL.  
  
 O padrão XML exige que o XML codificado em UTF-16 seja iniciado com um código de caractere UTF-16 com BOM 0xFEFF. Ao trabalhar com associações WSTR e BSTR, o OLE DB Driver para SQL Server não exige ou adiciona um BOM porque a codificação é insinuada pela associação. Ao trabalhar com associações BYTES, XML ou IUNKNOWN, a intenção é proporcionar simplicidade ao lidar com outros processadores XML e sistemas de armazenamento. Nesse caso, um BOM deve estar presente com XML codificado em UTF-16, e o aplicativo não precisa se preocupar com a codificação real, desde que a maioria dos processadores XML (incluindo o SQL Server) deduz a codificação inspecionando os primeiros bytes do valor. Dados XML recebidos do Driver do OLE DB para SQL Server usando BYTES, XML, ou associações de IUNKNOWN são sempre codificados em UTF-16 com um BOM e sem uma declaração de codificação inserida.  
  
 Conversões de dados fornecidas por serviços principais do OLE DB (**IDataConvert**) não são aplicáveis a DBTYPE_XML.  
  
 A validação ocorre quando os dados são enviados para o servidor. Validação do lado do cliente e as alterações de codificação devem ser tratados pelo seu aplicativo. É recomendável que você não processar os dados XML diretamente, mas em vez disso, deve usar um leitor DOM ou SAX para processá-la.  
...
#### <a name="the-columns-and-procedureparameters-schema-rowsets"></a>Os conjuntos de linhas de esquema de COLUMNS e PROCEDURE_PARAMETERS  
 Adições para os conjuntos de linhas de esquema COLUMNS e PROCEDURE_PARAMETERS incluem as seguintes colunas:  
  
|Nome da coluna|Tipo|Description|  
|-----------------|----------|-----------------|  
|SS_XML_SCHEMACOLLECTION_CATALOGNAME|DBTYPE_WSTR|O nome de um catálogo no qual uma coleção de esquema XML é definida. NULL para uma coluna não XML ou uma coluna XML não digitada.|  
|SS_XML_SCHEMACOLLECTION_SCHEMANAME|DBTYPE_WSTR|O nome de um esquema no qual uma coleção de esquema XML é definida. NULL para uma coluna não XML ou uma coluna XML não digitada.|  
|SS_XML_SCHEMACOLLECTIONNAME|DBTYPE_WSTR|O nome da coleção de esquemas XML. NULL para uma coluna não XML ou uma coluna XML não digitada.|  
...
#### <a name="the-ssxmlschema-schema-rowset"></a>O conjunto de linhas de esquema de SS_XMLSCHEMA  
 Um novo conjunto de linhas de esquema de SS_XMLSCHEMA é introduzido para que os clientes recuperem as informações de esquema XML. O conjunto de linhas SS_XMLSCHEMA contém as seguintes colunas:  
...
#### <a name="the-dbpropsetsqlserverparameter-property-set"></a>O conjunto de propriedades de DBPROPSET_SQLSERVERPARAMETER  
 Para oferecer suporte a **xml** de tipo de dados por meio de OLE DB, OLE DB Driver para SQL Server implementa o novo conjunto de propriedades DBPROPSET_SQLSERVERPARAMETER, que contém os seguintes valores.  
...
#### <a name="the-dbpropsetsqlservercolumn-property-set"></a>O conjunto de propriedades de DBPROPSET_SQLSERVERCOLUMN  
 Para dar suporte a criação de tabelas de **ITableDefinition** interface Driver OLE DB para SQL Server adiciona três colunas novas ao conjunto de propriedades DBPROPSET_SQLSERVERCOLUMN.  
...
#### <a name="the-isscommandwithparameters-interface"></a>A interface ISSCommandWithParameters  
 Para oferecer suporte a **xml** de tipo de dados por meio de OLE DB, OLE DB Driver para SQL Server implementa um número de alterações, incluindo a adição do [ISSCommandWithParameters](../../oledb/ole-db-interfaces/isscommandwithparameters-ole-db.md) interface. Essa nova interface herda da interface OLE DB **ICommandWithParameters**. Além dos três métodos herdados de **ICommandWithParameters**; **GetParameterInfo**, **MapParameterNames**, e **SetParameterInfo**; **ISSCommandWithParameters** fornece o [GetParameterProperties](../../oledb/ole-db-interfaces/isscommandwithparameters-getparameterproperties-ole-db.md) e [SetParameterProperties](../../oledb/ole-db-interfaces/isscommandwithparameters-setparameterproperties-ole-db.md) métodos que são usados para manipular específico do servidor tipos de dados.  
...
#### <a name="the-irowsetchange-interface"></a>A interface IRowsetChange  
 ...
 Se DBTYPE_BSTR, DBTYPE_WSTR ou DBTYPE_VARIANT for especificado, o provedor armazena a instância XML que reside no buffer do consumidor na coluna adequada.  
  
 Se for especificado DBTYPE_IUNKNOWN/ISequentialStream, se o consumidor não especificar um objeto de armazenamento, o consumidor deve criar um **ISequentialStream** objeto com antecedência, associar o documento XML com o objeto e passar o objeto para o provedor por meio de **IRowsetChange:: SetData** método. O consumidor também pode criar um armazenamento de objeto, defina o argumento pObject como IID_ISequentialStream, criar um **ISequentialStream** de objeto e, em seguida, passar o **ISequentialStream** o objeto para o **IRowsetChange:: SetData** método. Em ambos os casos, o provedor pode recuperar o objeto XML por meio de **ISequentialStream** de objeto e inseri-lo em uma coluna adequada.  
