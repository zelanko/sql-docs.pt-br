 ...
 **sp_describe_undeclared_parameters** sempre retorna retorna o status de zero em caso de sucesso. Se o procedimento lançar um erro e o procedimento é chamado como uma RPC, o status de retorno é preenchido com o tipo de erro, conforme descrito na coluna error_type de sys.DM exec_describe_first_result_set. Se o procedimento for chamado de [!INCLUDE[tsql](../../includes/tsql-md.md)], o valor de retorno sempre será zero, até mesmo em casos de erro.  
...
|-----------------|---------------|-----------------|  
|**parameter_ordinal**|**int não NULL**|Contém a posição ordinal do parâmetro no conjunto de resultados. A posição do primeiro parâmetro será especificada como 1.|  
