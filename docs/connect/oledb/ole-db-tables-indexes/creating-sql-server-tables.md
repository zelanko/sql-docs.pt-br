...
description: Criando SQL Server tabelas usando o driver OLE DB para SQL Server
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
...
## <a name="dbcolumndesc-usage"></a>Uso de DBCOLUMNDESC  
 O consumidor pode indicar um tipo de dados de coluna usando o membro *pwszTypeName* ou o membro *wType*. Se o consumidor especificar o tipo de dados em *pwszTypeName*, o driver OLE DB para SQL Server ignorará o valor de *wType*.  
 ...
 O driver OLE DB para SQL Server reconhece um subconjunto de valores DBTYPE enumerados OLE DB no membro *wType* . Para obter mais informações, consulte [mapeamento de tipo de dados em ITableDefinition](../../oledb/ole-db-data-types/data-type-mapping-in-itabledefinition.md).  
...
|DBPROP_COL_DESCRIPTION|Leitura/gravação: leitura/gravação<br /><br /> Padrão: nenhum<br /><br /> Descrição: a propriedade de coluna DBPROP_COL_DESCRIPTION não é implementada pelo OLE DB Driver for SQL Server.<br /><br /> O membro *dwStatus* da estrutura DBPROP retorna DBPROPSTATUS_NOTSUPPORTED quando o consumidor tenta gravar o valor da propriedade.<br /><br /> Definir a propriedade não constitui um erro fatal para o driver de OLE DB para SQL Server. Se todos os outros valores de parâmetro forem válidos, será criada a tabela do [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)].|  
