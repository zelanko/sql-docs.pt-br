---
title: "Replication Agente de Leitor de Log | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Agente de Leitor de Log, executáveis"
  - "Agente de Leitor de Log, referência de parâmetro"
  - "agentes [replicação do SQL Server], Agente de Leitor de Log"
 ...
 **-DistributorSecurityMode** [ **0**| **1**]  
 Especifica o modo de segurança do Distribuidor. Um valor de **0** indica [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] modo de autenticação (padrão) e um valor de **1** indica [!INCLUDE[msCoName](../../../includes/msconame-md.md)] modo de autenticação do Windows.  
 ...
 **-ReadBatchThreshold** *number_of_commands*  
 É o número de comandos de replicação a serem lidos no log de transações, antes de ser emitido para o Assinante pelo Agente de Distribuição. O padrão é 0. Se esse parâmetro não for especificado, o Log Reader Agent lerá o final do log ou o número especificado em **- ReadBatchSize** (número de transações).  
  
 **-RecoverFromDataErrors**  
 Especifica que o Agente de Leitor de Log continuará a executar, quando encontrar erros em dados de colunas publicados de um Publicador não SQL Server. Por padrão, tais erros fazem o Agente de Leitor de Log falhar. Quando você usa **- RecoverFromDataErrors**, dados de coluna são replicados como NULL ou um valor não nulo apropriado e mensagens de aviso são registradas para o [MSlogreader_history](../../../relational-databases/system-tables/mslogreader-history-transact-sql.md) tabela. Esse parâmetro só tem suporte para Editores Oracle.  
