...
ms.topic: "article"
helpviewer_keywords: 
  - "consultas de bancos de dados [SQL Server]"
  - "logons [SQL Server], recriando em outra instância do servidor"
  - "gatilhos [SQL Server], DLL"
  - "mensagens de erro definidas pelo usuário [SQL Server]"
  - "permissões [SQL Server], acesso a metadados"
  - "Mecanismo de texto completo [SQL Server]"
  - "metadados [SQL Server], bancos de dados disponíveis para outras instâncias"
  - "logons [SQL Server Agent], recriando em outra instância do servidor"
  - "failover [SQL Server], gerenciando metadados"
  - "notificações de eventos [SQL Server], metadados"
  ...
  - "servidores vinculados [SQL Server], metadados"
  - "Provedor WMI para eventos de servidor, metadados"
  - "anexando bancos de dados [SQL Server]"
  - "envio de logs [SQL Server], metadados"
  - "criptografia [SQL Server], metadados"
  - "configuração do servidor [SQL Server]"
  - "consultas distribuídas [SQL Server], metadados"
  - "procedimentos armazenados estendidos [SQL Server], metadados"
  - "credenciais [SQL Server], metadados"
...
##  <a name="server_configuration_settings"></a> Parâmetros de configuração de servidor  
...
##  <a name="cross_database_queries"></a> Consultas de bancos de dados  
...
##  <a name="database_ownership"></a> Propriedade de banco de dados  
...
##  <a name="user_defined_error_messages"></a> Mensagens de erro definidas pelo usuário  
 Mensagens de erro definidas pelo usuário residem na exibição do catálogo [sys.messages](../Topic/sys.messages%20\(Transact-SQL\).md). Essa exibição do catálogo é armazenada no **mestre**. Se um aplicativo de banco de dados depender de mensagens de erro definidas pelo usuário e o banco de dados for disponibilizado em outra instância do servidor, use [sp_addmessage](../../relational-databases/system-stored-procedures/sp-addmessage-transact-sql.md) para adicionar essas mensagens definidas pelo usuário à instância do servidor de destino.  

  
##  <a name="event_notif_and_wmi_events"></a> Notificações de eventos e eventos WMI (em nível de servidor)  
...
> **IMPORTANTE:** [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] Em vez disso, use a [Integração CLR](../Topic/Common%20Language%20Runtime%20\(CLR\)%20Integration%20Overview.md).  
...
##  <a name="ifts_service_properties"></a> Mecanismo de texto completo para propriedades do SQL Server  
...
##  <a name="sb_applications"></a> Aplicativos do Service Broker  
