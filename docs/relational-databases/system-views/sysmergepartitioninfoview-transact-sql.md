...
|**tipo**|**tinyint**|Indica o tipo do artigo, que pode ser um dos seguintes:<br /><br /> **0x0A** = tabela.<br /><br /> **0x20** = somente esquema de procedimento.<br /><br /> **0x40** = somente esquema de exibição ou somente o esquema de exibição indexada.<br /><br /> **0x80** = somente esquema de função.|  
...
|**logical_record_deleted_view_rule**|**sysname**|Semelhante ao **logical_record_view**, exceto que mostra linhas filho na tabela "excluída" Atualizar e excluir os gatilhos.|  
|**logical_record_level_conflict_detection**|**bit**|Indica se os conflitos devem ser detectados no nível de registro lógico, ou no nível de linha ou coluna.<br /><br /> **0** = linha ou coluna-nível de detecção de conflito é usada.<br /><br /> **1** = lógico detecção de conflitos de registro é usada, em que uma alteração em uma linha no publicador e a alteração em uma função de linha a mesma lógica registro no assinante é tratado como um conflito.<br /><br /> Quando esse valor é 1, somente a resolução de conflito do nível de registro lógica pode ser usada.|  
