...
`[ @spid1 = ] 'session ID1'` é um número de ID de sessão de [!INCLUDE[ssDE](../../includes/ssde-md.md)] de **Sys. dm_exec_sessions** para o qual o usuário deseja informações de bloqueio. a *sessão ID1* é **int** com um valor padrão de NULL. Execute **sp_who** para obter informações de processo sobre a sessão. Se a *sessão ID1* não for especificada, serão exibidas informações sobre todos os bloqueios.  
  
`[ @spid2 = ] 'session ID2'` é outro número de ID de sessão de [!INCLUDE[ssDE](../../includes/ssde-md.md)] de **Sys. dm_exec_sessions** que pode ter um bloqueio ao mesmo tempo que a *sessão ID1* e sobre o qual o usuário também deseja informações. a *sessão ID2* é **int** com um valor padrão de NULL.  
  
## <a name="return-code-values"></a>Valores do código de retorno  
 0 (êxito)  
  
## <a name="result-sets"></a>Conjuntos de resultados  
 O conjunto de resultados **sp_lock** contém uma linha para cada bloqueio mantido pelas sessões especificadas nos parâmetros **\@spid1** e **\@spid2** . Se nem **\@spid1** nem **\@spid2** forem especificadas, o conjunto de resultados relatará os bloqueios para todas as sessões atualmente ativas na instância do [!INCLUDE[ssDE](../../includes/ssde-md.md)].  
