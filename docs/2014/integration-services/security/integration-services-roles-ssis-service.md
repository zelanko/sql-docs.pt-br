...
# <a name="integration-services-roles-ssis-service"></a>Funções do Integration Services (Serviço do SSIS)
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)][!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] inclui as três funções fixas de nível de `db_ssisadmin`banco de dados,, **db_ssisltduser**e **db_ssisoperator**, para controlar o acesso a pacotes. As funções podem ser implementadas somente em pacotes que são salvos `msdb` no banco [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]de dados do no. Você atribui funções a um pacote por meio do [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]. As atribuições de função são salvas no banco de `msdb` dados.  
