...
# <a name="developing-custom-objects-for-integration-services"></a>Desenvolvendo objetos personalizados para o Integration Services
  Quando o fluxo de controle e fluxo de dados objetos que estão incluídos com [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] não satisfizerem seus requisitos completamente, você pode desenvolver vários tipos de objetos personalizados em seu próprio, incluindo:  
  
-   **Tarefas personalizadas**.  
  
-   **Gerenciadores de conexão personalizada.** Conectam-se a fontes de dados externas sem-suporte atualmente.  
...
6.  Opcionalmente, exibir um link para exemplos e conteúdo da Ajuda para o objeto personalizado, no **caixa de ferramentas do SSIS**.  
  
7.  Criar, implantar e depurar seu novo objeto personalizado conforme descrito em [compilando, implantando e depurando objetos personalizados](../../integration-services/extending-packages-custom-objects/building-deploying-and-debugging-custom-objects.md).  
...
## <a name="providing-links-to-samples-and-help-content"></a>Fornecendo links para exemplos e conteúdo da ajuda  
 Para exibir um link no **caixa de ferramentas do SSIS** para exemplos e conteúdo da Ajuda para um objeto personalizado escrito em código gerenciado, use as seguintes propriedades.  
...
-   <xref:Microsoft.SqlServer.Dts.Runtime.DTSTaskAttribute.HelpKeyword%2A>  
 ...
 Para exibir um link para exemplos e conteúdo da Ajuda para um objeto personalizado escrito em código nativo, adicione entradas no arquivo de Script de Registro (.rgs) para SamplesTag, HelpKeyword e HelpCollection. A seguir está um exemplo.  
...
> [!NOTE]  
>  Muitos [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] provedores de log tem uma interface de usuário personalizada que implementa <xref:Microsoft.SqlServer.Dts.Runtime.Design.IDtsLogProviderUI> e substitui o **configuração** caixa de texto com uma lista suspensa filtrada de gerenciadores de conexão disponíveis. Porém, interfaces do usuário personalizadas não são implementadas para provedores de log personalizados nesta versão do [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]. Especificar um valor para a propriedade <xref:Microsoft.SqlServer.Dts.Runtime.DtsLogProviderAttribute.UITypeName%2A> do <xref:Microsoft.SqlServer.Dts.Runtime.DtsLogProviderAttribute> não tem efeito.  
  
 A tabela a seguir fornece uma referência fácil às interfaces que você deve implementar quando desenvolve uma interface do usuário personalizada para cada tipo de objeto personalizado. Ele também explica que o usuário vê se você optar por não desenvolver uma interface de usuário personalizada para seu objeto, ou se você não conseguir vincular seu objeto à sua interface do usuário usando o **UITypeName** propriedade no atributo do objeto. Embora o potente Editor Avançado possa ser satisfatório para um componente de fluxo de dados, a janela Propriedades é uma solução menos amigável para tarefas e gerenciadores de conexões, e um enumerador de Foreach personalizado não pode ser configurado sem um formulário personalizado.  
...
|Provedor de log|<xref:Microsoft.SqlServer.Dts.Runtime.Design.IDtsLogProviderUI><br /><br /> (Não implementado no [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)])|Caixa de texto **configuração** coluna|  
...
-   Entrada de blog, [processo de compilação de solução do Visual Studio oferecem um aviso sobre a dependência indireta no assembly do .NET Framework devido a referências SSIS](http://go.microsoft.com/fwlink/?LinkId=215662), em blogs.msdn.com.  
