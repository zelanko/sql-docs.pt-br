 ...
 **Para usar SQL Server Management Studio para restaurar um backup de banco de dados**  
...
> [!NOTE]  
>  Ao especificar uma tarefa de restauração usando [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)], é possível gerar o script [!INCLUDE[tsql](../../includes/tsql-md.md)][RESTORE](/sql/t-sql/statements/restore-statements-transact-sql) correspondente clicando em **Script** e selecionando um destino para o script.  
...
## <a name="restoring-from-azure-storage"></a>Restaurando do armazenamento do Azure  
 Ao restaurar um backup armazenado no armazenamento do Azure, a interface do usuário de restauração tem uma nova opção de dispositivo de backup. **URL** na caixa de diálogo **selecionar dispositivos de backup** . Quando você clica em **Adicionar**, isso leva você para a caixa de diálogo **conectar ao Azure** que permite especificar as informações de credencial do SQL para autenticar a conta de armazenamento.  Uma vez conectado à conta de armazenamento, os arquivos de backup são exibidos na caixa de diálogo **localizar um arquivo de backup no Azure** , onde você pode selecionar o arquivo a ser usado para a restauração.  
