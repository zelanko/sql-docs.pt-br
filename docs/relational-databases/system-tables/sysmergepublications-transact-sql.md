...
|**retenção**|**int**|O período de retenção para o conjunto de publicação inteira, onde a unidade é indicada pelo valor da **retention_period_unit** coluna.|  
...
|**allow_anonymous**|**int**|Indica se a publicação permite assinaturas anônimas.<br /><br /> **0** = assinaturas anônimas não permitidas.<br /><br /> **1** = anônimo assinaturas são permitidas.|  
...
|**enabled_for_internet**|**bit**|Indica se os arquivos de sincronização para a publicação são expostos à Internet pelo FTP e outros serviços.<br /><br /> **0** = a sincronização de arquivos podem ser acessados pela Internet.<br /><br /> **1** = a sincronização de arquivos não podem ser acessados pela Internet.|  
...
|**snapshot_in_defaultfolder**|**bit**|Especifica se arquivos de instantâneo são armazenados na pasta padrão:<br /><br /> **0** = os arquivos de instantâneo estão na pasta padrão.<br /><br /> **1** = o instantâneo de arquivos são armazenados no local especificado por **alt_snapshot_folder**.|  
...
|**post_snapshot_script**|**nvarchar(255)**|O ponteiro para um. **sql** arquivo que o Merge Agent é executado depois que todos os a replicação outros scripts de objetos e dados foram aplicados durante uma sincronização inicial.|  
...
|**ftp_address**|**sysname**|Endereço de rede do serviço de protocolo de transferência de arquivo (FTP) para o distribuidor. Especifica onde os arquivos de instantâneo de publicação ficam localizados para serem separados pelo Agente de Mesclagem se o FTP estiver habilitado.|  
...
|**publication_number**|**smallint**|Especifica a coluna de identidade que fornece um mapeamento de 2 bytes para **pubid**. **PubID** é um identificador exclusivo para uma publicação, enquanto o número da publicação só é exclusivo em um banco de dados especificado.|  
...
|**snapshot_jobid**|**binário (16)**|Identifica o trabalho de agente que gera o instantâneo quando o Assinante pode iniciar o processo de geração de instantâneo.|  
