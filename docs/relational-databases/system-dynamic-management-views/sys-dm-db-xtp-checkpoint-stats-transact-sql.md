...
|bytes_to_end_of_log|**bigint**|Não processadas pelo controlador de correspondentes para os bytes entre bytes de log `last_lsn_processed` e `end_of_log_lsn`.|  
|log_consumption_rate|**bigint**|Taxa de consumo de log de transações por controlador (em KB/s).|  
|active_scan_time_in_ms|**bigint**|Tempo gasto pelo controlador de verificar ativamente o log de transações.|  
|total_wait_time_in_ms|**bigint**|Tempo de espera cumulativo para o controlador ao não verificando o log.|  
|waits_for_io|**bigint**|Número de esperas do log de e/s gerada pelo thread de controlador.|  
|io_wait_time_in_ms|**bigint**|Tempo acumulado gasto aguardando e/s de log por thread do controlador.|  
|waits_for_new_log_count|**bigint**|Número de esperas incorridas pelo thread do controlador para um novo log a ser gerado.|  
|new_log_wait_time_in_ms|**bigint**|Tempo acumulado gasto aguardando em um novo log pelo thread de controlador.|  
|idle_attempts_count|**bigint**|Número de vezes que o controlador de transição para um estado ocioso.|  
|tx_segments_dispatched|**bigint**|Número de segmentos visto pelo controlador de e enviadas para os serializadores. Segmento é uma parte de contígua de log que formam uma unidade de serialização. Ele é dimensionado no momento para 1MB, mas pode mudar no futuro.|  
|segment_bytes_dispatched|**bigint**|Contagem total de bytes de bytes enviado pelo controlador para serializadores, desde a reinicialização do banco de dados.|  
|bytes_serialized|**bigint**|Contagem total de bytes serializados desde a reinicialização do banco de dados.|  
|serializer_user_time_in_ms|**bigint**|Tempo gasto pelo serializadores no modo de usuário.|  
|serializer_kernel_time_in_ms|**bigint**|Tempo gasto pelo serializadores no modo kernel.|  
|xtp_log_bytes_consumed|**bigint**|Contagem total de bytes de log consumida desde a reinicialização do banco de dados.|  
...
|hardened_recovery_lsn|**numeric(38)**|Recuperação será iniciado a partir deste LSN.|  
...
|hardened_root_file_watermark|**bigint**|**Interno somente**. A distância é válido para ler o arquivo raiz até (esse é um tipo internamente relevante somente – chamado BSN).|  
|hardened_truncation_lsn|**numeric(38)**|LSN do ponto de truncamento.|  
|log_bytes_since_last_close|**bigint**|Bytes do último Fechar ao final do log atual.|  
|time_since_last_close_in_ms|**bigint**|Tempo decorrido desde o último fechamento do ponto de verificação.|  
...
|recovery_lsn_candidate|**bigint**|**Somente internamente**. Candidato para serem selecionadas como recoverylsn quando current_checkpoint_id for fechado.|  
|outstanding_checkpoint_count|**bigint**|Número de pontos de verificação no pipeline aguardando para ser fechado.|  
|closing_checkpoint_id|**bigint**|ID do ponto de verificação de fechamento.<br /><br /> Serializadores estão trabalhando em paralelo, depois que terminaram concluídos o ponto de verificação é um candidato a ser encerrada pelo thread de fechamento. Mas o thread de fechamento só pode fechar um de cada vez e deve estar em ordem, para que o ponto de verificação de fechamento é aquele que o thread de fechamento está trabalhando.|  
