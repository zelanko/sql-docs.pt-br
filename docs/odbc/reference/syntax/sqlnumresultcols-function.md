...
|HY010|Erro de sequência de função|(DM) uma função de execução assíncrona foi chamada para o identificador de conexão que está associado a *StatementHandle*. Essa função assíncrona ainda estava em execução quando o **SQLNumResultsCols** função foi chamada.<br /><br /> (DM) **SQLExecute**, **SQLExecDirect**, ou **SQLMoreResults** foi chamado para o *StatementHandle* e retornado SQL_PARAM_DATA_ DISPONÍVEL. Essa função foi chamada antes de recuperação para todos os parâmetros de fluxo de dados.<br /><br /> (DM), a função foi chamada antes de chamar **SQLPrepare** ou **SQLExecDirect** para o *StatementHandle*.<br /><br /> (DM) uma função de execução assíncrona (não la) foi chamada para o *StatementHandle* e ainda estava em execução quando esta função foi chamada.<br /><br /> (DM) **SQLExecute**, **SQLExecDirect**, **SQLBulkOperations**, ou **SQLSetPos** foi chamado para o  *StatementHandle* e retorna SQL_NEED_DATA. Essa função foi chamada antes de dados foi enviados para todas as colunas ou parâmetros de dados em execução.<br /><br /> Consulte [função SQLPrepare](../../../odbc/reference/syntax/sqlprepare-function.md) para obter detalhes sobre quando um identificador de instrução pode ser liberado.|  
 ...
 **SQLNumResultCols** pode retornar qualquer SQLSTATE que pode ser retornado por **SQLPrepare** ou **SQLExecute** quando chamado depois **SQLPrepare** e antes de  **SQLExecute**, dependendo de quando a fonte de dados avalia a instrução SQL associada à instrução.  
 ...
 **SQLNumResultCols** pode ser chamado com êxito somente quando a instrução está no estado preparado, executado ou posicionado.  
 ...
 Se a instrução associada *StatementHandle* não retornar colunas, **SQLNumResultCols** define **ColumnCountPtr* como 0.  
  
 O número de colunas retornadas pela **SQLNumResultCols** é o mesmo valor do campo SQL_DESC_COUNT do ird.  
  
 Para obter mais informações, consulte [foi um resultado definido criado?](../../../odbc/reference/develop-app/was-a-result-set-created.md) e [é como os metadados usados?](../../../odbc/reference/develop-app/how-is-metadata-used.md).  
