...
description: SQL Server e Instância Gerenciada do Banco de Dados SQL do Azure permitem que você implemente algumas das funcionalidades usando linguagens .NET usando a integração do CLR (Native Common Language Runtime) como SQL Server módulos do lado do servidor (procedimentos, funções e gatilhos).
