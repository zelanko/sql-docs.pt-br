...
# <a name="sqldriverconnect"></a>SQLDriverConnect
 ...
 Para obter mais informações [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sobre atributos de conexão e comportamentos padrão de driver, consulte [SQLSetConnectAttr](sqlsetconnectattr.md).  
 ...
 Se o valor do parâmetro *DriverCompletion* for SQL_DRIVER_NOPROMPT ou SQL_DRIVER_COMPLETE_REQUIRED e o idioma ou o banco de dados vier da cadeia de conexão e `SQLDriverConnect` for inválido, o retornará SQL_ERROR.  
  
 Se o valor do parâmetro *DriverCompletion* for SQL_DRIVER_NOPROMPT ou SQL_DRIVER_COMPLETE_REQUIRED e o idioma ou o banco de dados vierem das definições de fonte de dado `SQLDriverConnect` ODBC e forem inválidos, o usará o idioma padrão ou o Database para a ID de usuário especificada e retornará SQL_SUCCESS_WITH_INFO.  
  
 Se o valor do parâmetro *DriverCompletion* for SQL_DRIVER_COMPLETE ou SQL_DRIVER_PROMPT e se o idioma ou o banco de `SQLDriverConnect` dados for inválido, o exibirá novamente a caixa de diálogo.  
...
## <a name="sqldriverconnect-support-for-service-principal-names-spns"></a>Suporte de SQLDriverConnect a SPNs (nomes de entidade de serviço)  
 ...
 O SQLDriverConnect aceitará as novas palavras-chave da `ServerSPN` cadeia `FailoverPartnerSPN`de conexão e reconhecerá os novos atributos de conexão SQL_COPT_SS_SERVER_SPN e SQL_COPT_SS_FAILOVER_PARTNER_SPN.  
...
## <a name="examples"></a>Exemplos  
 A chamada a seguir ilustra a quantidade mínima de dados necessários `SQLDriverConnect`para:  
