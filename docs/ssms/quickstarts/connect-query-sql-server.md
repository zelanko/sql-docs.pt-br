...
description: Conecte-se a uma instância do SQL Server usando o SQL Server Management Studio e executando consultas T-SQL básicas.
ms.prod: sql
ms.technology: ssms
ms.topic: quickstart
...
Este guia de início rápido ensina a usar o SSMS (SQL Server Management Studio) para conectar-se à instância do SQL Server e executar alguns comandos T-SQL (Transact-SQL) básicos. O artigo demonstra como fazer o seguinte nas etapas abaixo:
...
Para concluir este artigo, você precisa do SQL Server Management Studio e acesso a uma instância do SQL Server.
...
* **Windows**: [baixe o SQL Server 2019 Developer Edition](https://www.microsoft.com/sql-server/sql-server-downloads).
* **macOS**: [baixe o SQL Server 2019 no Docker](../../linux/quickstart-install-connect-docker.md).
...
1. Inicie o SQL Server Management Studio. Na primeira vez em que você executar o SSMS, a janela **Conectar-se ao Servidor** será aberta. Se ela não for aberta, você poderá abri-la manualmente selecionando **Pesquisador de Objetos** > **Conectar** > **Mecanismo de Banco de Dados**.
    ...
    ![O link de conexão no Pesquisador de Objetos](media/connect-query-sql-server/connect-object-explorer.png)
...
2. Na janela **Conectar-se ao servidor**, siga a lista a seguir:
    ...
    * Para **Autenticação**, selecione **Autenticação do Windows**. Este artigo usa a Autenticação do Windows, mas também é possível fazer o logon do SQL Server. Se você selecionar **Logon do SQL**, deverá inserir um nome de usuário e uma senha. Para obter mais informações sobre os tipos de autenticação, confira [Conectar ao servidor (mecanismo de banco de dados)](../f1-help/connect-to-server-database-engine.md).
    ...
    ![Campo "Nome do servidor" com a opção de usar a instância do SQL Server](media/connect-query-sql-server/connection-2.png)
...
> [!NOTE]
> Neste artigo, você já usou a *Autenticação do Windows* para conectar-se ao SQL Server local, mas esse método não é compatível com o Banco de Dados SQL do Azure. Como tal, essa imagem mostra o uso da autenticação do SQL para se conectar ao banco de dados SQL do Azure. Para obter mais informações, confira [Autenticação local do SQL](../../relational-databases/security/choose-an-authentication-mode.md) e [Autenticação do SQL do Azure](/azure/sql-database/sql-database-security-overview#access-management).
...
1. Clique com o botão direito do mouse na instância do servidor no Pesquisador de Objetos e selecione **Nova Consulta**:
   ...
   ![O link de nova consulta](media/connect-query-sql-server/new-query.png)
...
1. Selecione o banco de dados desejado na lista suspensa de bancos de dados, conforme é mostrado aqui:
   ...
   ![Alterar banco de dados](media/connect-query-sql-server/change-db.png)
...
1. Cole o seguinte snippet de código T-SQL na janela de consulta e, em seguida, selecione **Executar**:
   ...
   ![A lista de resultados](media/connect-query-sql-server/query-results.png)
...
* Também é possível exibir a duração da consulta e o número de linhas retornadas pela consulta executada anteriormente.
    ...
    ![Propriedades da conexão](media/connect-query-sql-server/connection-properties.png)
...
2. Altere o servidor que usa sua consulta.
   ...
   ![O comando Alterar Conexão](media/connect-query-sql-server/change-connection.png)
...
Você também pode se conectar e consultar o [SQL Server](../../azure-data-studio/quickstart-sql-server.md), um [Banco de Dados SQL do Azure](../../azure-data-studio/quickstart-sql-database.md) e o [Azure Synapse Analytics](../../azure-data-studio/quickstart-sql-dw.md) usando o Azure Data Studio.
