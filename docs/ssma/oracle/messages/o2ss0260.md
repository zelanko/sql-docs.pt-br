...
A disposição (criptografia) é um processo de ocultar o código-fonte. A disposição ajuda a proteger o código-fonte dos concorrentes e de outras pessoas que podem fazer o mau uso. No Oracle, é possível encapsular o código-fonte usando `DBMS_DDL` subprogramas.
...
**Etapa 2:** Adicione a `WITH ENCRYPTION` palavra-chave ao código Transact-SQL convertido, conforme mostrado abaixo. Isso vai criptografar seu código em SQL Server.
