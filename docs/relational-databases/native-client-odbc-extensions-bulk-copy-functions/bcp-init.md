 ...
 *szTable*  
 É o nome da tabela do banco de dados a ser copiada de ou para. Este nome também pode incluir o nome do banco de dados ou o nome do proprietário. Por exemplo, **pubs.gracie.titles,** **pubs.. títulos**, **gracie.títulos**, e **títulos** são todos nomes de tabela legal.  
  
 Se *o eDirection* for DB_OUT, *o szTable* também pode ser o nome de uma exibição de banco de dados.  
  
 Se *o eDirection* for DB_OUT e uma instrução SELECT for especificada usando [bcp_control](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-control.md) antes [de bcp_exec](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-exec.md) ser chamado, **bcp_init** *szTable* deve ser definida como NULL.  
  
 *szDataFile*  
 É o nome do arquivo de usuário a ser copiado de ou para. Se os dados forem copiados diretamente das variáveis usando [bcp_sendrow,](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md)defina *szDataFile* para NULL.  
...
## <a name="remarks"></a>Comentários  
 Ligue **para bcp_init** antes de chamar qualquer outra função de cópia em massa. **bcp_init** realiza as inicializações necessárias para uma cópia [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]em massa de dados entre a estação de trabalho e .  
  
 A função **bcp_init** deve ser fornecida com uma alça de conexão ODBC habilitada para uso com funções de cópia em massa. Para habilitar a alça, use [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) com SQL_COPT_SS_BCP definido como SQL_BCP_ON em uma alça de conexão alocada, mas não conectada. A tentativa de atribuir o atributo em um identificador conectado resulta em erro.  
  
 Quando um arquivo de dados é especificado, **bcp_init** examina a estrutura da fonte do banco de dados ou da tabela de destino, não do arquivo de dados. **bcp_init** especifica valores de formato de dados para o arquivo de dados com base em cada coluna na tabela de banco de dados, exibição ou conjunto de resultados SELECT. Essa especificação inclui o tipo de dados de cada coluna, a presença ou ausência de um indicador de comprimento ou nulo e cadeias de caracteres de bytes de terminador nos dados, além da largura de tipos de dados de comprimento fixo. **bcp_init** define esses valores da seguinte forma:  
  
-   O tipo de dados especificado é o tipo de dados da coluna na tabela, exibição ou conjunto de resultados de SELECT do banco de dados. O tipo de dados é enumerado por tipos de dados nativos do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] especificados em sqlncli.h. Os dados são representados em seu próprio formato de computador. Ou seja, os dados de uma coluna de tipo de dados **inteiros** são representados por uma seqüência de quatro bytes que é grande ou pouco endian a partir do computador que criou o arquivo de dados.  
  
-   Se um tipo de dados de banco de dados tiver comprimento fixo, os dados do arquivo de dados também terão comprimento fixo. Funções de cópia em massa que processam dados (por exemplo, [bcp_exec](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-exec.md)) analisam as linhas de dados esperando que o comprimento dos dados no arquivo de dados seja idêntico ao comprimento dos dados especificados na tabela de banco de dados, exibição ou lista de colunaselect. Por exemplo, os dados de uma coluna de banco de dados definida como **char(13)** devem ser representados por 13 caracteres para cada linha de dados no arquivo. Os dados de comprimento fixo podem ser prefixados com um indicador nulo se a coluna do banco de dados permitir valores nulos.  
...
-   Ao copiar para o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], a posição ordinal de uma coluna no arquivo de dados deve ser idêntica à posição ordinal da coluna na tabela do banco de dados. Ao copiar [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]de **, bcp_exec** coloca dados com base na posição ordinal da coluna na tabela de banco de dados.  
  
-   Se um tipo de dados de banco de dados for variável em comprimento (por exemplo, **varbinary(22)** ou se uma coluna de banco de dados pode conter valores nulos, os dados no arquivo de dados são prefixados por um indicador de comprimento/nulo. A largura do indicador varia dependendo do tipo de dados e da versão da cópia em massa.  
 ...
 As cópias em massa para o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] podem ser otimizadas para tabelas que não contêm índices, definindo o modelo de recuperação de banco de dados como SIMPLE ou BULK_LOGGED. Para obter mais informações, consulte [pré-requisitos para registro mínimo em banco de](../../relational-databases/import-export/prerequisites-for-minimal-logging-in-bulk-import.md) dados de importação e alter a [granel](../../t-sql/statements/alter-database-transact-sql.md).  
  
 Se nenhum arquivo de dados for usado, você deve chamar [bcp_bind](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-bind.md) para especificar o formato e [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] o local na memória dos dados de cada coluna e, em seguida, copiar linhas de dados para o bcp_sendrow de [uso](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md).  
