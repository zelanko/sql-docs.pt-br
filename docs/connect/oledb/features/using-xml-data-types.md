 ...
 **ISequentialStream** deve ser usado para recuperação de documentos XML extensos. As mesmas técnicas usadas para outros tipos de valor extenso também se aplicam ao XML. Para obter mais informações, consulte [tipos de valor grande usando](../../oledb/features/using-large-value-types.md).  
...
>  Nenhum dado é retornado para valores de XML NULL.  
 ...
 A validação ocorre quando os dados são enviados para o servidor. Validação do lado do cliente e as alterações de codificação devem ser tratados pelo seu aplicativo. É recomendável que você não processar os dados XML diretamente, mas em vez disso, deve usar um leitor DOM ou SAX para processá-lo.  
