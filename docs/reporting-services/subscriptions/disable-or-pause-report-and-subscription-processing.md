...
Há várias abordagens que você pode usar para desabilitar ou pausar o processamento de relatório e assinatura do [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] . As abordagens neste artigo abrangem a desabilitação de uma assinatura para interromper a conexão com a fonte de dados. Nem todas as abordagens são possíveis com ambos [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] modos de servidor. As seguintes tabelas resumem os métodos e suporte [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] modos de servidor:  
...
>Novo no SQL 2016 Reporting Services *habilitar e desabilitar assinaturas*. Novas opções de interface de usuário permitem que você habilite e desabilite rapidamente as assinaturas. As assinaturas desabilitadas mantêm suas outras propriedades de configuração, como o cronograma, e podem ser facilmente reabilitadas. Você também pode habilitar e desabilitar programaticamente assinaturas ou auditar quais assinaturas são desabilitadas.  
  ...
  ![As habilitar e desabilitar botões de página de assinaturas ](../../reporting-services/subscriptions/media/disable-or-pause-report-and-subscription-processing/subscription-enable-and-disable-buttons.png)  
  
No portal da web, navegue até a assinatura na **Minhas assinaturas** página ou o **assinaturas** página de uma assinatura individual. Selecione uma ou mais assinaturas e clique no botão desabilitar ou no botão habilitar na faixa de opções (veja a imagem acima). A coluna de status será alterado para "Desabilitado" ou "Enabled", respectivamente.  
  
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] escreve uma linha no [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] registrar em log quando uma assinatura é habilitada ou desabilitada. Por exemplo, no arquivo de log do servidor de relatório:  
...
-   **Modo nativo:** no portal da web, selecione o **configurações** botão ![botão configurações](media/ssrs-portal-settings-gear.png) na barra de menus na parte superior da tela de portal da web e selecione **configurações do Site**no menu suspenso. Selecione o **agendas** guia para exibir a página agendas. Selecione as caixas de seleção ao lado de programação (ões) você deseja habilitar ou desabilitar e, em seguida, selecione a **habilitar** ou **desabilitar** botão, respectivamente, para executar a ação desejada. A coluna de status será atualizado para "Desabilitado" ou "Enabled" adequadamente.  
...
Uma maneira de tornar um relatório indisponível é remover temporariamente a atribuição de função que fornece acesso ao relatório. Esta abordagem pode ser usada em todos os relatórios, independentemente de como a conexão de fonte de dados seja feita. Esta abordagem visa apenas o relatório, sem afetar a operação de outros relatórios ou itens.  
 ...
 Para remover a atribuição de função, abra o **segurança** página do relatório no portal da web. Se o relatório herda a segurança de um pai, é possível selecionar **Personalizar segurança** e selecionar **Confirmar** na caixa de diálogo **Segurança do item** para criar uma política de segurança restritiva que omite as atribuições de função que fornecem amplo acesso (por exemplo, você pode remover uma atribuição de função que fornece acesso a Todos e manter a atribuição de função que fornece acesso a um pequeno grupo de usuários, como Administradores).  
...
4.  Expanda o **funções** nó e selecione a função desejada.  
  
5.  Clique com o botão direito do mouse na função e selecione **Propriedades**.  
  
6.  Desmarque a **gerenciar assinaturas individuais** e o **gerenciar todas as assinaturas** tarefas.  
