---
title: "Transa&#231;&#245;es em bancos de dados e transa&#231;&#245;es distribu&#237;das para grupos de disponibilidade AlwaysOn e espelhamento de banco de dados (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/13/2017"
...
helpviewer_keywords: 
  ...
  - "transações envolvendo todos os bancos de dados [SQL Server]"
  - "transações [espelhamento de banco de dados]"
  - "Grupos de disponibilidade [SQL Server], interoperabilidade"
  - "solução de problemas [SQL Server], transações entre bancos de dados"
ms.assetid: 9f7ed895-ad65-43e3-ba08-00d7bff1456d
caps.latest.revision: 33
ms.author: "mikeray"
manager: "jhubbard"
---
# Transa&#231;&#245;es em bancos de dados e transa&#231;&#245;es distribu&#237;das para grupos de disponibilidade AlwaysOn e espelhamento de banco de dados (SQL Server)
...
## Suporte para transações entre bancos de dados na mesma instância do SQL Server  
Transações entre bancos de dados na mesma instância do SQL Server não têm suporte em Grupos de Disponibilidade AlwaysOn. Isso significa que não há dois bancos de dados em uma transação entre bancos de dados que possam ser hospedados pela mesma instância do SQL Server. Isso é verdadeiro mesmo se os bancos de dados fizerem parte do mesmo grupo de disponibilidade.  
  
Também não há suporte para transações entre bancos de dados para espelhamento de banco de dados.  
  
##  <a name="dtcsupport"></a> Suporte a transações distribuídas  
Há suporte para transações distribuídas em Grupos de Disponibilidade AlwaysOn. Isso se aplica a transações distribuídas entre bancos de dados hospedados por duas instâncias do SQL Server diferentes. Isso também se aplica a transações distribuídas entre o SQL Server e outro servidor compatível com o DTC.  
...
-   Grupos de Disponibilidade devem ser criados com o comando **CREATE AVAILABILITY GROUP** e a cláusula **WITH DTC_SUPPORT = PER_DB**. No momento, você não pode alterar um grupo de disponibilidade existente.  

- Todas as instâncias do SQL Server que farão parte do Grupo de Disponibilidade devem ter o SQL Server 2016 ou posterior.
  
 
 ## Sem suporte para transações distribuídas
 Casos específicos em que não há suporte para as transações distribuídas:
 
 -  quando mais de um banco de dados envolvido na transação está no mesmo grupo de disponibilidade.
 
 -  quando pelo menos um banco de dados está em um grupo de disponibilidade e outro banco de dados está na mesma instância do SQL Server. 
 ...
 ## Recomendação
 Em ambientes de produção, você deve deixar o serviço DTC em cluster. Se você não deixar o serviço DTC em cluster, o SQL Server usará o serviço DTC local. Com o serviço DTC local, a disponibilidade geral da solução é reduzida. Quando o DTC estiver em cluster, transações em andamento podem ser recuperadas se o nó de cluster falhar.
...
## Cenário de exemplo com espelhamento de banco de dados  
...
> [!NOTE]  
>  Não há suporte para o uso do Espelhamento de Banco de Dados com o DTC nem para o uso de Grupos de Disponibilidade AlwaysOn com o DTC em maneiras não aprovadas neste tópico.  Isso não significa que os aspectos do produto não relacionados ao DTC não tenham suporte; no entanto, quaisquer problemas decorrentes do uso incorreto de transações distribuídas não terão suporte.  
