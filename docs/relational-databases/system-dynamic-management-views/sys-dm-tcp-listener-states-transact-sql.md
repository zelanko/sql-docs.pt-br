---
title: sys.DM tcp_listener_states (Transact-SQL) | Microsoft Docs
ms.custom: 
ms.date: 06/10/2016
...
|**estado**|**tinyint**|O estado do ouvinte do grupo de disponibilidade, um dos seguintes:<br /><br /> 1 = Online. O ouvinte está escutando e processando solicitações.<br /><br /> 2 = Reinício pendente. o ouvinte está offline, pendente de uma reinicialização.<br /><br /> Se o ouvinte do grupo de disponibilidade estiver escutando na mesma porta que a instância do servidor, esses dois ouvintes sempre terão o mesmo estado.<br /><br /> Não permite valor nulo.<br /><br /> Observação: Os valores nesta coluna são provenientes do objeto TSD_listener. A coluna não dá suporte a um estado offline porque quando o TDS_listener estiver offline, ele não pode ser consultado para o estado.|  
