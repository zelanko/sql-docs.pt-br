...
ms.locfileid: "39085848"
---
# <a name="how-to-perform-realtime-scoring-or-native-scoring-in-sql-server"></a>Como executar pontuação em tempo real ou pontuação nativa no SQL Server
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

Este artigo fornece instruções e exemplos de código para saber como executar a pontuação em tempo real e recursos de pontuação nativos no SQL Server 2017 e o SQL Server 2016. O objetivo de pontuação em tempo real e pontuação nativa é melhorar o desempenho das operações de pontuação em pequenos lotes.

Tanto a pontuação em tempo real como a pontuação nativa são projetados para permitir que você use um modelo do machine learning sem a necessidade de instalar o R. Tudo o que você precisa fazer é obter um modelo previamente treinado em um formato compatível e salvá-la em um banco de dados do SQL Server.
...
+ **Pontuação nativa**: a função PREDICT de T-SQL no SQL Server 2017
+ **Pontuação em tempo real**: usando o sp\_rxPredict o procedimento armazenado no SQL Server 2016 ou SQL Server 2017.
...
+ Neste momento, somente o RevoScaleR e o MicrosoftML podem criar modelos compatíveis. Tipos de modelo adicionais podem ser disponibilizados no futuro. Para obter a lista dos algoritmos com suporte no momento, consulte [em tempo real de pontuação](../real-time-scoring.md).
...
## <a name="realtime-scoring-with-sprxpredict"></a>Em tempo real de pontuação com sp_rxPredict
...
> [!NOTE]
> Pontuação de trabalho e em tempo real, funcionalidade de SQL CLR precisa ser habilitado na instância; Além disso, o banco de dados precisa ser marcado como confiável. Quando você executar o script, essas ações são executadas para você. No entanto, considere as implicações de segurança adicionais antes de fazer isso!
...
> 
> Portanto, você talvez precise filtrar os tipos sem suporte em seus dados de entrada antes de usá-lo para a pontuação em tempo real.
> 
> Para obter informações sobre tipos SQL correspondentes, consulte [mapeamento de tipo de SQL-CLR](https://msdn.microsoft.com/library/bb386947.aspx) ou [Mapeando dados de parâmetro CLR](https://docs.microsoft.com/sql/relational-databases/clr-integration-database-objects-types-net-framework/mapping-clr-parameter-data).
...
## <a name="realtime-scoring-in-microsoft-r-server-or-machine-learning-server"></a>Em tempo real de pontuação no Microsoft R Server ou Machine Learning Server

Machine Learning Server dá suporte a modelos publicados como um serviço web de pontuação distribuído em tempo real. Para obter mais informações, consulte estes tópicos:
