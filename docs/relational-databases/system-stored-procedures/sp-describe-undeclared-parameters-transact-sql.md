...
`[ \@tsql = ] 'Transact-SQL\_batch'` uma ou mais instruções [!INCLUDE[tsql](../../includes/tsql-md.md)]. O *Transact-SQL_batch* pode **ser nvarchar (** _n_ **)** ou **nvarchar (max)** .  
  
`[ \@params = ] N'parameters'` \@params fornece uma cadeia de caracteres de declaração para parâmetros para o lote de [!INCLUDE[tsql](../../includes/tsql-md.md)], da mesma forma que o sp_executesql funciona. *Os parâmetros* podem ser **nvarchar (** _n_ **)** ou **nvarchar (max)** .  
...
|**suggested_tds_type_id**|**int não nulo**|Para uso interno.|  
|**suggested_tds_length**|**int não nulo**|Para uso interno.|  
...
-   Se a entrada \@TSQL não for um lote de [!INCLUDE[tsql](../../includes/tsql-md.md)] válido. A validade é determinada pela análise e análise do lote de [!INCLUDE[tsql](../../includes/tsql-md.md)]. Todos os erros causados pelo lote durante a otimização da consulta ou durante a execução não são considerados ao determinar se o lote de [!INCLUDE[tsql](../../includes/tsql-md.md)] é válido.  
  
-   Se \@params não for nulo e contiver uma cadeia de caracteres que não seja uma cadeia de caracteres de declaração sintaticamente válida para parâmetros, ou se ele contiver uma cadeia de caracteres que declare qualquer parâmetro mais de uma vez.  
  
-   Se a entrada [!INCLUDE[tsql](../../includes/tsql-md.md)] lote declarar uma variável local do mesmo nome que um parâmetro declarado em \@params.  
...
-   Uma expressão com tipos de dados que não dependem de parâmetros não declarados para todas as entradas.  
 ...
 Por exemplo, considere a consulta `SELECT dbo.tbl(@p1) + c1 FROM t1 WHERE c2 = @p2 + 2`. As expressões dbo. tbl (\@P1) + C1 e C2 têm tipos de dados, e Expression \@P1 e \@P2 + 2 não.  
...
-   Dedução simples  
     ...
     Se E (\@p) = \@p e TT (\@p) existir, ou seja, se \@p for diretamente um argumento para uma das expressões listadas no início da etapa 2, o algoritmo de dedução de tipo deduzirá o tipo de dados de \@p para ser TT (\@p). Por exemplo:  
  
    ```sql
    SELECT * FROM t WHERE c1 = @p1 AND @p2 = dbo.tbl(@p3)  
    ```  
  
     O tipo de dados para \@P1, \@P2 e \@P3 será o tipo de dados de C1, o tipo de dados de retorno de dbo. tbl e o tipo de dados de parâmetro para dbo. tbl, respectivamente.  
  
     Como um caso especial, se \@p é um argumento para um operador \<, >, \<= ou > =, as regras de dedução simples não se aplicam. O algoritmo de dedução de tipo usará as regras de dedução gerais explicadas na próxima seção. Por exemplo, se c1 for uma coluna do tipo de dados char(30), considere as duas consultas a seguir:  
    ...
    ```  
     ...
     No primeiro caso, o algoritmo de dedução de tipo deduz **Char (30)** como o tipo de dados para \@p de acordo com as regras anteriores neste tópico. No segundo caso, o algoritmo de dedução de tipo deduz **varchar (8000)** de acordo com as regras de dedução gerais na próxima seção.  
...
1.  O tipo de dados que produz o menor número de conversões implícitas em E (\@p) é selecionado. Se um tipo de dados específico produzir um tipo de dados para E (\@p) diferente de TT (\@p), o algoritmo de dedução de tipo considerará que isso é uma conversão implícita extra do tipo de dados de E (\@p) para TT (\@p).  
...
2.  Se houver vários tipos de dados ligados ao menor número de conversões, o tipo de dados com a precedência mais alta será usada. Por exemplo  
    ...
    ```  
     ...
     Essa regra somente se aplicará se houver uma conversão implícita entre cada tipo de dados ligado de acordo com regra 1 e o tipo de dados com a precedência mais alta. Se não houver nenhuma conversão implícita, a dedução de tipo de dados falhará com um erro. Por exemplo, no `SELECT @p FROM t`de consulta, a dedução de tipo de dados falha porque qualquer tipo de dados para \@p seria igualmente bom. Por exemplo, não há conversão implícita de **int** para **XML**.  
...
4.  Para os propósitos da regra 1, o algoritmo de dedução de tipo considera certas conversões melhores que outras. As conversões, na ordem da melhor para a pior são:  
 ...
 Como um exemplo final, dado um `SELECT NULL + @p`de consulta, **int** é escolhido para \@p porque resulta em uma conversão de tipo (c).  
