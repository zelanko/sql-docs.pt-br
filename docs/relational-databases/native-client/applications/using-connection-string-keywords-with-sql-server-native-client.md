...
|**APLICATIVO**|Nome do aplicativo que chama [SQLDriverConnect](../../../relational-databases/native-client-odbc-api/sqldriverconnect.md) (opcional). Se especificado, esse valor é armazenado no **Master.dbo** coluna **program_name** e é retornado por [sp_who](../../../relational-databases/system-stored-procedures/sp-who-transact-sql.md) e [APP_NAME](../../../t-sql/functions/app-name-transact-sql.md) funções.|  
...
|**WSID**|A ID da estação de trabalho. Normalmente, trata-se do nome de rede do computador em que está o aplicativo (opcional). Se especificado, esse valor é armazenado no **Master.dbo** coluna **hostname** e é retornado por [sp_who](../../../relational-databases/system-stored-procedures/sp-who-transact-sql.md) e [HOST_NAME](../../../t-sql/functions/host-name-transact-sql.md) função.|  
  
> **Observação:** aplicam configurações regionais de conversão de moeda, numéricos, data e tipos de dados de tempo. A configuração da conversão só se aplica à conversão de saída, sendo visível apenas quando valores de moeda, número, data ou hora são convertidos em cadeias de caracteres.  
...
|**Endereço**|SSPROP_INIT_NETWORKADDRESS|O endereço de rede de uma instância do [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] na organização.<br /><br /> Para obter mais informações sobre sintaxe de endereço válida, consulte a descrição do **endereço** palavra-chave ODBC, neste tópico.|  
|**APLICATIVO**|SSPROP_INIT_APPNAME|A cadeia de caracteres que identifica o aplicativo.|  
