...
ms.topic: "article"
helpviewer_keywords: 
  - "logs [Integration Services], habilitando"
...
>  Se você implantar o projeto [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] no servidor [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)], o nível de log definido para a execução do pacote substituirá o log do pacote configurado usando o [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)].  
 ...
 Por padrão, os contêineres de um pacote usam a mesma configuração de registro que o seu contêiner pai. Para obter informações sobre como definir opções de log para contêineres individuais, consulte [Configurar o registro em log por meio de um arquivo de configuração salvo](../../integration-services/performance/configure-logging-by-using-a-saved-configuration-file.md).  
  
### Para habilitar registros em um pacote  
...
4.  Na coluna **Configuração**, selecione um gerenciador de conexões ou clique em **\<Nova conexão>** para criar um novo gerenciador de conexões do tipo apropriado para o provedor de log. Dependendo do provedor selecionado, use um dos seguintes gerenciadores de conexões:  
