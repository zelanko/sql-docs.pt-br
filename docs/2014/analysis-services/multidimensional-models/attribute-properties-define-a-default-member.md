...
## <a name="resolving-the-default-member-when-no-default-member-is-specified"></a>Resolvendo o membro padrão quando não houver um membro padrão especificado.  
 Se nenhum membro padrão for especificado para uma hierarquia de atributo e a hierarquia de atributo for agregável (a `IsAggregatable` propriedade no atributo está definida como `True`), o membro (All) é o membro padrão. Se nenhum membro padrão for especificado e a hierarquia de atributo não for agregável (a `IsAggregatable` propriedade no atributo está definida como `False`), um membro padrão é selecionado do nível superior da hierarquia de atributo.  
  
## <a name="specifying-the-default-member"></a>Especificando o membro padrão  
 Cada atributo em uma dimensão em [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] possui um membro padrão, que pode ser especificada usando o `DefaultMember` propriedade para um atributo. Essa configuração será usada para avaliar expressões se não houver um atributo incluído em uma consulta. Se a consulta especificar a hierarquia de uma dimensão, os membros padrão dos atributos da hierarquia serão ignorados. Se uma consulta não especifica uma hierarquia em uma dimensão, o `DefaultMember` para atributos de dimensão entram em vigor.  
  
 Se o `DefaultMember` configuração de um atributo estiver em branco e sua `IsAggregatable` está definida como `True`, o membro padrão é todos os membros. Se o `IsAggregatable` está definida como `False`, o membro padrão é o primeiro membro do primeiro nível visível.  
...
## <a name="see-also"></a>Consulte também  
 [Configurar o &#40;todos os&#41; nível para hierarquias de atributo](database-dimensions-configure-the-all-level-for-attribute-hierarchies.md)  
