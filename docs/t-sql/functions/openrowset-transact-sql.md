...
## <a name="arguments"></a>Argumentos  
 ...
 '*provider_string*'  
 É uma cadeia de conexão específica ao provedor, que é passada na propriedade DBPROP_INIT_PROVIDERSTRING para inicializar o provedor OLE DB. *provider_string* encapsula normalmente todas as informações de conexão necessárias para inicializar o provedor. Para obter uma lista de palavras-chave que são reconhecidas pelo provedor OLE DB [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], confira [Uso de palavras-chave de cadeia de conexão com o Driver do OLE DB para SQL Server](../../connect/oledb/applications/using-connection-string-keywords-with-oledb-driver-for-sql-server.md).  
...
> [!IMPORTANT]
> O [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] não oferece suporte à leitura de arquivos do Windows.
...
Do [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1 em diante, o data_file pode estar localizado no Armazenamento de Blobs do Azure. Para obter exemplos, consulte [Exemplos de acesso em massa a dados no Armazenamento de Blobs do Azure](../../relational-databases/import-export/examples-of-bulk-access-to-data-in-azure-blob-storage.md).
...
|*code_page*|Indica a página de código de origem na qual são codificados os dados de caracteres do arquivo de dados; por exemplo, 850.<br /><br /> **Importante** As versões anteriores à [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] não dão suporte à página de código 65001 (codificação UTF-8).|  
 ...
 ERRORFILE ='*file_name*'  
 **Aplica-se a:** [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1 e builds posteriores. Começando pelo [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)], o `error_file_path` pode estar no Armazenamento de Blobs do Azure.     
...
**Aplica-se a:** [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1 e builds posteriores.       
...
> [!IMPORTANT]  
> [!INCLUDE[msCoName](../../includes/msconame-md.md)] recomenda que você importe dados XML apenas usando a opção SINGLE_BLOB, em vez de SINGLE_CLOB e SINGLE_NCLOB, porque só SINGLE_BLOB oferece suporte a todas as conversões de codificação do Windows.  
...
Especifica um arquivo de valores separados por vírgula em conformidade com o padrão [RFC 4180](https://tools.ietf.org/html/rfc4180).
 ...
 FORMATFILE ='*format_file_path*'  
  **Aplica-se a:** [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1. Começando com o [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1, o format_file_path pode estar localizado no Armazenamento de Blobs do Azure. Para obter exemplos, consulte [Exemplos de acesso em massa a dados no Armazenamento de Blobs do Azure](../../relational-databases/import-export/examples-of-bulk-access-to-data-in-azure-blob-storage.md). 
...
### <a name="a-using-openrowset-with-select-and-the-sql-server-ole-db-driver"></a>A. Usar OPENROWSET com SELECT e o Driver do OLE DB para SQL Server  
 O exemplo a seguir usa o [Driver do Microsoft OLE DB para SQL Server](../../connect/oledb/oledb-driver-for-sql-server.md) Native Client para acessar a tabela `HumanResources.Department` do banco de dados do AdventureWorks2016 no servidor remoto `Seattle1`. Uma instrução `SELECT` é usada para definir o conjunto de linhas retornado. A cadeia de caracteres de provedor contém as palavras-chave `Server` e `Trusted_Connection`. Essas palavras-chave são reconhecidas pelo Driver do OLE DB [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].  
...
### <a name="d-using-openrowset-and-another-table-in-an-inner-join"></a>D. Usando OPENROWSET e outra tabela em um INNER JOIN  
...
### <a name="e-using-openrowset-to-bulk-insert-file-data-into-a-varbinarymax-column"></a>E. Usando OPENROWSET para inserir dados de arquivo em massa em uma coluna varbinary(max)  
...
### <a name="f-using-the-openrowset-bulk-provider-with-a-format-file-to-retrieve-rows-from-a-text-file"></a>F. Usando o provedor OPENROWSET BULK com um arquivo de formato para recuperar linhas de um arquivo de texto  
...
### <a name="g-specifying-a-format-file-and-code-page"></a>G. Especificando um arquivo de formato e uma página de código  
...
```  
### <a name="h-accessing-data-from-a-csv-file-with-a-format-file"></a>H. Acessando dados de um arquivo CSV com um arquivo de formato  
**Aplica-se a:** [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1.   
```sql
SELECT *
...
### <a name="i-accessing-data-from-a-csv-file-without-a-format-file"></a>I. Acessando dados de um arquivo CSV sem um arquivo de formato
...
### <a name="j-accessing-data-from-a-file-stored-on-azure-blob-storage"></a>J. Acessando dados de um arquivo armazenado no Armazenamento de Blobs do Azure   
**Aplica-se a:** [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1.   
