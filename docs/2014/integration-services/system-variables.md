...
|**InteractiveMode**|Booliano|Indica se o pacote é executado em modo interativo. Se um pacote estiver sendo executado no [!INCLUDE[ssIS](../includes/ssis-md.md)] Designer, essa propriedade será definida como `True`. Se um pacote é executado usando o **DTExec** utilitário de prompt de comando, a propriedade é definida como `False`.|  
