...
|**Indented**|Indica se será preciso gerar XML recuado. O valor padrão de `false` gera XML não recuado e compactado.|  
...
|**Esquema**|Indica se será preciso renderizar o XSD (XML schema definition) ou se os dados XML reais serão renderizados. Um valor de `true` indica que um esquema XML será renderizado. O valor padrão é `false`.|  
