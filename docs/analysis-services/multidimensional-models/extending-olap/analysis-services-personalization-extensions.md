...
### <a name="mdx-extensions-and-enhancements"></a>Extensões e aprimoramentos MDX  
 ...
 O comando UPDATE MEMBER foi adicionado para evitar a recriação de membros quando uma atualização é necessária com a perda de precedência resultante na solução de cálculos. As atualizações não é possível alterar o escopo do membro calculado, mover o membro calculado para um pai diferente ou definir uma opção diferente **solveorder**.  
