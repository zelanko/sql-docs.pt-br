...
ms.topic: "article"
helpviewer_keywords: 
  - "Instrução BULK INSERT, importando dados de um arquivo de dados remoto"
  - "importação em massa [SQL Server], métodos"
  - "exportação em massa [SQL Server], métodos"
  - "função OPENROWSET, BULK INSERT"
  - "importação em massa [SQL Server], segurança"
  - "provedores de conjuntos de linhas em massa [SQL Server]"
  - "exportação em massa [SQL Server], instrução BULK INSERT"
  - "acesso a dados remotos [SQL Server], importação em massa"
  - "importação em massa [SQL Server], instrução BULK INSERT"
  - "operações de importação e exportação em massa Transact-SQL"
...
> **OBSERVAÇÃO** Quando você usa BULK INSERT ou OPENROWSET(BULK...), é importante entender como a versão do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] trata a representação. Para obter mais informações, consulte "Considerações sobre segurança", posteriormente neste tópico.  
...
> **OBSERVAÇÃO** Essa restrição não se aplica ao utilitário **bcp**, pois o cliente lê o arquivo sem levar o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] em consideração.  
