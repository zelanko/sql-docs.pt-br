...
|Exemplo|Description|  
|-------------|-----------------|  
|`=Parameters!` *\<ParameterName>* `.IsMultiValue`|Retorna `False`.<br /><br /> Verifica se um parâmetro é de diversos valores. Se `True`, o parâmetro é de diversos valores e é uma coleção de objetos. Se `False`, o parâmetro é um valor único e é um único objeto.|  
...
|`=Join(Parameters!` *\<MultivalueParameterName>* `.Value,", ")`|Retorna um `String` que consiste em uma lista delimitada por vírgulas de valores em um parâmetro de vários valores. Você pode usar qualquer delimitador no segundo parâmetro para Unir.|  
