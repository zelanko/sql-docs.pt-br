---
title: Propriedades de configuração do Apache Spark e do Apache Hadoop
titleSuffix: SQL Server big data clusters
description: Artigo de referência das propriedades de configuração do Apache Spark e do Apache Hadoop (HDFS).
author: MikeRayMSFT
ms.author: mikeray
...
ms.locfileid: "88645747"
---
# <a name="apache-spark--apache-hadoop-hdfs-configuration-properties"></a>Propriedades de configuração do Apache Spark e do Apache Hadoop (HDFS)

[!INCLUDE[SQL Server 2019](../includes/applies-to-version/sqlserver2019.md)]

Para configurar o Apache Spark e o Apache Hadoop nos Clusters de Big Data, você precisará modificar o perfil de cluster (bdc.json) no momento da implantação.

## <a name="supported-configurations"></a>Configurações com suporte
Os Clusters de Big Data usam os mesmos valores de configuração padrão que o respectivo projeto de software livre.
...
> [!NOTE]
> Há algumas definições de configuração das quais alteramos o padrão. O JSON abaixo lista essas configurações e os respectivos padrões. Essas configurações ainda podem ser alteradas. Para obter as configurações não listadas no JSON, veja os links acima.

> [!NOTE]
> Para incluir o Spark no pool de armazenamento, defina o valor booliano `includeSpark` no arquivo de configuração `bdc.json` em `spec.resources.storage-0.spec.settings.spark`. Confira [Configurar o Apache Spark e o Apache Hadoop nos Clusters de Big Data](configure-spark-hdfs.md) para obter instruções.


###  <a name="big-data-clusters-specific-default-configurations"></a>Configurações padrão específica dos Clusters de Big Data
As configurações listadas no JSON são os padrões específicos do BDC. Elas podem ser modificadas no momento da implantação, se necessário. Elas são listadas aqui, pois são diferentes das respectivas configurações padrão do projeto.
