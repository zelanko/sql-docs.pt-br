...
-   Especifique o número de seções cruzadas ou *dobras*, nos quais os dados de estrutura de partição.  
  
 O **validação cruzada** assistente e em seguida, cria um novo modelo em cada uma das partições, testa o modelo nas outras partições e reporta a precisão do modelo. Após a conclusão, o **validação cruzada** assistente cria um relatório que mostra as métricas para cada dobra e fornece um resumo do modelo na agregação. Essas informações podem ser usadas para determinar o quão bons são os dados subjacentes de um modelo ou comparar modelos diferentes criados com base nos mesmos dados.  
...
1.  No **precisão e validação** grupo de **mineração de dados** de faixa de opções, clique em **validação cruzada**.  
...
3.  No **especificar parâmetros de validação cruzada** na caixa de **número de partições** caixa, escolha o número de partições entre os quais dividir o conjunto de dados. Uma partição é uma seção cruzada de dados selecionada aleatoriamente.  
  
4.  Opcionalmente, defina o número máximo de linhas a serem usadas na validação cruzada digitando um número no **máximo de linhas** caixa de texto.  
...
6.  Selecione um valor da **estado de destino** lista suspensa.  
  
     Se a coluna previsível contiver dados numéricos contínuos, essa opção não estará disponível.  
  
7.  Opcionalmente, especifique um valor para usar como o **limite de destino** ao considerar previsões como precisas. Esse valor é expresso como uma probabilidade, que é um número entre 0 e 1, onde 1 é garantia da previsão da precisão, 0 significa que não há nenhuma possibilidade de a previsão estar correta e .5 corresponde a uma previsão aleatória.  
...
#### <a name="classification-passfail"></a>Classificação: Aprovado/reprovado  
 ...
 Aprovado/reprovado é calculado pela contagem de casos que atendem às seguintes condições: **passar** se o estado previsto com a mais alta probabilidade é o mesmo que o estado de entrada e a probabilidade é maior que o valor especificado para **Limiar de estado**; caso contrário, **falha**.  
 ...
 *Comparação de precisão* é uma medida associada a probabilidade. Se um resultado mais provável é quando você usa o modelo do que quando você faz uma previsão aleatória, o modelo será considerado para fornecer *comparação de precisão positiva*. No entanto, se o modelo faz previsões que serão menos prováveis que a probabilidade aleatória, a pontuação de comparação de precisão é *negativo*. Assim, essa métrica indica a quantidade de aperfeiçoamento que pode ser obtida com o uso do modelo, no qual uma pontuação mais alta é melhor.  
 ...
 *Erro de raiz quadrada média* (RMSE) é um método padrão em estatísticas para observando a comparação de conjuntos de dados diferentes e atenuar as diferenças que podem ser introduzidas pela escala de entradas.  
...
#### <a name="mean-absolute-error"></a>Erro de Média Absoluta  
 O *erro absoluto médio* é o erro médio do valor previsto para o valor real. Ele é calculado pela obtenção da soma absoluta dos erros e descoberta da média desses erros.  
