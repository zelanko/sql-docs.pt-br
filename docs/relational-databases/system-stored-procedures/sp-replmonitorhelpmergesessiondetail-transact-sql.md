...
|-----------------|---------------|-----------------|  
|**PhaseID**|**inteiro**|É a fase da sessão de sincronização que pode ser uma das seguintes:<br /><br /> **0** = linha de inicialização ou Resumo<br /><br /> **1** = carregar<br /><br /> **2** = baixar|  
...
|**PercentComplete**|**vírgula**|Indica a porcentagem do total de alterações aplicada a uma linha de detalhe de um determinado artigo em sessões atualmente em execução ou com falha.|  
|**RelativeCost**|**vírgula**|Indica o tempo gasto para sincronizar o artigo como uma porcentagem do tempo de sincronização total para a sessão.|  
|**Permanência**|**inteiro**|Duração da sessão do agente.|  
|**Suplementos**|**inteiro**|Número de inserções em uma sessão.|  
|**Atualizações**|**inteiro**|Número de atualizações em uma sessão.|  
|**Excluído**|**inteiro**|Número de exclusões em uma sessão.|  
|**Conflitos**|**inteiro**|Número de conflitos ocorrido em uma sessão.|  
|**ErrorID**|**inteiro**|ID de um erro de sessão.|  
|**SeqNo**|**inteiro**|Ordem das sessões no conjunto de resultados.|  
|**RowType**|**inteiro**|Indica que tipo de informação que cada linha no conjunto de resultados representa.<br /><br /> **0** = inicialização<br /><br /> **1** = carregar Resumo<br /><br /> **2** = detalhes do carregamento do artigo<br /><br /> **3** = Resumo de download<br /><br /> **4** = detalhes do download do artigo|  
|**SchemaChanges**|**inteiro**|Número de alterações de esquema em uma sessão.|  
