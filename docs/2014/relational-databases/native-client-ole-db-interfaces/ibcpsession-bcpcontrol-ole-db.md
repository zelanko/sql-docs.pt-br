...
|BCP_OPTION_DELAYREADFMT|Um booliano, se definido como verdadeiro, fará com que [ibcpsession:: Bcpreadfmt](ibcpsession-bcpreadfmt-ole-db.md) Leia na execução. Se false (padrão), ibcpsession:: Bcpreadfmt será imediatamente ler o arquivo de formato. Ocorrerá um erro de sequência se `BCP_OPTION_DELAYREADFMT` é verdadeiro e é chamada de ibcpsession:: BCPColumns ou ibcpsession:: BCPColFmt.<br /><br /> Um erro de sequência também ocorrerá se você chamar `IBCPSession::BCPControl(BCPDELAYREADFMT, (void *)FALSE))` depois de chamar `IBCPSession::BCPControl(BCPDELAYREADFMT, (void *)TRUE)` e ibcpsession:: Bcpwritefmt.<br /><br /> Para obter mais informações, consulte [descoberta de metadados](../native-client/features/metadata-discovery.md).|  
