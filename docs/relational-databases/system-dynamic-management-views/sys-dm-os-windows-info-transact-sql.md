...
|**windows_sku**|**int**|Para Windows, retorna a ID do Windows estoque mantendo SKU (unidade). Para obter uma lista de IDs e descrições, consulte [função GetProductInfo](https://msdn.microsoft.com/library/ms724358.aspx). É anulável. |  
|**os_language_version**|**int**| Para Windows, retorna o identificador de localidade (LCID) do Windows do sistema operacional. Para obter uma lista de valores LCID e descrições, consulte [IDs de localidade atribuídas pela Microsoft](https://go.microsoft.com/fwlink/?LinkId=208080). Não pode ser NULL.|  
