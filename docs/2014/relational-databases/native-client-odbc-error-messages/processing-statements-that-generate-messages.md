...
```  
 ...
 Se a instrução PRINT ou RAISERROR vem depois de uma instrução SQL (como uma instrução SELECT), as informações de PRINT ou RAISERROR são retornadas quando [SQLMoreResults](../native-client-odbc-api/sqlmoreresults.md)posições no resultado definido que contém o erro. **SQLMoreResults** retorna SQL_SUCCESS_WITH_INFO ou SQL_ERROR, dependendo da severidade da mensagem. As mensagens são recuperadas chamando **SQLGetDiagRec** até ele retornar SQL_NO_DATA.  
