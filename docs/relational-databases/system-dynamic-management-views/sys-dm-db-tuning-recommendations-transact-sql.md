...
| **tipo** | **nvarchar(4000)** | O nome da opção de ajuste automático que gerou a recomendação, por exemplo,`FORCE_LAST_GOOD_PLAN` |
| **motivo** | **nvarchar(4000)** | Motivo por que essa recomendação foi fornecida. |
...
| **estado** | **nvarchar(4000)** | Documento JSON que descreve o estado da recomendação. Campos a seguir estão disponíveis:<br />-   `currentValue`-estado atual da recomendação.<br />-   `reason`– constante que descreve o motivo pelo qual a recomendação está no estado atual.|
...
| **Reverter\_ação\_duração** | **time** | Duração da ação de reversão. |
...
| **pontuação** | **int** | Estimado valor/impacto para esta recomendação para o valor de 0 a 100 escala (quanto maior o melhor) |
