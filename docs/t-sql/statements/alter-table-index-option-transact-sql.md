...
>  Os valores 0 e 100 do fator de preenchimento são idênticos em todos os aspectos.  
 ...
 ON  
 Uma mensagem de aviso ocorre quando valores de chave duplicados forem inseridos em um índice exclusivo. Somente as linhas que violam a restrição de exclusividade falharem.  
  
 OFF  
 Uma mensagem de erro ocorre quando valores de chave duplicados forem inseridos em um índice exclusivo. Toda a operação INSERT será revertida.  
 ...
 **Aplica-se a**: do [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] ao [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Aplica-se somente a tabelas columnstore. COLUMNSTORE especifica a descompactação de uma partição compactada com a opção COLUMNSTORE_ARCHIVE. Quando os dados são restaurados, o índice COLUMNSTORE continua a ser compactadas com a compactação de columnstore é usada para todas as tabelas columnstore.  
  
 COLUMNSTORE_ARCHIVE  
 **Aplica-se a**: do [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] ao [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
  
 Aplica-se a tabelas columnstore, que são armazenadas com um índice columnstore clusterizado. COLUMNSTORE_ARCHIVE mais compacta a partição especificada para um tamanho menor. Isso pode ser usado para fins de arquivamento, ou em outras situações que exijam menos armazenamento e possam dispensar mais tempo para armazenamento e recuperação  
...
EM partições **(** { \<partition_number_expression > | \<intervalo >} [ **,**... * n * ] **)** **Aplica-se a**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] por meio de [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Especifica as partições às quais se aplica a configuração DATA_COMPRESSION. Se a tabela não for particionada, o argumento ON PARTITIONS gerará um erro. Se a cláusula ON PARTITIONS não for fornecida, a opção DATA_COMPRESSION se aplica a todas as partições de uma tabela particionada.  
 ...
 **Aplica-se a**: do [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] ao [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
 ...
 Um **SWITCH** ou recompilação de índice online for concluído, assim como não há operações de bloqueio para essa tabela. *WAIT_AT_LOW_PRIORITY* indica que, se o **SWITCH** ou operação de recompilação de índice online não pode ser concluída imediatamente, ele espera. A operação mantém bloqueios de baixa prioridade, permitindo que outras operações que mantêm bloqueios em conflito com a instrução DDL para continuar. A omissão de **WAIT AT LOW PRIORITY** opção é equivalente a `WAIT_AT_LOW_PRIORITY ( MAX_DURATION = 0 minutes, ABORT_AFTER_WAIT = NONE)`.  
  
MAX_DURATION = *tempo* [**minutos** ]  
 O tempo (um valor inteiro especificado em minutos) que o **SWITCH** ou espera de bloqueio de recompilação de índice online que deve ser adquirido, ao executar o comando DDL. A opção SWITCH ou a operação de recompilação de índice online tenta ser concluída imediatamente. Se a operação está bloqueada para o **MAX_DURATION** de tempo, uma da **ABORT_AFTER_WAIT** ações é executada. **MAX_DURATION** hora é sempre em minutos e a palavra **minutos** pode ser omitido.  
