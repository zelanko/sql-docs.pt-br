---
title: Problemas conhecidos no serviços de Machine Learning | Microsoft Docs
ms.prod: sql
...
Este artigo descreve problemas conhecidos ou limitações com componentes que são fornecidos como uma opção no SQL Server 2016 e SQL Server 2017 de aprendizado de máquina.
...
- Serviços de R (no banco de dados) do Machine Learning
- Serviços para o Python (no banco de dados) do Machine Learning
...
### <a name="r-script-runtime-error-sql-server-2017-cu5-cu7-regression"></a>Erro de tempo de execução do Script R (SQL Server 2017 CU5 CU7 regressão)
...
### <a name="install-the-latest-service-release-to-ensure-compatibility-with-microsoft-r-client"></a>Instalar a versão mais recente do serviço para garantir a compatibilidade com o Microsoft R Client
...
### <a name="r-components-missing-from-cu3-setup"></a>Componentes de R ausentes da configuração CU3
...
### <a name="unable-to-install-python-components-in-offline-installations-of-sql-server-2017-ctp-20-or-later"></a>Não é possível instalar componentes do Python em instalações offline do SQL Server 2017 CTP 2.0 ou posterior
...
### <a name="launchpad-services-fails-to-start-if-the-version-is-different-from-the-r-version"></a>Os serviços do Launchpad não pode ser iniciado se a versão for diferente da versão do R
...
> *O serviço do Launchpad do SQL Server falhou ao iniciar devido ao seguinte erro: O serviço não respondeu à solicitação de início ou controle de forma oportuna.*
...
### <a name="remote-compute-contexts-are-blocked-by-a-firewall-in-sql-server-instances-that-are-running-on-azure-virtual-machines"></a>Contextos de computação remota estão bloqueados por um firewall nas instâncias do SQL Server em execução em máquinas virtuais do Azure
...
**Aplica-se a:** serviços de R do SQL Server 2016 Express Edition

### <a name="performance-limits-when-libraries-used-by-sql-server-are-called-from-other-tools"></a>Limites de desempenho quando as bibliotecas usadas pelo SQL Server são chamadas de outras ferramentas
...
### <a name="sql-server-data-tools-does-not-support-permissions-required-by-external-scripts"></a>SQL Server Data Tools não oferece suporte a permissões exigidas para scripts externos
...
### <a name="external-script-execution-is-throttled-due-to-resource-governance-default-values"></a>Execução de script externo está limitada devido a valores de padrão de governança de recursos
...
### <a name="access-denied-warning-when-executing-r-scripts-on-sql-server-in-a-non-default-location"></a>Acesso negado aviso durante a execução de scripts do R no SQL Server em um local não padrão
...
### <a name="serialization-error-between-old-and-new-versions-of-revoscaler"></a>Erro de serialização entre versões novas e antigas do RevoScaleR
...
### <a name="real-time-scoring-does-not-correctly-handle-the-learningrate-parameter-in-tree-and-forest-models"></a>Pontuação em tempo real não lida corretamente com o _learningRate_ parâmetro nos modelos de árvore e de floresta
...
### <a name="avoid-clearing-workspaces-when-you-execute-r-code-in-a-includessnoversionincludesssnoversion-mdmd-compute-context"></a>Evitar a limpeza de espaços de trabalho quando você executar o código R em um [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] contexto de computação
...
### <a name="limited-support-for-sql-data-types-in-spexecuteexternalscript"></a>Suporte limitado para tipos de dados SQL no sp\_execute\_externo\_script
...
Qualquer processamento de dados de cadeia de caracteres de [!INCLUDE[tsql](../includes/tsql-md.md)] para R e, em seguida, [!INCLUDE[tsql](../includes/tsql-md.md)] novamente pode resultar na corrupção. Isso ocorre devido a diferentes codificações usadas em R e em [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)], bem como os vários agrupamentos e linguagens que têm suporte em R e [!INCLUDE[tsql](../includes/tsql-md.md)]. As cadeias de caracteres em codificação não-ASCII podem ser possivelmente tratadas de forma incorreta.

Quando você envia dados de cadeia de caracteres para R, convertê-lo em uma representação de ASCII, se possível.

Essa limitação se aplica a dados passados entre o SQL Server e o Python também. Caracteres multibyte devem ser passados como UTF-8 e armazenados como Unicode.
...
### <a name="variable-scoping-error-when-you-use-the-transformfunc-parameter"></a>Variável de erro de escopo quando você usa o parâmetro transformFunc
...
Paralelismo para `rxExec` entre vários processos está planejado para uma versão futura.

### <a name="increase-the-maximum-parameter-size-to-support-rxgetvarinfo"></a>Aumentar o tamanho máximo do parâmetro para dar suporte a rxGetVarInfo
...
### <a name="call-to-pretrained-model-fails-if-path-to-model-is-too-long"></a>Chamada para o modelo pré-treinado falhará se o caminho para o modelo é muito longo
...
### <a name="error-when-saving-serialized-model-to-sql-server"></a>Erro ao salvar serializou o modelo para o SQL Server
...
### <a name="telemetry-warning-on-successful-execution-of-python-code"></a>Aviso de telemetria em uma execução bem-sucedida de código do Python
...
### <a name="revolution-r-enterprise-is-not-supported"></a>Não há suporte para o Revolution R Enterprise
