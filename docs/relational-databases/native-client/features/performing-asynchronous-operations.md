...
> [!NOTE]  
>  Os objetos de fonte de dados usados para o pool de conexões não podem chamar a interface **ISSAsynchStatus** no provedor de OLE DB de cliente [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] nativo. A interface **ISSAsynchStatus** não é exposta para objetos da fonte de dados em pool.  
>   
...
>   
>  Além disso, a DLL de proxy de comunicação remota/stub (no MDAC 2,8) não pode chamar a interface **ISSAsynchStatus** no cliente nativo [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]. A interface **ISSAsynchStatus** não é exposta por comunicação remota.  
>   
