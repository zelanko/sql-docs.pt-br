...
-   [Definições de configuração geral (RSReportServer. config)](#bkmk_generalconfiguration)  
...
-   [Configuração geral de extensão (s) da interface do usuário de entrega](#bkmk_ui)  
  
-   [Extensões (arquivo RSReportServer. config) modo nativo](#bkmk_extensions)  
  
    -   [Configuração geral de extensão (s) de entrega](#bkmk_extensionsgeneral)  
  
        -   [Definições de configuração de extensão de entrega de compartilhamento de arquivo](#bkmk_fileshare_extension)  
  
        -   [Definições de configuração de extensão de email do servidor de relatório](#bkmk_email_extension)  
  
        -   [Configuração da extensão da biblioteca de documentos do SharePoint do servidor de relatório](#bkmk_documentlibrary_extension)  
  
        -   [Configuração de extensão de entrega nula](#bkmk_null_extension)  
  
    -   [Configuração geral de extensão (s) da interface do usuário de entrega](#bkmk_ui)  
  
    -   [Configuração geral de extensões de renderização](#bkmk_rendering)  
  
    -   [Configuração geral de extensão (s) de dados](#bkmk_data)  
  
    -   [Configuração geral de extensões de consulta semântica](#bkmk_semantic)  
  
    -   [Configuração de geração de modelo](#bkmk_model)  
  
    -   [Configuração da extensão de segurança](#bkmk_security)  
  
    -   [Configuração da extensão de autenticação](#bkmk_authentication)  
  
    -   [Processamento de eventos](#bkmk_eventprocessing)  
  
    -   [Personalização da definição de relatório](#bkmk_reportdefinition)  
  
    -   [RDLSandboxing](#bkmk_rdlsandboxing)  
  
-   [MapTileServerConfiguration (arquivo RSReportServer. config)](#bkmk_MapTileServer)  
  
-   [Arquivo de configuração padrão para um servidor de relatório no modo nativo](#bkmk_nativedefaultfile)  
  
-   [Arquivo de configuração padrão para um servidor de relatório no modo do SharePoint](#bkmk_sharepointdefaultfile)  
...
##  <a name="bkmk_generalconfiguration"></a>Definições de configuração geral (RSReportServer. config)  
 A tabela a seguir fornece informações sobre as configurações gerais que aparecem na primeira parte do arquivo. As configurações são apresentadas na ordem em que aparecem no arquivo de configuração. A última coluna da tabela indica se a configuração se aplica a **um servidor de** relatório no modo nativo **(N)** ou a um servidor de relatório no modo do SharePoint ou a ambos.  
  
> [!NOTE]  
>  Neste tópico, "o número inteiro máximo" se refere ao valor INT_MAX de 2147483647.  Para obter mais informações, consulte [limites](https://msdn.microsoft.com/library/296az74e\(v=vs.110\).aspx) dehttps://msdn.microsoft.com/library/296az74e(v=vs.110).aspx)inteiros (.  
...
|**ConnectionType**|Especifica o tipo de credenciais que o servidor de relatório usa para se conectar ao banco de dados do servidor de relatório. Os valores válidos são `Default` e `Impersonate`. 
  `Default` será especificado se o servidor de relatório for configurado para usar um logon do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ou a conta de serviço para se conectar ao banco de dados do servidor de relatório. 
  `Impersonate` será especificado se o servidor de relatório usar uma conta do Windows para se conectar ao banco de dados do servidor de relatório.|N|  
...
|**DatabaseQueryTimeout**|Especifica o número de segundos após o qual uma conexão com o banco de dados do servidor de relatório atinge o tempo limite. Esse valor é passado para a propriedade System. Data. SqlClient. SQLCommand. CommandTimeout. Os valores válidos variam de 0 a 2147483647. O padrão é 120. Um valor 0 especifica um tempo de espera ilimitado e, portanto, não é recomendado.|N|  
...
|**AlertingExecutionLogCleanup** Alguns|O padrão é 10.080.<br /><br /> Determina quanto tempo deve manter os valores do log de execução de Alertas. O padrão é 7 dias.|S|  
...
|**LogonMethod**|Esse valor especifica o tipo de logon `RSWindowsBasic`para. Se você especificar `RSWindowsBasic`, esse valor será necessário. Os valores válidos são 2 ou 3, onde cada valor representa o seguinte:<br /><br /> 
  `2` = Logon de rede de servidores de alto desempenho para autenticar senhas de texto não criptografado<br /><br /> 
...
|**Territórios**|Esse valor é usado para `RSWindowsBasic`. Especifica uma partição de recurso que inclui recursos de autorização e autenticação usados para controlar o acesso a recursos protegidos em sua organização.|N|  
...
|`MemorySafetyMargin`|Especifica uma porcentagem de `WorkingSetMaximum` que define o limite entre cenários de pressão média e baixa. O valor padrão é 80. Para obter mais informações `WorkingSetMaximum` sobre e configurar a memória disponível, consulte [Configurar a memória disponível para aplicativos do servidor de relatório](../report-server/configure-available-memory-for-report-server-applications.md).|N,S|  
...
|**Reciclar**|Especifica um momento de reciclagem para o domínio de aplicativo, medido em minutos. O intervalo de valores válidos é de 0 ao inteiro máximo. O padrão é 720.|N,S|  
...
|**PolicyLevel**|Especifica o arquivo de configuração de política de segurança. O valor válido é Rssrvrpolicy. config. Para obter mais informações, consulte [usando Reporting Services arquivos de política de segurança](../extensions/secure-development/using-reporting-services-security-policy-files.md).|N,S|  
...
##  <a name="bkmk_UI"></a>Interface do usuário (arquivo RSReportServer. config)  
...
|**ReportBuilderTrustLevel**|Não modifique este valor; ele não é configurável. No [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] e em versões posteriores, Construtor de relatórios é `FullTrust`executado apenas no. Para obter mais informações, consulte [Configurar o acesso do Construtor de Relatórios](configure-report-builder-access.md) . Para obter mais informações sobre como descontinuar o modo de confiança parcial, consulte [Funcionalidade descontinuada para SQL Server Reporting Services no SQL Server 2014](../discontinued-functionality-to-sql-server-reporting-services-in-sql-server.md).|N,S|  
|`PageCountMode`|Somente para o Gerenciador de Relatórios, esta configuração especifica se o servidor de relatório calcula um valor de contagem de página antes de o relatório ser processado ou enquanto o relatório é exibido. Os valores válidos são `Estimate` (padrão) e `Actual`. Use `Estimate` para calcular informações de contagem de página enquanto o usuário exibe o relatório. Inicialmente, a contagem de página é definida como 2 (para a página atual mais uma página adicional), mas aumenta conforme o usuário navega pelo relatório. Use `Actual` se quiser calcular a contagem de página com antecedência, antes que o relatório seja exibido. 
  `Actual` é fornecida para compatibilidade com versões anteriores. Se `PageCountMode` for definido como `Actual`, o relatório inteiro deverá ser processado para obter uma contagem de páginas válida, aumentando o tempo de espera antes de o relatório ser exibido.|N,S|  
  
##  <a name="bkmk_extensions"></a>Extensões (arquivo RSReportServer. config) modo nativo  
...
###  <a name="bkmk_extensionsgeneral"></a>Configuração geral de extensão (s) de entrega  
...
####  <a name="bkmk_fileshare_extension"></a>Definições de configuração de extensão de entrega de compartilhamento de arquivo  
...
####  <a name="bkmk_email_extension"></a>Definições de configuração de extensão de email do servidor de relatório  
 O email do servidor de relatório usa um dispositivo de rede SMTP para enviar relatórios a endereços de email. Esta extensão de entrega deve ser configurada antes de ser usada. Para obter mais informações, consulte [configurar um servidor de relatório para entrega de email &#40;SSRS Configuration Manager&#41;](../../sql-server/install/configure-a-report-server-for-e-mail-delivery-ssrs-configuration-manager.md) e [entrega de email no Reporting Services](../subscriptions/e-mail-delivery-in-reporting-services.md).  
...
|**SMTPConnectionTimeout**|Especifica um valor inteiro que indica o número de segundos a aguardar por uma conexão de soquete válida com o serviço SMTP antes de atingir o tempo limite. O padrão é 30 segundos, mas esse valor será ignorado se **SendUsing** for definido como 2.|  
...
|**De**|Especifica um endereço de email do qual os relatórios são enviados no *abc@host.xyz*formato. O endereço aparece na linha **De** de uma mensagem de email de saída. Este valor é necessário se você estiver usando um servidor SMTP remoto. Deve ser uma conta de email válida que tenha permissão para enviar email.|  
...
|**SendEmailToUserAlias**|Esse valor funciona com **DefaultHostName**.<br /><br /> Quando **SendEmailToUserAlias** é definido como `True`, os usuários que definem assinaturas individuais são automaticamente especificados como destinatários do relatório. O campo **Para** fica oculto. Se esse valor for `False`, o campo **para** será visível. Defina este valor como `True` se desejar ter controle máximo na distribuição de relatórios. Os valores válidos incluem os seguintes:<br /><br /> `True`= O endereço de email do usuário que está criando a assinatura é usado. Esse é o valor padrão.<br /><br /> 
...
####  <a name="bkmk_null_extension"></a>Configuração de extensão de entrega nula  
...
###  <a name="bkmk_ui"></a>Configuração geral de extensão (s) da interface do usuário de entrega  
...
-   [Configurações de informações de dispositivo para extensões de renderização &#40;Reporting Services&#41;](../device-information-settings-for-rendering-extensions-reporting-services.md)  
 ...
 Para obter informações sobre os atributos para a ** \<extensão filho>** elemento sob ** \<render>**, consulte o seguinte:  
...
###  <a name="bkmk_data"></a>Configuração geral de extensão (s) de dados  
...
###  <a name="bkmk_model"></a>Configuração de geração de modelo  
...
###  <a name="bkmk_security"></a>Configuração da extensão de segurança  
...
###  <a name="bkmk_authentication"></a>Configuração da extensão de autenticação  
...
###  <a name="bkmk_reportdefinition"></a>Personalização da definição de relatório  
...
##  <a name="bkmk_MapTileServer"></a>MapTileServerConfiguration (arquivo RSReportServer. config)  
 `MapTileServerConfiguration`define as definições de [!INCLUDE[msCoName](../../includes/msconame-md.md)] configuração para os serviços Web do Bing Maps que fornecem um plano de fundo de bloco para um item de relatório de mapa em um relatório publicado em um servidor de relatório. Todos os elementos filho são necessários.  
 ...
 **C:\Arquivos de Programas\Microsoft SQL Server\MSRS12.MSSQLSERVER\Reporting Services\ReportServer**  
...
##  <a name="bkmk_sharepointdefaultfile"></a>Arquivo de configuração padrão para um servidor de relatório no modo do SharePoint  
 Por padrão, o arquivo rsreportserver.config é instalado no seguinte local:  
  
 **C:\Arquivos de Programas\Common Files\Microsoft Shared\Web Server Extensions\14\WebServices\Reporting**  
...
## <a name="see-also"></a>Consulte Também  
 [Modifique um arquivo de configuração de Reporting Services &#40;RSreportserver. config&#41;](modify-a-reporting-services-configuration-file-rsreportserver-config.md)   
 ...
 [Inicializar um servidor de relatório &#40;Configuration Manager SSRS&#41;](../install-windows/ssrs-encryption-keys-initialize-a-report-server.md)   
